...
we have $\Card(\dom(S_{a_i})) \le \Card(\dom(S_{a_{i+1}}))$ for all $i \in [1,h-1]$.
By the choice of $b_i$ we also have 
$\Card(S_{a_{i}}) = \Card(S_{b_{i}}) \not = \Card(S_{a_{i+1}})$.
We deduce that
$\Card(\dom(S_{a_i})) <  \Card(\dom(S_{a_{i+1}}))$ for all $i \in [1,h-1]$.
We conclude  that
$\Card(\dom(S_{a_i})) <  \Card(\dom(S_{a_{j}}))$ for all $i,j \in [1,h]$, $i < j$,
by induction on $j$.
...
$c = \Card(\dom(S))$ the domain-cardinality of the sequence. Then either 
...
\Sigma_{x \in \dom(R)} \max(\size(\beta, R(x))$.
\end{enumerate}
\end{definition}
...
$\max(\size(\gamma, RS(x)) \le \max(\size(\beta, R(x))$, hence 
$
\size(\gamma, RS) =
\Sigma_{x \in \dom(RS)} \max(\size(\gamma, RS(x)) \le
\Sigma_{x \in \dom(RS)} \max(\size(\beta, RS(x)) =
\size(\beta, R)
$.
\end{proof}
...
If $\dom(S) =$ is any injective domain-stable sequence with constant-domain-cardinality
...
Assume $\vec{R}, R$ is any sequence of injective relations, with $c = \Card(\dom(R))$. 
...
or $k \in [1,h]$ and if 
$c_i = \Card(\dom(\vec{C_i}))$, 
$d_i = \Card(\dom(\vec{D_i}))$ for all $i \in [1,k]$ 
...
%\begin{enumerate}
%\item %1
%Suppose$ \vec{R}S$ is stable and $\Card(\dom(S))> c_h$. Then
%$c_{h+1} = \Card(\dom(S))$, $a_h = n$, $a_{h+1} = n+1$ and 
...
Assuem be given a set $\setR$ of colored relations on $[1,\MaxInd]$. We assume
...
The state of an algorithm is any sequence $(R_1, d_1, \ldots, R_n, d_n)$ such that:
\begin{enumerate}
\item
$R_1, \ldots, R_n \in \setR^+$

\item
for all $i \in [n-1]$, $\Card(\dom(R_i) \not = \dom(R_{i+1})$ 
...
is the graph of some bijection $\phi$. Therefore for
all $x \in \dom(R_1)$ we have $R(x) = R_1 \ldots R_n R_{n+1}(x) = R_{n+1}(\phi(x))$.

If $\phi:X \rightarrow Y$ is a bijection, for any sum $\Sigma_{x \in X} a_{\phi(x)}$, 
we have $\Sigma_{x \in X} a_{\phi(x)} = \Sigma_{y \in Y} a_y$. 

Thus, from $\dom(R) = \dom(R_1)$ and $R(x) = R_{n+1}(\phi(x))$ and $\phi$ deduce that 
$$
\size(R) =
\Sigma_{x \in \dom(R)} \max(\size(R(x)) = 
\Sigma_{x \in \dom(R_1)} \max(\size( R_{n+1}(\phi(x))) =
\Sigma_{y \in \dom(R_{n+1})} \max(\size( R_{n+1}(y)) =
\size(R_{n+1})
$$
...
the inductive formula number $x$ of $\alpha$ progresses to the inductive formula number $y$
 ...
 $f(\alpha,\vec{S})=(l_1, \ldots, l_h)$ elements. If $c_i = \Card(\dom(R_{a_i})$,
