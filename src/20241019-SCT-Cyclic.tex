...
\begin{theorem}[Finiteness of $\setR$-compatible transitions 1]
...
\subsection{A Proof of the Termination Theorem 
...
We also need a sufficient condition for a relation $R$ being
...
$R$ should be injective and $\dom(R)=\dom(RS)=\dom(S)$.

For any binary relation $R$ on $X$
...
We define the value $\val(Y,\sigma)$ of a subset $Y \subseteq X$ of $X$
w.r.t. a valuation 
$\sigma:X \rightarrow \N$ as the sum of all values of $\sigma$ on $X$ 
$$
...
\section{Translating Global Trace Condition into Nested Induction
(for Injective Relations and any Cardinality)}
...
for $j \in [i,l]$ is weakly decreasing. 
When all these sequences are 
constant we say that the list $L$ is domain-stable.
Here is the formal definition.

\begin{definition}
[Domain-Stability for a Sequence]
...
\item
$L$ is \emph{domain-stable} if and only if all $i \in [1,l]$ 
we have $\Card(\dom(R_i)) = \Card(\dom(R_{i,l}))$.

\item
$L$ has \emph{constant cardinality} if and only if all 
$i,j \in [1,l]$, $i \le j$ 
we have $\Card(\dom(R_{i,j})) = \Card(\dom(R_{1,l}))$.

\item 
$\pi$ is domain-stable (respectively, has constant cardinality)
if the associated relation sequence 
$\{\universe{\pi(i)} | i \in [1,l]\}$, obtained by taking the union
of each pair $\pi(i)$, is domain-stable 
(respectively, has constant cardinality).
\end{enumerate}

\end{definition}

If $L = \{R_i | i \in [1,l\}$ 
is domain-stable and $i \in [1,l]$, then the first set 
$\dom(R_{i,i})$ and the last set $\dom(R_{i,l})$ in the weakly 
decreasing sequence $\{\dom(R_{i,j}) | j \in [i,l]\}$ are equal. 
Thus, if $L$ is domain stable then all these sequences are constant.
The next Lemma describes in details the structure of a domain-stable
sequence $\pi$ on a set $\setR$ of relation pairs.

\begin{lemma}
[Domain-stable injective bi-coloring and the Progressing Domain condition]
\label{lemma-domain-stable-injective}
Assume that 
$\pi:[1,l] \rightarrow \Pfin(X \times X)^2$ is a \emph{domain-stable} finite list
of \emph{injective} pairs of relations on $X$, compatible with the assignment list 
...
\begin{enumerate}
\item
\label{lemma-domain-stable-injective-01}
...
\item
\label{lemma-domain-stable-injective-02}
...
\item
\label{lemma-domain-stable-injective-03}
...
\item
\label{lemma-domain-stable-injective-04}
Let $k \in [1,n]$
with $n = \Card(X)$.
Assume that the composition closure of $\setR = \{\pi(i) | i \in [1,l]\}$ 
satisfies the Progressing-domain condition.
...
\item
%\label{lemma-domain-stable-injective-01}
If $L$ is a domain-stable sequence of injective relations then by Lemma 
...
%$\pi$ is a list of injective relation pairs and $i \le j-1$, 
%therefore $R_{i,j-1}$ is injective.
%By stability of $\pi$ and $i \le j-1$ we have 
%$\dom(R_i) = \dom(R_{i,j-1})$ and  $\dom(R_{i,j-1}) =  \dom(R_{i,j})$.
%By definition we have $\dom(R_{i,j}) = \dom(R_{i,j-1}R_j)$,
%we deduce that $\dom(R_{i,j-1}) = \dom(R_{i,j-1}R_j)$.
%By Lemma 
%\ref{lemma-composition-injective-relations}.
%\ref{lemma-composition-injective-relations-03} 
%applied to $R_{i,j-1}, R_j$ and to $\dom(R_{i,j-1}) = \dom(R_i)$ we deduce that 
%$R_{i,j-1} \restr \dom(R_{i}), \dom(R_{j})$ is a bijection. 
...
\begin{proof}
[Proof of Termination Theorem 1, \ref{theorem-finiteness-value-flow-1}]

%We say that a bi-coloring sequence $\pi:[1,m] \rightarrow \setR$
%is domain-stable if the relation sequence $\universe{\pi}$ defined by
%$\universe{\pi}(i) = \universe{\pi(i)}$ for all $i \in [1,m]$ is domain-stable.

We first define a list of $n$ weights $w(\pi,\theta)$ for a 
length $m$ sequence $\pi$
with stable domains. We associate to each cardinality $k \in [1,m]$
a single weight.

Suppose there is some $i \in [1,m]$
such that $\Card(\dom(\universe{\pi(i)}))=k$.
Then we define $w(\pi,\theta)_k = (m_k,r_k)$ 
where $m_k$ is the least value for $\val(\pi(i),\theta(i))$
such that $\Card(\dom(\universe{\pi(i)}))=k$. We define 
$r_k = \genfrac(){0pt}{0}{n}{k} - s_k$, where $s_k \ge 1$ is the number of $i \in [1,m]$ such that
$\val(\pi(i),\theta(i)) = m_k$
and $\Card(\dom(\universe{\pi(i)}))=k$
\footnote{$r_k$ is a bound to the number of times the values
$(m_1,r_1, \ldots, m_k)$ can repeat equal if we extend $\pi$,
and $r_k=0$ means these values cannot repeat.
Indeed, we 
will prove that if we consecutively have $\Card(\dom(\universe{\pi(i)}))=k$ and
$ \val(\pi(i),\theta(i)) = m_k$, then the set $Y = \dom(\universe{\pi(i)})$
cannot repeat. 
$Y$ is any subset of $k$ elements of $[1,n]$, therefore the number of 
possible values for $Y$ starts from $\genfrac(){0pt}{0}{n}{k}$ and decreases by $1$
in each step, and it cannot go below zero.}

If there is no $i \in [1,m]$ such that $\Card(\dom(\universe{\pi(i)}))=k$
then we set $w(\pi,\theta)_k = (\infty,0)$.

We define $w(\pi,\theta)$ as the sequence
$(w(\pi,\theta)_1, \ldots, w(\pi,\theta)_n)$ of weights.

In each domain
stable segment by Lemma \ref{lemma-composition-injective-relations-02}
we have $\Card(\dom(R_{i,m})) = \Card(\dom(R_i R_{i+1,m})) \le
\Card(\dom(R_{i+1,m}))$. By stability we have $\dom(R_{i}) =
\dom(R_{i,m})$ and $\dom(R_{i+1}) =
\dom(R_{i+1,m})$, we deduce $\Card(\dom(R_{i})) 
\le \Card(\dom(R_{i+1}))$.

For all $k \in [1,n]$ and any domain stable $ \pi : [1,m] \rightarrow \setR $ 
we define $S(\pi,k) = \{i \in [1,m] | \Card(\dom(R_i)) = k \}$. 
We claim that each $S(\pi,k)$ is a possibly empty segment. Indeed, if $i, j \in 
...
We can decompose each domain-stable $q:[1,m] \rightarrow \setR$ into
a consecutive composition $q_1 \conc \ldots \conc q_n$, 
with each $q_k$ the possibly empty restriction of $q$ to $S(\pi,k)$.
By definition, each $q_k$ is a \emph{constant cardinality} path,
indeed for each $i, j \in S(\pi,k)$, $i \le j$ we have $\Card(\dom(R_{i,j}))=\Card(\dom(R_i))=k$. 

Assume $L=(c_1, \ldots, c_n)$ is a coloring list. We define a re-grouping
of $L$ as any coloring sequence 
$
M = 
(c_1\ldots c_{a_1},  
...
$ 
of composition of 2-colorings takne from $L$, 
 with the same ordering as in $L$,
 and whose product is the same as $L$: $\Pi M = \Pi L$.
 $M$ is obtained by partitioning $L$ into adjacent segments 
$[a_1,a_2-1], [a_2,a_3-1], \ldots, [a_{m-1},a_m - 1],[a_m,n]$,
with $a_1=1$, 
then composing the coloring of each segment $[a_{i},a_{i+1}-1]$
 into a single coloring.
 Say: we can group $L=(c_1,c_2,c_3,c_4,c_5,c_6)$ into
 $M=(c_1c_2,c_3,c4,c_5,c_6)$, and in many more ways.
...
with adjacence sequence $a \star (\length(L_2)+1)$ 
                           %%%$a \star (\length(L_1)+1)$.
...
We define a map $c(\pi,\theta) = (q,\eta)$
returning a compatible pair bi-coloring sequence/assignment sequence,
and for some increasing map $a:[1,m] \rightarrow [1,n]$ we have:
...
$\eta=\theta \comp a$, namely $\eta(t,x) = \theta(a(t),x)$ for all
 $t \in [1,m]$, $x \in X$. 
\item
$q$ has stable domain and  
\item
$(\pi,\theta) <_1 (\pi',\theta')$ implies $w(c(\pi,\theta)) >_\lex w(c(\pi',\theta'))$
in the lexicographic ordering.
\end{enumerate}

More in detail we will have $c(\pi,\theta) = (q,\eta)$, 
$c(\pi,\theta) = (q,\eta)$
$c(\pi',\theta') = (q',\eta')$ and $q= q_1 \conc \ldots \conc q_n$,
$q' = q'_1 \conc \ldots \conc q'_n$ are decomposition with each $q_k, q'_k$
 a sequence of 2-colored relations whose domains all have cardinality $k$,
then for some $k \in [1,n]$ we have $q_1 = q'_1$, \ldots, $q_{k-1} = q'_{k-1}$
and $q_k <_1 q'_k$ and $q' = q'_1 \ldots q'_k$. This explains why we have
$w(c(\pi,\theta)) >_\lex w(c(\pi',\theta'))$.

%16:42 21/02/2025
Suppose $\pi' = \pi \star (\Stat,\Prog)$ and $\theta' = \theta \star \sigma$
and $c(\pi,\theta) = (q,\eta)$ with $\length(q) = \length(\eta) = m$ in order to
define $c(\pi',\theta') = (q',\eta')$, sequences of length $m'$. 
...
sequences of length $m' = m+1$. 

\item
Assume that $q \star (\Stat,\Prog)$ is \emph{not} stable. 
Then we take the first $m' \in [1,m]$ such that 
$
\dom(q(m') \comp \ldots \comp q(m))
\subset 
\dom(q(m') \comp \ldots \comp q(m) \comp (\Stat,\Prog))
$.
We define $q'(i) =  q(i)$ for all $i \in [1,m'-1]$ and
$q'(m') = q(m') \comp \ldots \comp q(m) \comp (\Stat,\Prog)$.
We define $\eta'(i) = \eta(i)$ for all $i \in [1,m']$. 
\end{enumerate}
This is $c(\pi',\theta')$. 

%In the definition of $c(\pi',\theta')$ we remove no $q(j)$ such that
%$\Card(\dom(\universe{q(i)})) < k$, because we cannot have 
%$\Card(\dom(\universe{q(i)}) = k$ and $q(i) \comp \ldots \comp q(j-1)$ injective
%and $\Card(\dom(\universe{q(j)})) < k$. Indeed, we proved that
In any domain-stable sequence of injective relations the cardinality sequence
$\Card(\dom(\universe{q(i)}))$ is equal to 
$\Card(\dom(\universe{q(i)}\ldots\universe{q(m)}))$
and therefore it is weakly increasing in the index $i \in [1,m]$.

%22:25 19/02/2025

We prove that $w(c(\pi,\theta))>w(c(\pi',\theta'))$ by cases on the
definition of $c(\pi',\theta')$.

\begin{enumerate}

\item
Assume that $q \star (\Stat,\Prog)$ is stable. 
Then $w(c(\pi,\theta))_l = w(c(\pi',\theta'))_l$ for all $l<m'$.
If $\dom(\pi'(m')) = \dom(\pi'(m))$ 
we use Lemma \ref{lemma-domain-stable-injective}.
\ref{lemma-domain-stable-injective-03}
to prove that $w(c(\pi,\theta))_{m'} > w(c(\pi',\theta'))_{m'}$.
If $\dom(\pi'(m')) \not = \dom(\pi'(m))$ 
we use Lemma \ref{lemma-domain-stable-injective}.
\ref{lemma-domain-stable-injective-04}
to prove that $w(c(\pi,\theta))_{m'} > w(c(\pi',\theta'))_{m'}$.

\item
Assume that $q \star (\Stat,\Prog)$ is \emph{not} stable. 
Then $w(c(\pi,\theta))_l = w(c(\pi',\theta'))_l$ for all $l<m'$
and $w(c(\pi,\theta))_{m'} > w(c(\pi',\theta'))_{m'}$ because
the domain of $q'(m')$ is smaller than the domain of $q(m')$.
In the case 
$\Card(\dom(q'(m')) > \Card(\dom(q(1))), \ldots, \Card(\dom(q(m'-1)))$
we have $w(c(\pi,\theta))_{m'} = \infty > w(c(\pi',\theta'))_{m'}$.
In the case $\Card(\dom(q'(m')) = \Card(\dom(q(m'-1)))$, by arguing
as in the previous case we prove that $q'$ has smaller indexes than 
$(q(1), \ldots, q(m'-1))$, therefore smaller indexes than $q$.
...
%
%We define an increasing map
%$f:(\setT,>_1) \rightarrow ([0,\infty]^n, <_\lex)$ as follows. 
%%14:57 20/02/2025
%
%Assume we have a transition 
%$\theta = \langle (r_1,\sigma_1), \ldots, (r_m,\sigma_m) \rangle$
%on $\setR$. For all $s \in [1,m]$
%we have $r_s = (\Stat_{i_s},\Prog_{i_s} \in \setR$ and 
%we define $R_s = \universe{r_s} =\Stat_{i_s} \cup \Prog_{i_s}$. 
%By our assumption on $\setR$, the relation $R_s$ is injective.
%
%For any $p,q \in [1,m]$, $p \le q$ let 
%us abbreviate the composition $r_p \ldots r_q$ of relation pairs 
%with $r(p,q) = \Pi_{s=p}^{s=q} r_s$ and $R(p,q) = \universe{r(p,q)} =
%\universe{r_p} \ldots \universe{r_q} = R_p  \ldots R_q$.
%In particular we have $r(p) = r_p$ and $R(p,p)=R_p$.
%
%All relation pairs $r(p,q)$ are in $\setR$ by 
%\emph{composition-closure of $\setR$}. By the assumption PDC on $\setR$
%all $r(p,q)$ are progressing.
%%%%%%%%%%%
%% THE NEXT REMARK IS TRUE BUT POSSIBLY IT IS USELESS
%%%%%%%%%%%
%%Therefore all relations $R(p,q)$ on $X$ are finite non-empty compositions of 
%%injective relations, hence all relations $R(p,q)$ are \emph{injective relations}.
%%\\
%%%%%%%%%%%
%An alternative definition of $r(p,q) \in \setR$ runs as follows. We form all possible
%compositions of relations by selecting one relation
%in each pair $r_s = (\Stat_{i_s}, \Prog_{i_s})$, for all $s \in [p,q]$.
%Then $r(p,q) = (\Stat', \Prog')$, with $\Stat' = \Stat_{i_p} \ldots \Stat_{i_q}$
%and $\Prog' = $ the union of all compositions including at least one
%$\Prog_{i_s}$ for some $s \in [p,q]$. 
%\\
% 
%We define the sequence $c_\theta(i) =  \Card(\dom(R(i,m)))$ of the cardinalities
%of the domains of $R(i,m)$, for all $i \in [1,m]$.
%For all $i \in [1,m-1]$ we have $R(i,m)) = R_i R(i+1,m)$ and $R_i$ injective.
%For all $i \in [1,m-1]$, by Lemma 
%\ref{lemma-composition-injective-relations}.
%\ref{lemma-composition-injective-relations-02},
%{lemma-composition-injective-relations-03} 
%we deduce that:
%\begin{enumerate}
%\item
%$c_\theta(i) = \Card(\dom(R(i,m))) \le \Card(\dom(R(i+1,m))) = c_\theta(i+1)$ (i.e.,
%$c$ is weakly increasing)
%\item
%if $c_\theta(i)=c_\theta(i+1)$ then $R(i,m) \cap (\dom(R(i,m)) \times \dom(R(i+1,m)))$
%is a bijection.
%\end{enumerate}
%
%We define $S_\theta(j) = \{i \in [1,m] | c_\theta(i) = j \}$. 
%
%The set $S_\theta(j)$ is a segment, possibly empty.
%Indeed, if $n,m \in S_\theta(j)$ and $n<p<m$ then $j = c_\theta(n) \le c_\theta(p) \le c_\theta(m) = j$.
%We deduce that $c_\theta(p)=j$ and we conclude that $p \in S_\theta(j)$.
%
%If $i, i+1 \in S_\theta(j)$ we have $c_\theta(i) = c_\theta(i+1) = j$ and therefore
%$R(i,m) \cap (\dom(R(i,m)) \times \dom(R(i+1,m)))$ 
%is a bijection by Lemma 
%\ref{lemma-composition-injective-relations}.
%{lemma-composition-injective-relations-03}.
%
%
%Let us abbreviate $v_{\theta, i} = \val(R(i,m),\sigma_i)$ for all $i \in [1,m]$.
%
%Let us assume that $i, i+1 \in S_\theta(j)$.
%We claim that we have $v_{\theta,i}  > v_{\theta,i+1}$. 
%The reason is that $R_i$ is a bijection restricted to $\dom(R(i,m))
%\times \dom(R(i+1,m))$ by Lemma 
%\label{lemma-composition-injective-relations}.
%\label{lemma-composition-injective-relations-03}.
%Thus, we have 
%$v_{\theta,i} = \val(R(i,m),\sigma_i) > \val(R(i+1,m),\sigma_i) = v_{\theta,i+1}$ 
%by Lemma 
%\ref{lemma-decreasing-progressing-bijection}
%and the fact that $R(i+1,m)$ is weakly progressing and strictly progressing.
%This latter is a consequence of PDC for $\setR$ and composition-closure for $\setR$.
%\\
%
%Suppose that $S_\theta(j) \not = \emptyset$.
%From $v_{\theta,i}  > v_{\theta,i+1}$ for all $i, i+1 \in S_\theta(j)$ and 
%the fact that $S_\theta(j)$ is a segment 
%that  deduce that the minimum of $\{v_{\theta,i} | i \in S_\theta(j)\}$ 
%is $v_M$, for $M = \max(S_\theta(j))$ the last element
%of the segment $S_\theta(j)$. 
%In this case, we define $v_\theta(j) = \min(\{v_{\theta,i} | i \in S_\theta(j)\})$.
%
%Instead, in the case $S_\theta(j)=\emptyset$ we set $v_\theta(j) = \infty$,
%which is the minimum of the $\emptyset$ in the order $[0,\infty]$.
%
%We map any
%$\theta = \langle (R_1, \sigma_1),\ldots, (R_m,\sigma_m) \rangle \in \setT$ 
%into 
%$$
%f(\theta) 
%\ \ \ = \ \ \ 
%( v_\theta(1), \ldots, v_\theta(n) ) \in [0,\infty]^n
%$$
%Whenever we extend the sequence to 
%$\theta$ to $\eta = \theta \conc ((R_{m+1},\sigma_{m+1})) \in \setT$
%we have $\dom(R(i,m)) \supseteq \dom(R(i,m)R_{m+1}) =
%\dom(R(i,m+1))$. 
%\\
%
%We first remark that in the case we have 
%$\dom(R(i,m)) = \dom(R(i,m+1))$ for all $i \in [1,m]$,
%then we have $c_\eta(i) = c_\theta(i)$ for all $i \in [1,m]$.
%Let us abbreviate $k=c_\theta(m)$ the maximum value of $c_\theta$.
%Then we have $c_\eta(m+1) \ge c_\eta(m) =  c_\theta(i) = k$.
%
%We now consider all possible cases.
%
%\begin{enumerate}
%\item
%Assume $\dom(R(i,m)) = \dom(R(i,m+1))$ for all $i \in [1,m]$ and 
%$c_\eta(m+1) > k$. 
%
%In this case for all $h \in [1,k]$ we have $S_\eta(h) = S_\theta(h)$ 
%and we deduce that $v_\eta(h) = v_\theta(h)$.
%We have $S_\theta(k+1) = \emptyset$ while $S_\eta(k+1) = [m+1,m+1]$.
%In this case $f(\theta) = (v_\theta(1), \ldots, v_\theta(k),\infty, \ldots, \infty)$
%and $f(\eta) = (v_\eta(1), \ldots, v_\eta(k), v_\eta(k+1),\infty, \ldots, \infty)$.
%$f(\eta)$ is smaller than $f(\theta)$ in the lexicographic order
%because $v_\eta(k+1) < \infty$.
%
%\item
%Assume $\dom(R(i,m)) = \dom(R(i,m+1))$ 
%for all $i \in [1,m]$ and $c_\eta(m+1) = k$.
%
%In this case $S_\eta(k) = S_\theta(k)  \cup \{m+1\}$. We have 
%$v_\eta(k) = v_{m+1} < v_m = v_\theta(k)$.
%Thus, $f(\eta)$ is smaller than $f(\theta)$ in the lexicographic order.
%
%\item
%Assume $\dom(R(h,m)) \supset \dom(R(h,m+1))$ for some $h \in [1,m]$.
%Take the first such $h$. Then $\dom(R(i,m) = \dom(R(i,m+1))$
%for all $i<h$ and $\dom(R(h,m)) \supset \dom(R(h,m+1))$.
%We have $c_\theta(i) = c_\eta(i)$ for all $i<h$ and 
%$c_\theta(h) > c_\eta(h)$. Let $j = c_\eta(h)$. 
%In order to prove that $f(\eta)$ is smaller than $f(\theta)$ in the lexicographic 
%order we will prove
%that $v_\eta(j') = v_\theta(j')$ for all $j'<j$ and $v_\eta(j) < v_\theta(j)$.
%
%If $c_\theta(i) = j' < j$ then $i \le h-1$ and if $c_\eta(i)= j'  < j$ then 
%again $i \le h-1$.
%
%Thus, for all $j'<j$ we have $S_\eta(j'), S_\theta(j') \subseteq [1,h-1]$,
%hence $S_\eta(j') = S_\theta(j')$.
%From $\dom(R(i,m) = \dom(R(i,m+1))$
%for all $i<h$ we obtain $v_\eta(j') = v_\theta(j')$ for all $j'<j$.
%
%Now we prove that $v_\eta(j) < v_\theta(j)$. We distinguish two sub-cases.
%
%\begin{enumerate}
%\item
%Suppose there is some 
%$i<h$ such that $c_\theta(i) = j$. We take the last $i \in [1,m]$
%such that $c_\theta(i) = j$. We still have $i<h$ because if $i \ge h$
%then $c_\theta(i) \ge c_\theta(h) > c_\eta(h) = j$. 
%By the choice of $i$ we have $c_\eta(i) = c_\theta(i) = j$,
%therefore $i \in S_\eta(j)$ and by $i<h$ we conclude that:
%$v_\eta(j) = v_{\eta,h} < v_{\eta,i} = v_{\theta,i} = v_\theta(j)$. 
%
%\item
%Suppose there is no $i<h$ such that $c_\theta(i) = j$. 
%Then $S_\theta(j) = \emptyset$ while $h \in S_\eta(j)$ and therefore
%$v_\eta(j) \in \N$. We conclude that $v_\eta(j) < \infty = v_\theta(j)$. 
%\end{enumerate}
%\end{enumerate}
%
%
%
%Thus, $f$ is increasing from $(\Seq{\setR}, >_1)$ to $([0,\infty]^n, <_{\lex})$. 
%This ends the proof of the Termination
%Theorem \ref{theorem-finiteness-value-flow}.
%
%%10:10 04/02/2024
