...
\item 
the paths on $\setR^+$ 
with \emph{constant-cardinality domains} have a decreasing embedding in 
$[0,\infty] \times [0,\genfrac(){0pt}{0}{n}{k}[$ and therefore terminate;
\item
the paths on $\setR^+$ 
with \emph{stable-cardinality domains} have a decreasing embedding 
in $\setL$ and therefore terminate;
\item
\emph{all} paths on $\setR^+$ 
have a decreasing embedding $WG$ in the set of paths with stable-cardinality 
domains  and therefore terminate;
\end{enumerate}
...
%13:44 03/03/2025
Lemma 
\ref{lemma-stable-cardinality domain-injective}.
...
has at most $\genfrac(){0pt}{0}{n}{k}$ elements, that is,
length in $[0,  \genfrac(){0pt}{0}{n}{k}[$. This is the first step in the proof
of Main Theorem.
\\

For the second step in the proof of the Main Theorem, 
...
By the previous step in the proof we have $s \le \genfrac(){0pt}{0}{n}{k}$,
...
For the third step in the proof of the Main Theorem, 
...
we have  $w(\pi_k,\theta_k) > w(\pi'_k,\theta'_k)$ by the third case
of the Theorem. We conclude $W(\pi,\theta) > W(\pi',\theta')$.
...
%
%In each path with stable-cardinality domains 
%by Lemma 
%\ref{lemma-composition-injective-relations}.
%\ref{lemma-composition-injective-relations-02}
%we have $\Card(\dom(R_{i,m})) = \Card(\dom(R_i R_{i+1,m})) \le
...
For the forth and last step in the proof of the Main Theorem, 
we assume that $\pi:[1,l] \rightarrow \setR$ is \emph{any}
path on $\setR^+ = \setR$ and we define a decreasing map $WG$ from 
$(\pi,\theta) \in \setS$ to $WG(\pi,\theta) = (\pi',\theta') \in \setS$, 
with $\pi'$ paths with stable-cardinality domains. We will conclude that
all value flows terminate. 
...
We define a map $WG(\pi,\theta) = (q,\eta)$
...
and $q_k <_1 q'_k$ and $q' = q'_1 \ldots q'_k$. By the results of the third
step of the proof, this implies that we have 
$W(G(\pi,\theta)) >_\lex W(G(\pi',\theta'))$.

%%In the definition of $G(\pi',\theta')$ we remove no $q(j)$ such that
...
%%22:25 19/02/2025
%
%We prove that $w(G(\pi,\theta))>w(G(\pi',\theta'))$ by cases on the
%definition of $G(\pi',\theta')$.
%
%\begin{enumerate}
%
%\item
%Assume that $q \star (\Stat,\Prog)$ is stable. 
%Then $w(G(\pi,\theta))_l = w(G(\pi',\theta'))_l$ for all $l<m'$.
...
%to prove that $w(G(\pi,\theta))_{m'} > w(G(\pi',\theta'))_{m'}$.
%If $\dom(\pi'(m')) \not = \dom(\pi'(m))$ 
%we use Lemma \ref{lemma-stable-cardinality domain-injective}.
