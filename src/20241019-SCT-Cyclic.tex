...
cannot have length $\ge g\genfrac(){0pt}{1}{n}{k}$, where $k$ is 
the common cardinality of the domains.

Besides, we can prove that these value flows terminate by induction on the distance
between the length of the flow and $g\genfrac(){0pt}{1}{n}{k}$.
...
$\pi(i) \ldots \pi(j)$ is self-composable
and $\setR, \setG$ 
...
$< g\genfrac(){0pt}{1}{n}{k}$, for all $v \in \N$.
...
Assume $\dom(R_i) = \dom(R_j)$
 and $\pi(i) \ldots \pi(j)$ is self-composable in $\setG$. 
...
There are $g\genfrac(){0pt}{1}{n}{k}$ 
different cardinality $k$ subsets of an $n$ element
set $X$ with , therefore at most $\genfrac(){0pt}{1}{n}{k}$ indexes for 
$i\in [1,l]$ such that $\Card(\dom(R_i))=k$
and $\val(\dom(R_i),\theta(i))=v$ and 
$\pi(i) \ldots \pi(j)$ is self-composable in $\setG$. 
...
each constant-cardinality non-progressing domain path on $\setR^\setG$
has $<g\genfrac(){0pt}{1}{n}{k}$ elements, that is, has
length in $[0,  g\genfrac(){0pt}{1}{n}{k}[$. Then we can prove
