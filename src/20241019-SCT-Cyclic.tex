...
We need one last ingredients before proving the Main Theorem.
...
We will prove that termination of all value flows in four steps: 
\begin{enumerate}
\item
the paths on $\setR^+$ 
with constant-cardinality non-progressing domains 
terminate in at most $\genfrac(){0pt}{0}{n}{k}$ steps, where $k$ is 
the common cardinality of the domains; 
\item 
the paths on $\setR^+$ 
with constant-cardinality domains have an increasing embedding in 
$[0,\infty] \times [0,\genfrac(){0pt}{0}{n}{k}[$ and therefore terminate;
\item
the paths on $\setR^+$ 
with stable-cardinality domains have an increasing embedding 
in $\setL$ and therefore terminate;
\item
\emph{all} paths on $\setR^+$ 
with stable-cardinality domains have an increasing embedding 
in $\setL$ and therefore terminate;
\end{enumerate}
...
for all $i,j \in [1,l]$, $i \le j$.
The next Lemma describes in details more properties of a stable-cardinality 
domain sequence $\pi$ on a set $\setR$ of relation pairs.
...
Assume that 
$\pi:[1,l] \rightarrow \Pfin(X \times X)^2$ is a \emph{stable-cardinality domain} finite list
...
We have now all ingredients for proving the Main Theorem.
...
We can decompose each stable-cardinality domain $q:[1,m] \rightarrow \setR$ into
a consecutive composition $q_1 \conc \ldots \conc q_n$, 
with each $q_k$ the possibly empty restriction of $q$ to $S(\pi,k)$.
By definition, each $q_k$ is a \emph{constant cardinality domains} path,
...
In any stable-cardinality domain sequence of injective relations the cardinality sequence
...
we use Lemma \ref{lemma-stable-cardinality domain-injective}.
\ref{lemma-stable-cardinality domain-injective-03}
to prove that $w(c(\pi,\theta))_{m'} > w(c(\pi',\theta'))_{m'}$.
If $\dom(\pi'(m')) \not = \dom(\pi'(m))$ 
we use Lemma \ref{lemma-stable-cardinality domain-injective}.
\ref{lemma-stable-cardinality domain-injective-04}
to prove that $w(c(\pi,\theta))_{m'} > w(c(\pi',\theta'))_{m'}$.
