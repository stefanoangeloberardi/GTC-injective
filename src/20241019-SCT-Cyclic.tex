...
We are particulary interested to the case of transition graphs labeled by 
\emph{injective} bi-colored relations.

Our motivation is that this particular case is enough to check 
...
However, the algorithm checking cyclic proofs is a particular case of the
algorithm checking SCT.
...
Another motivation we have in mind is defining an automatic translation from 
...
\item
a finite non-empty set $G$ and a map $\setG: \dom(\setG) \rightarrow \setR$,
such that $\dom(\setG) \subseteq G \times G$. 
\end{enumerate}

Implicitly, $\setG$ defines a graph with set of nodes $G$, set of
edges $\dom(\setG) \subseteq G \times G$ and with edges labeled on $\setR$. 
...
We provide an interpretation of weak progression and progression later.
...
$$
Remark that the result of composition is a disjoint pair of finite relations on $X$.
We interpret composition of pairs later. 

Sometimes we skip the index $\setG$
and we write just $\composable{}{a}{b}{(\Stat,\Prog)}$.
We use $\setG$ to restrict the set of compositions which are allowed.
...
\item
We say that that $(\Stat,\Prog), (\Stat',\Prog')$ are
...
\end{definition}
 ...
 We consider the elements of $X$ as variables to which we associate a value in 
...
any sequence $b$ on $G$ such that $(b(i),b(i+1)) \in 
\dom(\setG)$ for any two consecutive elements of the path.
Implicitly, each path is a path in the graph associated to $G$ and labeled
on $\setR$ by $\setG$.
Thus, each path $b$ is associated by $\setG$ to a sequence $r$ on $\setR$
defined by $r(i) = (\Stat_i, \Prog_i) = \setG(b(i),b(i+1))$. If $b$
is finite, then the pair sequence $r$ is one unit shorter than $b$. If
...
\item
We call any map $\pi:D \rightarrow X$ a \emph{trace} 
(called a \emph{thread} in \cite{SCT-2001}): 
...
\item
We say that the trace $\pi:D \rightarrow X$ is \emph{compatible} with the path
...
then the two consecutive values  of the trace are related by $\setR$, namely
 $\pi(t) (\Stat \cup \Prog) \pi(t+1)$.
If furthermore we have $\pi(t) \Prog \pi(t+1)$,
...
(\ref{definition-GTC-inj}). This equivalence requires to
...
Our result can be used for checking termination of recursive calls, with a 
...
\quotationMarks{
\emph{all self-composalbe relation pairs $(\Stat,\Prog)$
...
To put otherwise, the ordering on $((m_1,r_1), \ldots, 
(m_n,r_n)) \in \setL$ is the lexicographic ordering on 
$(m_1,r_1, \ldots, m_n,r_n)$.
...
for $j \in [i,l]$ is weakly decreasing. The same holds for the sequence 
$\{\Card(\dom(R_{i,j})) | j \in [i,l]\}$ on $\N$.
...
proved the equivalence of SCT and GTC 
$$
\forall \setR, \setG \mbox{ comp.-closed}.  \ \ 
