...
Implicitly, $\dom(\setG) \subseteq G \times G$ is a graph with nodes in $G$
and with edges labeled on $\setR$. 
We write $\setG(a,b) = (\Stat,\Prog)$, or quotationMarks{the edge $(a,b)$ is
labeled by $(\Stat,\Prog)$} in the form: 
$\composable{\setG}{a}{b}{(\Stat,\Prog)}$. We often skip the index $\setG$
...
sequence $(\Stat_1,\Prog_1), \ldots, (\Stat_m,\Prog_m)$ on $\setR$
is \emph{composable in $\setG$}, just \emph{composable}
for short, if and only if for some $a_1, \ldots, a_m \in G$ 
we have $\composable {} {a_i \ } { \ a_{i+1}} {(\Stat_i,\Prog_i)}$
for all $i \in [1,m-1]$.

\item
We say that $(\Stat,\Prog) \in \setR$
is \emph{self-composable in $\setG$}, just \emph{self-composable}
for short, if and only if for some $a \in G$ we have 
$\composable {} {a} {a} {(\Stat,\Prog)}$.
