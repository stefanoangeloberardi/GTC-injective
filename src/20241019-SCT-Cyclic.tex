...
We suspect that SCT is PSPACE-complete 
also in the particular case of injective bi-colored relations required by cyclic 
proofs. However we will argue that checking SCT is simpler  
and faster in this case.

As a future work, we plan to define an automatic translation from 
...
using cyclic proofs. We do not have such a translation yet, but we believe that from
a simpler termination proof for SCT we will be able to define a simpler
translation from cyclic proofs to inductive proofs.

As a first step, we prove termination for SCT in the case of injective relations only using 
first order induction. With respect to a previous work,
we avoid the use Ramsey Theorem, and we use Intuitionistic Logic only.
