...
the composition of relation pairs.
...
\quotationMarks{
\emph{all relation pairs in $\setR^+$ 
...
If $\setR$ is a finite list of \emph{injective} relation pairs satisfying 
...
is any finite set of relation pairs on $X$, closed by composition, 
i.e., $\setR = \setR^+$.  
...
$[0,\infty] \times [0,\genfrac(){0pt}{0}{n}{k}[$. 
Therefore these value flows terminate.
...
There is a decreasing embedding from \emph{all} value flows on $\setR^+$ 
...
\item
$r = \genfrac(){0pt}{0}{n}{k} - s$, where $s \ge 1$ is the number of $i \in [1,l]$ 
such that $\val(\pi(i),\theta(i)) = m$.
\end{enumerate}
By point \ref{theorem-finiteness-value-flow-2-1} 
above we have $s \le \genfrac(){0pt}{0}{n}{k}$,
