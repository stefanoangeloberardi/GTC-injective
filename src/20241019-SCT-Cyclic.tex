...
in the path $a,a,a,a,\ldots$ labeled $R,R,R,R,...$ 
where $R = \universe{(\Stat,\Prog)}$. 
...
SCT $\Rightarrow$ PDC and GTC $\Rightarrow$ PDC. We proved one direction, namely
point \label{property-2} of the Claim.
\\
...
if $\setR$ is injective then PDC $\Rightarrow$ SCT and PDC $\Rightarrow$ GTC. 
...
If $R$ is transitive and injective, then any two elements of $\dom(R)$ related by $R$ are equal.
\end{proposition}
...
We now prove that all finite relations have some positive power which is idempotent
...
$R^{db} = R^{c+db}$. We take $n=db$.\footnote{There is an alternative proof using
Finite Ramsey Theorem. We take the complete graph on $\N$, and we color the 
edge between $i$ and any $j>i$ with $R^{j-i}$. By Ramsey Theorem there is 
a monocromatic set with $3$ elements $i<j<k$. If the color of the set is $R^m$,
then the edge between $i$ and $k$ is both $R^m$ and $R^mR^m$, therefore
$R^m$ is idempotent. }
\end{proof}

We can now prove that  PDC $\Rightarrow$ SCT.

\begin{theorem}
Assume $X$ is any finite set and $\setR \subseteq X \times X$ is any composition-closed
finite family of finite bi-colored 
\emph{injective} binary relations. If $\setR$ satisfies PDC and is then $\setR$ satisfies SCT.
\end{theorem}


\begin{proof}
Assume that $(S,P) \in \setR$ is some bi-colored finite relation on $X$,
in order to prove that for some $n \in \N$, $n>0$ there is some
$x \in X$ such that if $(S,P)^n = (S_n,P_n)$ the $x P_n x$. 
...
hence $x R^n y$. By proposition \ref{proposition-transitive-injective} we obtain that $x = y$.
We conclude that $x P_n x$, as wished.
\end{proof}


\section{An alternative proof of (PDC implies SCT)}
We first characterize the \emph{injective self-composable idempotent relations $R$} 
and we prove that all such relations are are the identity relation when restricted to $\dom(R)$.
In Prop. \ref{proposition-transitive-injective} we already proved that if $R$ is injective and
transitive, namely if $RR \subseteq R$, then $R \restr \dom(R) $ is \emph{included}
in the identity relation on $\dom(R)$.
...
If $R$ is injective, then $R^m$ is also the identity on $\dom(R^m)$, as the next Lemma says.
