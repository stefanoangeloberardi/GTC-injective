...
To this aim, we first characterize the 
\emph{injective self-composable idempotent relations $R$}:
they are are the identity when restricted to $\dom(R)$.

$R$ is a finite relation on a finite set $X$, therefore there are finitely many
values for $R^m$. We take the complete graph on $\N$, and we color the edge between $i$ and $j>i$ with $R^{j-i}$. By Ramsey Theorem there is 
a monocromatic set with $3$ elements $i<j<k$. If the color of the set if $R^m$,
then the edge between $i$ and $k$ is both $R^m$ and $R^mR^m$, therefore
$R$ is idempotent. If we $R$ is injective, then $R^m$ is 
also the identity on $\dom(R^m)$, as the next Lemma says.
