...
If  $i, j \in[1,n]$ and $i < j$ and $\DomCard(R_i) = \DomCard(R_j)$ then $R_i \ldots R_{j-1}$ has 
domain $\dom(R_i)$ and when restricted to $\dom(R_i) \times \dom(R_j)$ is a bijection.
\end{enumerate}
\end{lemma}
...
\begin{lemma}[Possibly Stationary]
...
If $RS$ is possibly stationary and $R,S$ is stable, then $R$ is possibly stationary
from $\dom(R)$ to $\dom(S)$.

\item
\label{lemma-possibly-stationary-04}
Assume that $R$ is a bijection from $\dom(R)$ to $\dom(S)$ and $R,S$ is stable.
...
Assume $\vec{S}$ has length $n \ge 1$. 
We provide a bound for the memory required to compute $\deg(\vec{S})$.
When $n=1$ then $\deg(\vec{S}) \in [0,0]$, so $\deg(\vec{S})=0$.
In order to compute $\deg(\vec{S},S)$ for $\vec{S},S$ of length $n+1 \ge 2$ we only we have
to know $\deg(\vec{S})$ and the last two elements of $\vec{S},S$. We \emph{do not} have
to know the entire sequence $\vec{S},S$, as the next Lemma says.

\begin{lemma}[Characterization of Stationarity Degree]
\label{lemma-degree}
Assume $\vec{S},S$ is some constant domain-cardinality sequence
of length $n \ge 2$. Let $d = \deg(\vec{S})$ and $d' = \deg(\vec{S},S)$.
...
$S_n$ is necessarily progressing from $\dom(S_n)$ to $\dom(S)$. By definition of degree we 
conclude that $d'=1$.

\item
Assume $S$ is possibly stationary and $d \ge 1$ and $S_{n}$ is necessarily progressing
from $\dom(S_{n})$ to  $\dom(S)$. Then $d'=1$ by definition of degree.

\item
Assume $S_n$ is possibly stationary and $d \ge 1$ and $S_n$ is possibly stationary
from $\dom(S_{n})$ to  $\dom(S)$. By definition of degree, the stationary
suffix of $\vec{S}$ is $S_{n-d+1}, \ldots, S_n$ cannot be extended while preserving $d \le n-1$.
This implies that either $d=n-1$ or $S_{n-d}$ is necessarily progressing
...
$$
is progressing for constant domain-cardinality Sequences.

\begin{lemma}[A Progressing Index for Constant Domain-Cardinality Sequences] 
...
has stationary degree $d$, then in the worst case \emph{all} infinite trace $\sigma$
...
Assume that $T$ has no necessarily progressing $x \in \dom(T)$,
...
all constant domain-cardinality extensions $R_1, \ldots, R_{d},T$
the relation $R_{d}$ is progressing between $\dom(R_{d})$ and $\dom(T)$.
...
For sake of simplicity we consider the case in which the unique inductive predicate is $N$. 
...
and $\pi_i = \{(S_j:\alpha_i \rightarrow \alpha_{j+1}) | j \in [a_i,b_i]\}$ 
...
is a state of $\vec{S} = S_1, \ldots, S_n$ and we write $\sigma \state \vec{S}$ if and only if 
...
We call 
$$
\States(\pi) =  \{\sigma \in \Seq(\ColRel \times \N \times \ColRel) | (\sigma \state\pi)\}
$$
the set of states. 

\item
Assume $\pi_1, \pi_2 \in \ProofUnfolding$ and $\sigma_1 \state \pi_1$ and 
$\sigma_2 \state \pi_2$. We write $(\pi_1,\sigma_1) \preceq (\pi_2,\sigma_2)$ for: 
$\size(\pi_1,\sigma_1) \le \size(\pi_2,\sigma_2)$ in the lexicographic order.
\end{enumerate}
\end{definition}

Each path $\pi$ has some state $\sigma$: we can always choose some codomains 
$\beta_1, \ldots, \beta_h$ and we always choose the trivial stationarity degree $d_i = 0$. 
We can decide $(\pi_1,\sigma_1) \preceq (\pi_2,\sigma_2)$ in polynomial time.
...
$\setR^+$ and $\sigma \in \States(\setR^+)$. We define a state 
$\tau = \trans(\sigma, R : \beta \rightarrow \gamma) \in \States(\setR^+)$ 
such that if $\sigma \state \pi$ then $\tau \state \pi_1$. We also check that 
$(\pi,\sigma) \succ (\pi_1, \tau)$ in the lexicographic ordering.
