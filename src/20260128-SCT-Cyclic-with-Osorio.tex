...
$$
\DomCard(R_i \ldots R_n) \le \DomCard(\dom(R_j \ldots R_n)
$$
...
because if $x \in \dom(RS)$ then for some $y \dom(S), z$ we have $x R y S z$.
...
\item
%\label{lemma-injective-stable-sequences-02}
Assume $i,j \in [1,n]$, $i < j$. By stability and Lemma \ref{lemma-injective-sequences-03}
...
for all product of sub-segments of $\vec{S}$.
...
$\dom(S_{i+1})$.  We can picture $\vec{S}$ as a composition of 
bijection from $\dom(S_i)$ to $\dom(S_{i+1})$, plus some traces missing $\dom(S_{i+1})$
and dying out.
