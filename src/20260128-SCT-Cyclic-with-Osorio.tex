...
We define the notion of grouping of a sequence of relations. An example: a possible 
grouping of $\vec{S} = S_1, S_2, S_3, S_4, S_5, S_6$ is the following list of $3$ products:
...
Then using the map $\phi(\vec{S},\cdot)$ we will provide an alternative definition of 
$\minStable(\vec{S})$ by induction on $\vec{S}$, 
together with an algorithm computing it in polynomial time. 
We first need the notion of $j$ dropping index for $i$, for $i,j \in [1,n]$.

\begin{definition}[Stable Matching]
Let $\vec{S} = S_1, \ldots, S_n$ and $i,j \in [1,n]$, $i < j$.
We say that $j$ is a dropping index for $i$ in $\vec{S}$
if and only if $\dom(S_i \ldots S_{j-1}) \supset \dom(S_i \ldots S_j)$.  
\end{definition}

By definition unfolding, $j$ is $j$ is a dropping index for $i$ in $\vec{S}$
if and only if there is some $x \in [1,\MaxInd]$ having some $S_i \ldots S_{j-1}$-image 
but no $S_i \ldots S_j$-image. 

If $j$ is the last dropping index for $i$ then 
$\dom(S_i \ldots S_j) = \dom(S_i \ldots S_{j+1}) = \ldots = \dom(S_i \ldots S_n)$.
If there are no dropping index for $i$ then 
$\dom(S_i ) = \dom(S_i S_{i+1}) = \ldots = \dom(S_i \ldots S_n)$.
...
of $i$ in $\vec{S}$, with $\phi(i) =  i$ if there is none. We write just $\phi(i)$ when $\vec{S}$ is clear from the context.
\item
...
all dropping index $b \in [1,n]$ of $a$ we have $b \in [1,i[$.
