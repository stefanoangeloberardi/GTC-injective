...
\item
We say that a sequence $\sigma \in \Seq(\ColRel \times \N \times \ColRel)$ is a state of 
$\vec{S} = S_1, \ldots, S_n$ and we write $\sigma \state \vec{S}$ if and only if for some 
degrees $d_1, \ldots, d_h$ of  $\vec{C_1}$, \ldots,  $\vec{C_1}$ we have
$$
\sigma \ \ \ 
= \ \ \ 
(
...
\item
For $\pi_1, \pi_2 \in \ProofUnfolding$ we we write $\pi_1 \preceq \pi_2$ for: 
$\size(\pi_1,\sigma_1) \le \size(\pi_2,\sigma_2)$ in the lexicographic order,
when $\sigma_1 \state \pi_1$ and $\sigma_2 \state \pi_2$.
\end{enumerate}
\end{definition}

The state of each path $\pi$ exists and it is unique, therefore we can compute the truth
value of $\pi_1 \le \pi_2$. The computation is polynomial time and only depends on
the state of $\pi_1$, $\pi_2$ and on $\beta_1, \ldots, \beta_h$.
...
that $\setR^+ \subseteq \TypedRel$ satisfies the Injectivity Condition IC and the Progressing 
...
$\setR^+$.
For each state $\sigma \in \States(\setR^+)$ we define a state 
$\tau = \trans(\sigma,R) \in \States(\setR^+)$ such that if $\sigma \state \pi$ then 
$\tau \state \pi_1$. We also check that $\pi \succ \pi_1$ in the lexicographic ordering.
...
$$
\trans(\sigma,S) = (U_1, d_1, V_1; \ \ldots; U_{i-1}, d_{i-1},V_{i-1}; \ U'_i, 0, \ U'_i)
$$
...
$$
\trans(\sigma,S) = 
(U_1, d_1,V_1;  \ \ldots; U_{i-2}, d_{i-1},V_{i-2};  \ U'_{i-1}, 0, U'_{i-1})
$$
...
$$
\trans(\sigma,S) = 
(U_1, d_1,V_1;  \ \ldots; U_{i-2}, d_{i-1},V_{i-2};  \ U'_{i-1}, d_{i-1}+1, U'_{i})
$$
...
We extend the transition map $\trans$ to any path in the proof-graph moving from the position
$\pi$ in the proof-graph of the state $\pi$ 
by repeating the one-step transition map for each edge of the path.
...
We set $\trans(\sigma,\pi') = \trans(\tau,R)$, the empty state.
