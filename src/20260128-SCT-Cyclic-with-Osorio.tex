...
If $R_1 \cap (\dom(R_1) \times \dom(R_2))$ is a bijection then 
$\size(R_1) \ge \size(\dom(R_2)) \ge \size(\beta,R)$.
...
then $\size(R_n) = \size(\dom(R_n) = \size(R_{n+1})$ is stationary,
...
we conclude that $\size(R_n) = \size(\dom(R_n)) = \size(R_{n+1})$.

Thus, if $R_1 \ldots R_n$ has stationary degree $d$, then  
$ \size(\dom(R_{n-d})) = \ldots = \dom(R_{n-1}) = \size(\dom(R_n))$, hence
all relations $R_{n-i}$ are stationary bijections between $\dom(R_{n-i})$
and $\dom(R_{n-i+1})$. Thus,
$R_1 \ldots R_{n+1} $ has stationary degree $d+1$.
...
Suppose $i \in [2,h]$. By assumption, $\dom(U'_i) \subset \dom(U_i)$
 ...
 $S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_n R)$ is a component with two or 
 more elements, therefore $1$ is a degree of its product $U'_{i-1}$ in $\setR^+$.
 ...
 This is a component with two or more elements, therefore $1$ is a degree of its product  
