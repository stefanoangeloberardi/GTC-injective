...
time \cite{Lee2} 
and an inductive proof in output of size $O(2^{n \log(n)})$.
...
of poly-exponential size, instead than an ouput of size $O(2^{n \log(n)})$.
   ...
   \section{Quick Overview of the GTC and PDC Conditions}
Let us start from a cyclic proof $P$ with set $\setR$ of bicolored relations on a set 
$[1,\MaxInd]$. $\MaxInd$ is the maximum number of inductive formula in the left-hand side 
of some sequent in $\Pi$. A bicolored relation is any pair $(\Stat,\Prog)$ of relations on
$[1,\MaxInd]$. Each relation $R$ is associated to an arrow $a \rightarrow b$ 
between nodes of $\Pi$, and any two relations $R:a \rightarrow b$, $S:b \rightarrow c$
are said to be composable. We call $\setR$ the set of relations $R:a \rightarrow b$
in the proof, for some $R, a, b$.
...
\questionMarks{relations}. We write $\dom(R) = \{x | \exists y (x R y)\}$ and
$R(x)$ for the set $\{y | x R y\}$ of $R$-images of $x \in \dom(R)$. We call 
$\DomCard(R) = \Card(\dom(R))$ the domain-cardinality of $R$. We extend
$\dom$ and $\DomCard$ to relation sequences by taking the product of the sequence:
$\dom(R_1, \ldots, R_n) =  \dom(R_1 \ldots R_n)$ and 
$\DomCard(R_1, \ldots, R_n) = \DomCard(R_1 \ldots R_n)$.


\begin{definition}[Injectivity, PDC and GTC]
\label{definition-inj-PDC-GTC}
Assume $\setR^+$ is the closure of $\setR$ under composition, $R=(\Stat,\Prog)$
is a bi-colored relation on $[1,\MaxInd]$, and $X,Y \subseteq [1,\MaxInd]$ are sets
of positive integer.

\begin{enumerate}
\item
\label{definition-inj-PDC-GTC-01}
$R$ is progressing from $X$ to $Y$ if and only if $\Prog \cap (X \times Y) \not = \emptyset$.

\item
\label{definition-inj-PDC-GTC-02}
The Progressing Domain Condition (PDC) for a bicolored relation $R : a \rightarrow a$ says: 
$R$ is progressing from $\dom(R)$ to $\dom(R)$.

\item
\label{definition-inj-PDC-GTC-03}
We say that closure $\setR^+$ of $\setR$
satisfies PDC if and only if for all $R: a \rightarrow a \in \setR$, $R$ satisfies PDC. 

\item
\label{definition-inj-PDC-GTC-04}
...
\item
\label{definition-inj-PDC-GTC-05}
...
chains of $\setR$, while PDC is a property of single relations $R \in \setR$, $R:a \rightarrow a$. 
...
{\bf An example.} Let $R = (\{(1,1)\},\{(2,1)\})$ with $R$ stationary on $(1,1)$
and progressing on $(2,1)$. Let $\setR=\{R: a \rightarrow a\}$.
Then $R$ satisfies PDC on the graph $\{(a,a)\}$, and $RR=R$, 
therefore  $\setR^+=\{R: a \rightarrow a\}$ and $\setR^+$ satisfies PDC. 
However, the only infinite traces in $R: a \rightarrow a, R: a \rightarrow a, R: a \rightarrow a,
\ldots$ are $(1,1,1,\ldots)$ and $(2,1,1,1,\ldots)$. The
first trace never progresses, the second trace progresses once, on the first arrow $(2,1)$. 
...
In order to do so, we first to derive some elementar properties of the sequences of uncolored injective relations on $[1,\MaxInd]$. These properties have nothing to do with colors and
with cyclic proofs.
...
We define the grouping relation between sequences of relations. An example: a possible 
grouping of $\vec{S} = S_1, S_2, S_3, S_4, S_5, S_6$ is the following list $\vec{T}$
of $3$ products:
 
$$ 
S_1S_2, \ \ \ S_3, \ \ \ S_4 S_5 S_6
$$

\begin{definition}[The Grouping Relation]
Assume $\vec{T} = T_1, \ldots, T_m$ and 
$\vec{S} = S_1, \ldots, S_n$. We define the grouping relation $\vec{T} \grouping \vec{S}$ 
by induction on $n$ as follows.
\begin{enumerate}

\item
If $n=0$, then $\vec{T}$ = empty sequence = $\vec{S}$.

\item
Suppose $\vec{T} = T_1, \ldots, T_m \grouping \vec{S}$. 
Then $T_1, \ldots, T_{i-1}, (T_i \ldots T_m S) \grouping \vec{S},S$, for all $i \in [1,m]$. 
\end{enumerate}
\end{definition}

To put otherwise, the groupings of $\vec{S} = S_1, \ldots, S_n$ are all and only 
sequences of non-empty products of consecutive elements of $\vec{S}$, of the form: 
$$
S_{a_1} \ldots S_{b_1}, \ \ \ 
S_{a_2} \ldots S_{b_2}, \ \ \ 
\ldots, \ \ \ 
...
We say that $\vec{S}$ is stable if and only if for all $i \in [1,n-1]$ we have 
$\dom(S_i S_{i+1}) = \dom(S_i)$.
...
We prove that all sub-segments are stable. We proved that all grouping of $\vec{S}$ are stable.
...
no proof because it is not required for the paper. For instance, 
$S_1 \ldots S_n$ is the maximum grouping and $S_1, \ldots, S_n$ the minimum grouping.

We use the grouping relation $\grouping$ to select a simplest stable grouping of $\vec{S}$.
...
We will prove that there is a minimum stable grouping  w.r.t. $\grouping$ for each $\vec{S}$.
Namely, we claim that 
there is some $\vec{T} \grouping \vec{S}$ such that $\vec{U} \grouping \vec{S}$
is stable if and only if $\vec{U} \grouping \vec{T}$. This is not
self-evident, because $\grouping$ is only a partial ordering.
...
We provide a existence proof and a characterization of the minimum stable grouping of 
...
By definition unfolding, if $j>i$ then $j$ is a dropping index for $i$ in $\vec{S}$
if and only if there is some $x \in [1,\MaxInd]$ having some $S_i \ldots S_{j-1}$-image 
but no $S_i \ldots S_j$-image: some traces from $x$ continue until step $j-1$, then they all
die out. 
...
\begin{enumerate}
\item
We define the integer $\phi(\vec{S},i) \in [i,n]$ as the maximum dropping index $j\in [i,n]$ 
of $i$ in $\vec{S}$. We write just $\phi(i)$ when $\vec{S}$ is clear from the context.
\item
We say that $i \in [1,n]$ is $\phi$-closed if and only if $\phi([1,i[) \subseteq [1,i[$
\end{enumerate}
\end{definition}
...
We define $\vec{S}^{\phi}=R_1, \ldots, R_{h+1}$ for some $h \in \N$ by: 
$$
...
in particular $\vec{S}^\phi$ is the minimum stable grouping of $\vec{S}$ w.r.t. $\grouping$.
\end{lemma}


\begin{proof}
We proved in \ref{lemma-stable-phi-1} that $\vec{S}^\phi$ is stable. 
We now prove that stable grouping of $\vec{S}$ are exactly the grouping of 
$\vec{S}^\phi$, we will deduce that $\vec{S}^\phi$ is the minimum stable grouping of 
$\vec{S}$ w.r.t. $\grouping$.

From $\vec{S}^\phi$ stable and 
...
$$ 
Then Lemma \ref{lemma-equivalent-phi} we have $\phi(a_i+1) \le a_{i+1} $.
...
and so forth. By induction on $j \in [i,n]$ we can prove that $S_i \ldots S_j$ has the form
...
We conclude that all stable grouping are greater or equal than $\vec{S}^\phi$.
\end{proof}
 
We proved the existence of the minimum stable grouping of $\vec{S}$, 
which we call $\minStable(\vec{S})$.
...
$\minStable(\vec{S})$ as the minimum stable grouping of $\vec{S}$ w.r.t. $\grouping$.
\end{definition}
...
and if $\DomCard(RS) \le \DomCard(S)$ then $R$ restricted to $\dom(R) \times \dom(S)$
is a bijection.
...
$\DomCard(RS) \le \Card(\bigcup_{x \in \dom(RS)} X_x) \le \DomCard(S)$.

Now assume that $\DomCard(RS) = \DomCard(S)$. Then 
$\DomCard(RS) = \Card(\bigcup_{x \in \dom(RS)} X_x) = \DomCard(S)$. 
From $\Card(\bigcup_{x \in \dom(RS)} X_x) = \DomCard(S)$, 
$\bigcup_{x \in \dom(RS)} X_x \subseteq \dom(S)$ and $\dom(S)$ finite we deduce
that $\bigcup_{x \in \dom(RS)} X_x = \dom(S)$. 
From the fact that all $\DomCard(RS) = \DomCard(S)$ and all $X_x$ are inhabited we 
...
$$
\forall i,j \in [1,n]. (i \le j) \implies \DomCard(R_i) \le \Card(\dom(R_j))
$$

\item
\label{lemma-injective-stable-sequences-02}
If  $i, j \in[1,n]$ and $i < j$ and $\DomCard(R_i) = \Card(\dom(R_j))$
...
$\Card(\dom(R_j)) = \Card(\dom(R_j \ldots R_n))$.
We conclude that if 
 $i, j \in[1,n]$ and $i < j$ and $\DomCard(R_i) = \Card(\dom(R_j))$
...
If $\vec{S}=S_1, \ldots S_n$ is constant-cardinality domain, then $\DomCard(S_i) = \DomCard(S_j)$ for all $i,j \in [1,n]$ and for all $k \in [i,n]$, $h \in [j,n]$ by
stability we have
$\DomCard(S_i) =\DomCard(S_i \ldots S_k)$ and
$\DomCard(S_j) = \Card(\dom(S_j \ldots S_h))$. This means that all products of
all segments in in $[1,n]$ have domains of the same cardinality: 
...
Thus, each $S_{a_{i}},\ldots,S_{b_{i}}$ is a constant-cardinality domain. 
...
are exactly the \emph{maximal} constant-cardinality-domain components of $\vec{S}$.

\begin{definition}[Suffixes of $\vec{S}$ with increasing cardinality] 
Assume $\vec{S}$ is sequence of injective relations. 
\begin{enumerate}

\item
Assume $\vec{S}$ is stable. Suppose $a_i$ is the first index of 
$S_{a_{i}}, \ldots, S_{b_{i}}$,
the $i$-th constant-cardinality-domain component of $\vec{S}$, with $i \in [1,h]$.
We set
$$
\DomIncr(\vec{S}) = \{S_{a_i} \ldots S_n | i \in [1,h] \}
$$

\item
In general we set 
$$
\DomIncr(\vec{S}) = \DomIncr(\minStable(\vec{S}))
$$
\end{enumerate}
\end{definition}

By definition unfolding, $\DomIncr(\vec{S})$ consists of all products $S_{a_i} \ldots S_n$ 
...
$\DomIncr(\vec{S}) $ is a finite information, no matter what is the size of $\vec{S}$,
and can be defined by induction on $\vec{S}$.

%15:39 29/01/2026

In order to inductively define $\DomIncr(\vec{S}) $, we have to describe how the 
\emph{maximal} constant-cardinality-domain component of $\minStable(\vec{S})$ 
change as $\vec{S}$ increases.
...
$$
\vec{C_j} = \vec{D_j} \ \ \ \mbox{ for all $j \in [1,k]$ }   \mbox{ and } \vec{D_h} = S_{n+1}
$$
...
$$
and for $R$ product of $\vec{C_{i+1}}, \ldots,\vec{C_k}, S_{n+1})$,
$S$ product of $\vec{C_{i}}, \vec{C_{i+1}}, \ldots,\vec{C_k}, S_{n+1})$
$$
\mbox{ either } (c_i > d_i)   
\mbox{ and } \vec{D_i} = S  \ \ \ 
\mbox{ or }    (c_i = d_i)
\mbox{ and } \vec{D_i} = \vec{C_i}, R
$$
\end{lemma}
...
measure is strictly decreasing w.r.t. $<$.
   ...
   \section{Constant-domain of Injective Bicolored Sequences}
...
with associated cardinality $\DomCard(C_1)=c$ has number of elements 
$\le \newton{\MaxInd}{c}$.
\end{lemma}
...
We say that $R \in \setR^+$ has stationary degree $d \in \N$ if and only if
$R$ can be decomposed as a product in $\setR^+$ having a suffix 
$R_1, \ldots, R_{d}$ such that all $R_i$ are stationary 
$\dom(R_i) \times \dom(R_{i+1})$. All traces in the product must move from 
$\dom(R_i)$ to $\dom(R_{i+1})$ through $R_i$, therefore must have $d$ consecutive
stationary steps. 

\begin{definition}[Stationary Degree of a Colored Relation]
Assume $\setR$ is a set of injective colored relations on $[1,\MaxInd]$
$R \in \setR^+$. 

We call any $d \in \N$ a stationary degree of $R$  in $\setR^+$ 
if and only if for some constant-cardinality domain sequence 
$S, R_1, \ldots, R_{d+1}$ in $\setR^+$ we have:

\begin{enumerate}

\item
$R_i \cap \dom(R_i) \times \dom(R_{i+1})$ is a stationary for all $i \in [1,d]$. 

\item
$S R_1 \ldots R_{d+1} = R$.

\end{enumerate}
\end{definition}

All $R \in \setR^+$ have at least one stationary degree in $\setR^+$, which is $0$. 
Suppose $R  \in \setR^+$ has stationary degree $d$, namely $R$ decomposes as 
$S R_1 \ldots R_{n+1}$ with all $R_i \cap \dom(R_i) \times \dom(R_{i+1})$
stationary. 

For all $i, j \in [1,d]$, $i \le j$ we have $R_i \ldots R_j \cap \dom(R_i) \times \dom(R_{j+1})$
equal to the product of all $R_i \cap \dom(R_i) \times \dom(R_{i+1})$, 
\ldots, $R_j \cap \dom(R_j) \times \dom(R_{j+1})$ and therefore it is stationary.
...
\item
If $S, R_1, \ldots, R_{d+1},T$ has stationary degree $d$,
Assume $S, R_1, \ldots, R_{d+1},T$ is constant-domain $\setR^+$-sequence, with 
$c = \DomCard(S)$ the domain-cardinality of the sequence. Then either 
$S \cap \dom(S) \times \dom(T)$ is a progressing bijection on $\dom(S) \times \dom(T)$
or $RS, T$ has degree $d+1$.

\item
Suppose $\setR^+$ stisfies PDC.
...
\item
Suppose that for
some $ T \in \setR^+$ the sequence $S, R_1, \ldots, R_{d+1}$ has constant domain-cardinality.
If $R_{d+1} \cap \dom(R_{d+1}) \times T)$ is progressing we are done.
Suppose $R_{d+1} \cap \dom(R_{d+1}) \times T)$ is stationary. 
Then $RT \in \setR^+$ has stationary degree $d+1$.

\item
$S, R_1, \ldots, R_{d+1}$ has constant-cardinality domain, therefore it is stable
and $\dom(R_i \ldots R_j) = \dom(R_i)$. If $\setR^+$ satisfies PDC, then 
$R_i \ldots R_j$ is progressing from $\dom(R_i \ldots R_j)$ to $\dom(R_i \ldots R_j)$,
namely from  $\dom(R_i)$ to $\dom(R_i)$. Instead $R_i \ldots R_j$ is \emph{not}
progressing from  $\dom(R_i)$ to $\dom(R_{j+1})$, this forces 
$\dom(R_i) \not = \dom(R_{j+1})$.
Let $c = \DomCard(R_1)$ the common 
domain cardinality of $R$ and of all products of consecutive elements of 
$S, R_1, \ldots, R_{d+1}$. 
There are $\newton{\MaxInd}{c}$ subsets of $[1,\MaxInd]$
of cardinality $c$, therefore $d \le \newton{\MaxInd}{c}$. 
If $d$ is any of the degrees of
$\vec{C}$, then we can extend $\vec{C}$ in a stationary way to 
an constant-domain stationary sequence of colored relations for at most
$\newton{\MaxInd}{c}-d$ times. When $\newton{\MaxInd}{c}-d=0$, in 
all constant-domain extensions $S, R_1, \ldots, R_{d+1},T$ of the sequence
the relation $R_{d+1}$ is progressing between $\dom(R_{d+1})$ and $\dom(T)$.
\end{enumerate}
\end{proof}


   \section{Injective bicolored Relation Sequences}
...
We assume having for each predicate a size map $\size : Assignments, 
[1,\MaxInd] \rightarrow \N^+$, returning an expression denoting a 
\emph{positive} integer. We require that:
\begin{enumerate}
\item
if $q(\vec{t})$ is the inductive formula number $i$, 
and $\sigma$ is a closed assigment making $q(\sigma(\vec{t}))$ derivable with inductive
rules, then $q(\sigma(\vec{t}))$ is derivable with some proof $\Pi$ with
$\sigma(\size(\alpha,i))$ inductive rules.
\item
if $q(\vec{u})$ is the inductive formula number $j$ and
and $q(\sigma(\vec{u}))$ is an assumption
of the conclusion of $\Pi$, then $\size(\sigma,j) < \sigma(\sigma,i)$. 
\end{enumerate}
We usually skip $\alpha$ and we just write $\size(i), \size(j)$.
...
\begin{enumerate}
\item 
if $q(\vec{t}) = \N(v)$, then $$\size(i)=v+1$$ because if $v$ has value $a$ 
...
We extend the size map to sets of inductive formulas in a node $\alpha$,
and relations $R$ associated to a path between two nodes $\alpha$ and $\beta$.
...
\item
For each relation $S$ we define $S(x) = \{y \in [1,\MaxInd] | x S y \}$. 
If $R$is colored we define $R(x) = \universe{R}(x)$, forgetting colors.
\item
We associate to each colored $R:\alpha \rightarrow \beta$ 
an index $\size(\beta, R) =
...
If $R:\alpha \rightarrow \beta$, $S:\beta \rightarrow \gamma$ 
are colored injective and $\dom(RS) \subset \dom(R)$ then
...
$x \in \dom(R)$ and if $z \in RS(x)$ then for some $y \in R(x)$ we have $x R y S z$
...
If $\vec{S} = S_1, \ldots, S_n \in \setR^+$ is stable then we will define a list of 
sizes $\size(\vec{S})$ for $\vec{S}$: 
For a generic injective sequence $\vec{R}$, we will first compute
the Minimum Stable Grouping of $\vec{R}$,
otherwise we cannot prove that the size is a decreasing map. 

\begin{definition}[Size of an Injective Sequence]
Suppose $\dom(S)$ is any injective domain-stable sequence with $h$ constant-components
$S_{a_i}\ldots S_{b_i}$ for $i \in [1,h]$. Then we set:

$$
\size(\vec{S}) 
= 
(
( \size(S_{a_1}\ldots S_n), \deg(a_1,\vec{S}) ), 
\ldots,
( \size(S_{a_h}\ldots S_n), \deg(a_h,\vec{S}) )
)
$$

If $\dom(R)$ is any injective sequence then we set 
$\size(\vec{S}) = \size(\minStable(\vec{R}))$.
\end{definition}

If $\vec{S}$ is stable then $\minStable(\vec{S}) = \vec{S}$, therefore 
...
If $\vec(S) = S_1, \ldots, S_n$ 
is any injective domain-stable sequence with constant-domain-cardinality
components $S_{a_i},\ldots, S_{b_i}$ for $i \in [1,h]$, 
and $d_i$ is a degree of $S_{a_i},\ldots, S_{b_i}$, then a state $\sigma$ of $\vec{S}$ is
$$
\sigma \ \ \ 
= \ \ \ 
(
S_{a_1}\ldots S_{b_h}, d_1, 
\ldots,
S_{a_h}\ldots S_{b_h}, d_h,
S_n
$$
If $\dom(R)$ is any injective sequence then a state $\sigma$ of $\dom(R)$ is a state
$\tau$ of $\vec{S} = \minStable(\vec{R})$. 
If $\sigma$ is any state we set:
$$
\size(\sigma)
=
(
\size(S_{a_1}\ldots S_{b_h}), d_1, 
\ldots,
\size(S_{a_h}\ldots S_{b_h}), d_h,
S_n
)
$$
...
$\minStable(\vec{R})$ has constant-domain-cardinality components  
...
Suppose we add any composable 
$R : \beta \rightarrow \gamma$ 
to the sequence $\vec{R}:\alpha \rightarrow \beta$. For each state $\sigma$ of 
$\vec{R}$ we define a next state $\tau = \trans(\sigma,R)$ in such a way
that $\sigma > \tau$ in the lexicographic ordering.

We consider the cases of Lemma \ref{lemma-cardinality-stability}. 
Suppose $\minStable(\vec{R}) = \vec{S}$.

\begin{definition}
\label{definition-transition}
Assume $\vec{R}, R$ is any sequence of injective relations, with $c = \DomCard(R)$. 
Suppose a state $\sigma$ of $\vec{R}$ is
$$
(
(\size(R_{a_1}\ldots R_{n}), d_1), 
\ldots,
...
$$ 
 and the constant-cardinality-domain components of
...
$S:\beta \rightarrow \gamma \in \setR^+$ we have $RS:\beta \rightarrow \gamma \in \setR^+$.
...
The state $\sigma$ of the Translation algorithm is either the empty sequence $()$,
...
By Lemma \ref{lemma-stable-sequences}.\ref{lemma-stable-sequences-01}, 
...
For each state $\sigma$ and each $R \in \setR$ we define a transition 
$\sigma \rightarrow_{R} \tau$ for some state $\tau$. Let $c = \DomCard(R)$.
Let 

$$
R'_i = R_i R \mbox{ for all $i \in [1,n]$ \ \ \ and }R'_{n+1} = R
$$ 

The first condition about is still verified for $R'_1, \ldots, R'_{n+1}$.

By Lemma \ref{lemma-domain-stability}.\ref{lemma-domain-stability-01}, 
we have $\dom(R'_i) = \dom(R_i R) \subseteq \dom(R_i)$ for
all $i \in [1,n]$. 


\begin{definition}[The Transition Function $f(\sigma,R)$]
\label{definition-transition-state}
Let $R_1, \ldots,R_n, R, R'_1, \ldots, R'_{n+1}$ as above.
Let $c = \DomCard(R'_i)$.
Suppose that $i$ is the first element of $i \in [1,n+1]$ such that:

\begin{enumerate} 
\item
either $i \in [1,n]$ and $\dom(R'_i) \subset \dom(R_i)$
\item 
or $i=n+1$
\end{enumerate}

We define a transition function $f(\sigma,R)$. 

\begin{enumerate}

\item 
\emph{Assume that $i=1$ or $\DomCard(R'_i) \not = \DomCard(R'_{i-1})$}.
Then $f$ sets the counter number $i$ to its maximum value

$$
f(\sigma,R) = (R'_1, d_1, \ldots, R'_{i-1}, d_{i-1}, R'_i, \newton{\MaxInd}{c}, R'_i)
$$

\item
\emph{Assume that $i>1$ and $\DomCard(R'_i) = \DomCard(R'_{i-1}) $
for some $c \in \N$}.
\begin{enumerate}
\item
\emph{Assume $R$ is progressing from $\dom(S)$ to $\dom(R)$}.
Then $f$ sets the counter number $(i-1)$ to its maximum value

$$
f(\sigma,R) = (R'_1, d_1, \ldots, R'_{i-1}, \newton{\MaxInd}{c}, R'_i)
$$

\item
\emph{Assume  $R$ is stationary from $\dom(S)$ to $\dom(R)$}.
Then $f$ reduces the counter number $(i-1)$ by one, unless the counter is zero:

$$
f(\sigma,R) = (R'_1, d_1, \ldots, R'_{i-1}, \max(0,d_{i-1} - 1), R'_i)
$$
...
\begin{lemma}
If $\sigma$ is a state for $\setR^+$ and $R \in \setR^+$ then $f(\sigma,R)$ is a state 
for $\setR^+$.
\end{lemma}

We define a graph $\setG$ with transitions $\sigma \rightarrow_R f(\sigma,R)$ labeled
by $R \in \setR$.

\begin{theorem}
In the part of $\setG$ reachable from the initial state $()$ we have 
$\size(\sigma) > \size(f(\sigma,R))$ lexicographically.
\end{theorem}
...
$\size(R) = \size(R_{n+1})$.
By Lemma \ref{}, $\dom(R) = \dom(R_1)$
and $R_1 \ldots R_n$ restricted to $\dom(R_1)$ and  $\dom(R_{n+1})$ 
...
then for all $x \in \dom(R_1)$ there is exactly  one $y \in \dom(R_1)$ with $y = \psi(x)$.
...
Lemma \ref{} implies that the sets $\dom(R_i)$ are distinct 
for $i \in [2,n+1]$, namely that they are at most $\newton{\MaxInd}{c}$.
The segment  $[2,n+1]$ has cardinality $((n+1)-2)+1$, namely $n$.
We conclude that $n \le \newton{\MaxInd}{c}$.
\end{enumerate}
\end{proof}
...
\item 
$R_1, \ldots, R_n, R_{n+1}$ is a constant-cardinality-domain sequence of 
relations in $\setR^+$. 
\item
...
\begin{enumerate}
\item
$R_n \cap \dom(R_{n}) \times \dom(R_{n+1})$ is stationary.
\item
If $R_1 \ldots R_n$ has stationary degree $d$, then 
...
we conclude that $R_n \cap \dom(R_{n}) \times \dom(R_{n+1})$ is stationary.

Thus, if $R_1 \ldots R_n$ has stationary degree $d$, then 
$R_1 \ldots R_{n+1} $ has stationary degree $d+1$.
...
the inductive formula number $x$ of $\alpha$ progresses to the inductive formula number $y$
 ...
 $f(\alpha,\vec{S})=(l_1, \ldots, l_h)$ elements. If $c_i = \DomCard(R_{a_i})$,
