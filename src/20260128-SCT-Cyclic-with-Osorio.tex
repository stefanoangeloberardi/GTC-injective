...
$\vec{S},S$ of length $n+1 \ge 2$ we only we have to know 
...
bits bounded by  $\MaxInd \times 2\MaxInd^2$.
...
\label{lemma-length-bound-03bis}
Assume $R \cap (\dom(R) \times \dom(S))$ is a bijection.
...
We have $\size(\pi,\dom(S)) \ge \size(\pi_1,S)$ by point \ref{lemma-length-bound-02} above.

\item
%\label{lemma-length-bound-03bis}
Assume $R \cap (\dom(R) \times \dom(S))$ is a bijection and 
$\size(\pi_1,R) = \size(\pi_2,RS)$.
By point\ref{lemma-length-bound-01} above we have $\size(\pi_1,R) = \size(\pi_2,S)$.
By point \ref{lemma-length-bound-03} above we have
...
by point {lemma-length-bound-03} above, therefore $\size(\pi_1,R) > \size(\pi_2,S)$
...
%
%By Lemma \ref{lemma-length-bound}.\ref{lemma-length-bound-03}
...
Lemma \ref{lemma-length-bound}.\ref{lemma-length-bound-03bis} 
...
We can decide $\size(\beta,\sigma_1) < \size(\gamma,\sigma_2)$ in polynomial time.
...
$\setR^+$ and $\sigma \in \States$. We define a state 
$\tau = \trans(\sigma, R : \beta \rightarrow \gamma) \in \States$ 
such that if $\sigma = \state(\pi)$ then $\tau = \state(\pi_1)$. We also check that 
$\pi \succ \pi_1$ in the lexicographic ordering.
...
\begin{definition}
[The Transition Function $\trans(\sigma,R : \beta \rightarrow \gamma) \in \States(\setR^+)$]
\label{definition-transition-state}

Suppose $\sigma = (\sigma_1, \ldots, \sigma_h) \in \States(\setR^+)$  and 
$\sigma_i = (U_i, d_i,V_i,\beta_i)$ for $i \in [1,h]$, with
$U_i = S_{a_i} \ldots S_{b_i}$
Suppose $(R:\beta \rightarrow \gamma) \in \setR^+$ and $\beta = \beta_h$.  We abbreviate
$$
U'_i = (U_i \ldots U_h R) \mbox{ for all $i \in [1,h+1]$}
$$

Suppose that $i$ is the unique element of $i \in [1,h+1]$ such that
$\dom(U'_j) = \dom(U_j)$ for all $j \in [1,i-1]$ and:
...
We define $\tau = \trans(\sigma,R:\beta_h \rightarrow \gamma)$. 

\begin{enumerate}

\item 
\emph{Assume that $i=1$ or $\DomCard(U'_i) \not = \DomCard(U'_{i-1})$}.
Then $\tau = (\sigma_1, \ldots, \sigma_{i-1}, \tau_i)$ and
...
Then $\tau = (\sigma_1, \ldots, \sigma_{i-2}, \tau_{i-1})$ 
...
to $\dom(S_{a_i} \ldots S_m R)$.  
 By Lemma \ref{lemma-length-bound}.\ref{lemma-length-bound-05} we deduce that 
 $
