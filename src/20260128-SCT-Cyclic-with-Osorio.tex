...
the stationary color, therefore possibly stationary. 
...
of length $n \ge 1$. The stationary degree $\deg(\vec{S})$ of $\vec{S}$ is the maximum $d<n$ 
such that $\vec{S}$ has some length $d$ possibly stationary suffix.
\end{definition}

All sequences have the possibly stationary suffix $\nil$, of length $0$.
Therefore $\deg(\vec{S})$ is a maximum on a set including $0$. 

Assume $\vec{S}$ is some constant domain-cardinality sequence
of length $n \ge 1$. Let $d = \deg(\vec{S})$.

\begin{enumerate}
\item
Assume $n=1$. Then the unique length $0$ suffix is the
possibly stationary sequence $\nil$, therefore $d=0$.
\item
Assume $n \ge2$. We have $d=0$ if and only if $S_n$ is necessarily progressing.
We have $d \in [1,n-1]$ if and only if $S_n$ is possibly stationary,
$S_{n-d}$ is necessarily progressing from $\dom(S_{n-d})$ to  $\dom(S_{n-d+1})$, 
and for all $i \in [n-d+1,n-1]$, $S_{i}$ is possibly stationary 
from $\dom(S_{i})$ to  $\dom(S_{i+1})$.
\end{enumerate}
...
$\setR^+$. For each state $\sigma \in \States(\setR^+)$ we define a state 
...
$\sigma_i = (U_i, d_i,V_i,\beta_i)$ for $i \in [1,h]$.
