...
For sake of simplicity we consider the case in which we have a unique inductive predicate
$N$. 
...
returning $0$ if there is no $i$-th inductive formula.
...
relation and $\pi \in \ProofUnfolding$ a finite composable path.
\begin{enumerate} 
\item
$\size(\pi, X) = \Sigma_{i \in X} \size(\pi,i)$.

\item
$\size(\pi, R) = \Sigma_{x \in \dom(R)} \max( \{ \size(\pi,y) | y \in R(x) \} )$.
\end{enumerate}
\end{definition}
...
and $\pi_1 = \pi, (R:\alpha \rightarrow \beta)$, $\pi_2 = \pi, (S:\beta \rightarrow \gamma)$
...
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi_1,y) | y \in R(x) \}
$, 
because the second sum has more addends and all addends are positive. We conclude that 
$
\size(\pi_2, RS)                                                                                    =
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_2,z) | z \in RS(x) \})     \le
...
and $\pi_i = \{(S_j:\alpha_i \rightarrow \alpha_{j+1}) | j \in [a_i,b_i]\}$ is the typed version of 
$\vec{C_i}$.
Suppose $C_i = S_{a_i}\ldots S_{b_i}$ is the product of $\vec{C_i}$.
\begin{enumerate}

\item
We say that a sequence $\sigma \in \Seq(\ColRel \times \N \times \ColRel)$ 
is a state of $\vec{S} = S_1, \ldots, S_n$ and we write $\sigma \state \vec{S}$ 
if and only if for some degree $d_i$ of  $\vec{C_i}$ we have
$$
\sigma \ \ \ 
= \ \ \ 
(
...
\item
For $\sigma, \tau \in \States$ we we write $\sigma \le_\pi \tau$ for: 
$\size(\pi,\sigma) \le \size(\pi,\tau)$ in the lexicographic order.
\end{enumerate}
\end{definition}
...
Suppose $\pi_1 = \pi, (R : \beta \rightarrow \gamma) \in \ProofUnfolding$ is a typed path in $\setR^+$.
For each state $\sigma \in \States(\setR^+)$ we define a state 
$\tau = \trans(\sigma,R) \in \States(\setR^+)$ such that if $\sigma \state \pi$ then 
$\tau \state \pi_1$ and $\sigma > \tau$ in the lexicographic ordering.
