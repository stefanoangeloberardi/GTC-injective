...
and $\pi_1 = \pi, (R:\alpha \rightarrow \beta) \in \setT$ a finite composable path.
\begin{enumerate}
\item
\label{lemma-decreasing-size-01}
$\size(\pi_1, RS) \le \size(\pi, R)$.
\item
\label{lemma-decreasing-size-02}
If $\dom(RS) \subset \dom(R)$ then
$\size(\pi_1, RS) < \size(\pi, R)$.
\end{enumerate}
\end{lemma}
...
\item
%\label{lemma-decreasing-size-01}
We have $\size(\pi_1, RS) =
\Sigma_{x \in \dom(RS)} \max(\size(\pi_1, RS(x))$. For all $x \in \dom(RS)$ we have
$x \in \dom(R)$ and if $z \in RS(x)$ then for some $y \in R(x)$ we have $y S z$,
therefore $\size(\pi,y) \ge \size(\pi_1,z)$. We deduce 
$\max( \{ \size(\pi_1,z) | z \in RS(x) \}\le \max( \{ \size(\pi,y) | y \in R(x) \}$, 
hence 
$
\size(\pi_1, RS)                                                                                   =
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_1,z) | z \in RS(x) \})     \le
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi,y) | y \in R(x) \}             \le
$
(by $\dom(RS) \subseteq \dom(R)$)
$
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi,y) | y \in R(x) \}             =
\size(\pi, R)
$.
...
Then 
$
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi,y) | y \in RS(x) \}           <
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi,y) | y \in R(x) \}
$, 
...
\size(\pi_1, RS)                                                                                    =
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_1,z) | z \in RS(x) \})     \le
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi,y) | y \in R(x) \}              <
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi,y) | y \in R(x) \}              =
\size(\pi, R)
$, 
as wished.
...
Assume $\pi_1 = \pi, (R:\alpha \rightarrow \beta) \in \setT$.
\begin{enumerate}
\item
...
Then $\size(\pi, R) \ge \size(\pi,\dom(S)) \ge \size(\pi_1,S)$.

\item
\label{lemma-length-bound-04}
Assume $R,S \in \InjColRel$ is a constant-cardinality domain sequence.
If $\size(\pi,R) = \size(\pi_1,S)$, then $\size(\pi,R) = \size(\pi,\dom(S)) = \size(\pi_1,S)$

\item
\label{lemma-length-bound-05}
Assume $R,S \in \InjColRel$ is a constant-cardinality domain sequence.
If $\size(\pi,\dom(R)) = \size(\pi,R)$, then $\size(\pi,RS) > \size(\pi_1,S)$
if and only if $\size(\pi,\dom(R)) > \size(\pi,\dom(S))$ or 
...
we conclude that $\size(\pi,R) = \size(\dom(\pi,S)) = \size(\pi_1,S)$. 
...
then $\size(R_n) = \size(\dom(R_{n+1})) = \size(R_{n+1})$,
...
$\size(\dom(R_{n+1})) = \size(R_{n+1})$. Now assume $d \ge 1$. 
$R_n \cap \dom(R_n) \times \dom(R_{n+1})$ is a bijection. By definition of degree
$d \ge 1$ we have $\size(\dom(R_{n})) = \size(R_n)$, therefore 
...
Assume  $\setR^+ \subseteq \InjColRel$ and $\vec(S) \in \Seq(\setR^+)$ is stable.
Suppose $\vec{S}$ has components $\vec{C_i} = S_{a_i},\ldots, S_{b_i}$ for $i \in [1,h]$.
Suppose $P_i = S_{a_i}\ldots S_{n}$ is the product of $\vec{C_i}, \ldots, \vec{C_h}$.
\begin{enumerate}
\item
We say that a sequence $\sigma \in \Seq(\setR^+ \times \N \times \setR^+)$ is a state of 
$\vec{S} = S_1, \ldots, Sn$ 
...
\item
Assume $\vec(R)$ is any injective sequence. Then $\sigma \state\vec(R)$ in $\setR^+$ 
if and only if $\sigma \state \minStable(\vec{R})$ in $\setR^+$. 
...
\States = 
\{\sigma \in \Seq(\setR^+ \times \N \times \setR^+)
  | \exists \vec{R} \in \Seq(\setR^+) (\sigma \state \vec{R})\}
$$
the set of states and for $\sigma, \tau \in \States$ we we write $\sigma \le \tau$ for 
$\size(\sigma) \le \size(\tau)$ in the lexicographic order.
\end{enumerate}
\end{definition}
...
Recall that we can decide whether $\size(\dom(R)) = \size(R)$ without computing the
...
\begin{proof}
Suppose $\setR^+$ is injective, $\pi = \{R_i:\alpha_i \rightarrow \alpha_{i+1}| i \in [1,n]\}$ 
is a sequence on $\setR^+$ untyped sequence $\vec{R}= \{R_i | i \in [1,n]\}$.  
...
Assume $\sigma = (U_1, d_1,V_1; \ldots; U_h, d_h, V_h) \state \pi$: by definition of $\state$
we have $U_i = S_{a_i}, \ldots, S_m $ and $V_i = S_{b_i}$. 

Let $\tau = \trans(\sigma,R) $. Suppose $\pi_1 = \pi, (R:\beta \rightarrow \gamma) \in \setT$ , 
in order to prove that $\tau \state \vec{R},R$. We argue by cases on the definition of $\tau$. 
Let
$$
...
Thus $(S_{b_i} \ldots S_m R)$ is a component of $\vec{R},R$ made of a single product, 
therefore $\tau \state \vec{R},R$. From $\dom(U'_i) \subset \dom(U_i)$ 
and Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-02}
...
(by stability and 
Lemma \ref{lemma-domain-stability}.\ref{lemma-domain-stability-02})  
$ \DomCard(S_{a_{h}} \ldots S_{b_{h}}$. Namely $R = U_{h+1}$ is a one-product 
component of $\vec{R},R$, with a larger domain-cardinality, and $\tau=\trans(\sigma,R)$ is the
concatenation of $\sigma$ and of $(R,0,R)$. We deduce $\tau \state \vec{R},R$ and 
$\sigma > \trans(\sigma,R)$. 
\end{enumerate}
...
\item
\emph{Assume either $U'_{i}$ has some necessarily progressing
$x \in \dom(U'_{i})$ or $d_{i-1}\ge 1$ and $V_{i-1}$ is progressing from $\dom(V_{i-1})$ to $\dom(U'_{i})$}. Assume
...
Suppose $i \in [2,h]$. By assumption, $\dom(U'_i) \subset \dom(U_i)$
 ...
 $0$ is a stationary degree of the last component. We deduce $\tau \state \vec{R},R$.
 
 We assumed that either $U'_{i}$ has some necessarily progressing $x \in \dom(U'_{i})$ 
 or $V_{i-1}$ is progressing from $\dom(V_{i-1})$ to $\dom(U'_{i})$. 
 By Lemma \ref{lemma-decreasing-size} we deduce that 
 %12:32 12/02/2026 
 $\size(U_{i-1}) = \size(V_{i-1} U'_i) > \size(U'_{i}) \ge \size(U'_{i-1})$. 
 Thus, $\sigma > \tau$.
 \item
 Suppose $i=h+1$. Therefore $\minStable(\vec{R},R) = S_1, \ldots, S_{b_h}, R$
 ...
 By Lemma \ref{lemma-decreasing-size}we deduce that 
 $\size(U_{h}) =  \size(V_{h} U'_{h+1})  > \size(U'_{h+1}) \ge \size(U'_{h})$.  
 Thus, $\sigma > \tau$.
 \end{enumerate}
...
If $i \in [2,h]$ then $S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$
 ...
 $d_{i-1}+1$ is a degree of $U'_{i-1}$. We deduce $\tau \state \vec{R},R$.
