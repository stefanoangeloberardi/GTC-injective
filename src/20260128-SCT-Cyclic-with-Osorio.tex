...
\begin{definition}[Stationary degree of $R$ in $\setR^+$]
We say that $R \in \setR^+$ has stationary degree $d \in \N^+$ wr.t. $\vec{C}, \setR^+$
if and only if $\vec{C}= R_1, \ldots, R_{d+1}$ is some 
constant domain-cardinality sequence in $\setR^+$ and:

\begin{enumerate}
\item
$R =  R_1 \ldots R_{d+1}$, namely $R$ is the product of $\vec{C}$

\item
$R_i$ is a stationary bijection from $\dom(R_i)$ to $\dom(R_{i+1})$, for all $i \in [2,d]$.
\end{enumerate}

\end{definition}

In the definition above, all traces in the path $\vec{C}=R_1, \ldots, R_{d+1}$ must move 
from $\dom(R_i)$ to $\dom(R_{i+1})$ through $R_i$, therefore must have $d$ consecutive
stationary steps. 

All $R \in \setR^+$ have at least stationary degree $0$ in $\setR^+$: we take $\vec{C}=R_1$.
If $R,S$ is a constant domain-cardinality sequence then $RS$ has degree $1$:
in this case the condition \quotationMarks{for all $i \in [2,d]$ \ldots} is vacously true
because $[2,1] = \emptyset$.
 
Suppose $R  \in \setR^+$ has stationary degree $d$, namely $R$ decomposes as 
$R_1 \ldots R_{d+1}$ with all $R_i \cap \dom(R_i) \times \dom(R_{i+1})$
stationary.  
Then for all $i, j \in [2,d]$, $i \le j$ we have 
$R_i \ldots R_j \cap \dom(R_i) \times \dom(R_{j+1})$
equal to the product of all $R_i \cap \dom(R_i) \times \dom(R_{i+1})$, 
\ldots, $R_j \cap \dom(R_j) \times \dom(R_{j+1})$, and therefore a stationary bijection.
...
Assume $\vec{C}= R_1, \ldots, R_{d+1}$ and $R$ has stationary degree $d$ in 
$\vec{C}, \setR^+$ and $c=\DomCard(R)$. 
...
\begin{enumerate}
\item
Suppose $R_1, \ldots, R_{d+1}, T$ is a constant domain-cardinality sequence, and 
$R = R_1 \ldots R_{d+1} T$.
By Lemma \ref{lemma-injective-stable-sequences}.\ref{lemma-injective-stable-sequences-02}
$R_{d+1}$ is a bijection from $\dom(R_{d+1})$ to $\dom(T)$.
If $d=0$ then $R$ has degree $1$. Suppose $d \ge 1$.
Either $R_{d+1}$ is a progressing bijection from $\dom(R_{d+1})$ to $\dom(T)$
or it is stationary, and since all $R_i$ with $i \in [2,d]$ are stationary from $\dom(R_i)$
...
Suppose $C_i = S_{a_i}\ldots S_{b_i}$ and $d_i$ is a degree of  $C_i$ w.r.t. 
$\vec{C},\setR^+$.
\begin{enumerate}
\item
A state $\sigma$ of $\vec{S}$ in $\setR^+$ is
$$
\sigma \ \ \ 
= \ \ \ 
(
...
\item
If $\vec(R)$ is any injective sequence then a state $\sigma$ of $\vec(R)$ in $\setR^+$ is
any state $\tau$ of $\minStable(\vec{R})$ in $\setR^+$. 

\item
If $\sigma$ is any state of $\vec(R)$ in $\setR^+$, then 
$$
\size(\sigma)
=
(
...
If  $\sigma$ is a state of $\dom(R)$ in $\setR^+$ then $\size(\sigma)$ is a size of $\vec(R)$ in 
$\setR^+$.
...
to the sequence $\vec{R}:\alpha \rightarrow \beta$ in $\setR^+$. For each state $\sigma$ of 
$\vec{R}$ we define a state $\tau = \trans(\sigma,R)$ of $\vec{R},R$, in such a way
...
\begin{definition}[The Transition Function $\trans(\sigma,R)$]
\label{definition-transition-state}

Suppose $\minStable(\vec{R}) = \vec{S}$ and
$\sigma = (C_1, d_1,T_1; \ldots; C_h, d_h T_h)$. We set
$$
C'_i = C_i R \mbox{ for all $i \in [1,h]$ \ \ \ and } C'_{h+1} = R
$$ 
Suppose that $i$ is the first element of $i \in [1,n+1]$ such that:

\begin{enumerate} 
\item
either $i \in [1,n]$ and $\dom(C'_i) \subset \dom(C_i)$
\item 
or $i=n+1$
\end{enumerate}

We define a transition function $\trans(\sigma,S)$. 

\begin{enumerate}

\item 
\emph{Assume that $i=1$ or $\DomCard(C'_i) \not = \DomCard(C'_{i-1})$}.
Then $\trans$ sets the counter number $i$ to its minimum value $0$

$$
\trans(\sigma,S) = (R'_1, d_1, T_1; \ \ldots; R'_{i-1}, d_{i-1},T_{i-1}; \ R'_i, 0, \ R'_i)
$$
...
$T_{i}$ is progressing from $\dom(T_{i})$ to $\dom(R'_{i-1})$}.
Then $\trans$ sets the counter number $(i-1)$ to $1$

$$
\trans(\sigma,S) = (R'_1, d_1,T_1;  \ \ldots;  \ R'_{i-1}, 1, R'_{i-1})
$$

\item
\emph{Assume $d-{i-1}= 0$ or 
$T_{i}$ is stationary from $\dom(T_{i})$ to $\dom(R'_{i-1})$}.
Then $\trans$ increases the counter number $(i-1)$ by one:

$$
\trans(\sigma,S) = (R'_1, d_1,T_1;  \ \ldots;  \ R'_{i-1}, d_{i-1}+1, R'_{i})
$$
...
If $\sigma$ is a state for $\setR^+$ and $R \in \setR^+$ then $\trans(\sigma,R)$ is a state 
for $\setR^+$.
\end{lemma}
...
$\size(\sigma) > \size(\trans(\sigma,R))$ lexicographically.
\end{theorem}
