...
$[1,\MaxInd]$. 
$\MaxInd \in \N$ is the maximum number of inductive formula in the left-hand side 
...
We denote with $\Rel = \PowerSet([1,\MaxInd] \times [1,\MaxInd])$ the set of binary relations on 
the finite set $[1,\MaxInd]$. $\InjRel$ is the subset of \emph{injective} colored relations.
...
has the progressing color. We call $\ColRel \subseteq \Rel^2$ the set of colored relations. 

If $R= (\Stat,\Prog) \in \ColRel$, we define 
$\universe{R} = \Stat \cup \Prog \in \Rel$ the associated (uncolored) relation and we say
that $R$ is injective if $\universe{R}$ is. 
$\InjColRel \subseteq \ColRel$ is the subset of injective colored relations.
For colored relation we define $\dom(R) = \dom(\universe{R})$ and 
$\DomCard(R) = \DomCard(\universe{R})$. 


We suppose be fixed some finite set $\Nodes = \{\alpha, \beta, \ldots\}$ whose elements
are called nodes. We call $\TypedRel = \ColRel \times \Nodes \times \Nodes$ the set
of typed colored relation, and we write any triple $(R,\alpha,\beta)$ as $R: \alpha \rightarrow \beta$. We say that $(R,\alpha,\beta)$ has domain $\alpha$ and codomain $\beta$.
$(R,\alpha,\beta)$ is injective if $\universe{R}$ is, and  $\TypedInjRel$ is the subset of
typed injective colored relations

We fix a $\setR \subseteq \TypedInjRel$ of typed injective colored relations in the paper. 
...
$\Stat \Stat' \cap \Prog'' = \emptyset$ and the definition of $\Stat''$ in $RS$
can be simplified to $\Stat'' = \Stat \Stat'$. Uncoloring and composition commute:
we can prove that $\universe{RS} = \universe{R}\universe{S}$ for all $R, S \in \ColRel$.
\\
...
Assume that $\setR^+ \subseteq \ColRel$ is the closure of a family $\setR \subseteq \ColRel$ 
under colored composition,  that $R=(\Stat,\Prog) \in \ColRel$, and that 
...
The Progressing Domain Condition (PDC) for $(R : \alpha \rightarrow \alpha)  \in \ColRel$ says: 
$R$ is progressing from $\dom(R)$ to $\dom(R)$.

\item
\label{definition-PDC-03}
$\setR^+$ satisfies PDC if and only if for all $(R : \alpha \rightarrow \alpha) \in \setR$,
$(R : \alpha \rightarrow \alpha)$ satisfy PDC. 
\end{enumerate}
\end{definition}
...
Assume that $\setR^+ \subseteq \TypedRel$ is the closure of a family $\setR \subseteq \TypedRel$ 
...
\begin{enumerate}
\item  
$\setR^+ \subseteq \TypedRel$ is a set of typed injective colored relations.
\item
$\setR^+$ satisfies PDC
...
Then $R: \alpha \rightarrow \alpha$ satisfies PDC %on the graph $\{(a,a)\}$, 
and $RR=R$, therefore  $\setR^+=\{R: \alpha \rightarrow \alpha\}$ and $\setR^+$ satisfies PDC. 
However, the only infinite traces in the infinite path 
$R: \alpha \rightarrow \alpha, R: \alpha \rightarrow \alpha, R: \alpha \rightarrow \alpha, \ldots$ are $(1,1,1,\ldots)$ and $(2,1,1,1,\ldots)$. The first trace never progresses, the second trace progresses 
once, on the first arrow $(2,1)$. 
...
A finite path is any finite composable sequence $\pi = R_1:\alpha_1 \rightarrow \alpha_2,
R_2:\alpha_2 \rightarrow \alpha_3, \ldots, R_n:\alpha_n \rightarrow \alpha_{n+1}$.
We say that $\pi: \alpha_1 \rightarrow \alpha_{n+1}$
$\Paths$ is the set of all finite paths.
We assume having some tree $\setT \subseteq \Paths$ of paths. We assume having
some map $\size:\setT, [1,\MaxInd] \rightarrow \N^+$, assigning a positive integer $\size(\pi,i)$
to each $\pi \in \setT$ and each $i \in [1,\MaxInd]$.

We assume that if $x$ $R$-progresses to $y$ when moving from $\pi:\alpha \rightarrow \beta$ 
to $\pi_1 = \pi, (R:\beta \rightarrow \gamma)$, 
then $\size(\pi, x) > \size(\pi_1, y)$, while if $(x,y)$ is $R$-stationary then 
$\size(\pi, x) = \size(\pi_1, y)$. 
...
This suggests using an induction with one parameter 
$$
\Sigma_{x \in \dom(S_{b_h} \ldots S_m)} \size(\pi,x)
$$ 
Whenever some $x \in \dom(S_m)$ progresses then the sum decreases. 
...
We can fix this objection by taking nested induction on one index for each component:
$$
\Sigma_{x \in \dom(S_{b_1}\ldots S_m)} \size(\pi,x), \ \ \ 
\ldots, \ \ \ 
\Sigma_{x \in \dom(S_{b_h}\ldots S_m)} \size(\pi,x)
$$
When a component is erased we use induction on the index for the last non-erased component
$\Sigma_{x \in \dom(S_{b_i}\ldots S_m)} \size(\pi,x)$.
...
$$
\Sigma_{x \in \dom(S_{b_i} \ldots S_m)} \max(\{\size(\pi,y) | y \in S_{b_i} \ldots S_m(x)\}
$$
expressing such worst-case possibility.

\item
Third, we can have up to $\newton{\MaxInd}{c_{a_i}}$ steps until the index number $i$
decreases (even if such a bound looks a vast exaggeration). We need $h$ auxiliary indexes
...
Such an index decreases by $1$ at each step in which the index number $i$ is the last index 
...
%
%We assume having a size map $\size : \Nodes, [1,\MaxInd] \rightarrow \N^+$, 
...
relation and $\pi \in \setT$ a finite composable path.
\begin{enumerate} 
\item
...
$\pi_2 = \pi_1, (S:\beta \rightarrow \gamma) $ and $\pi_2 \in \setT$.
\begin{enumerate}
\item
...
Suppose $\pi_1 = \pi, (R : \beta \rightarrow \gamma) \in \setT$ is a typed path in $\setR^+$.
...
Suppose $\setR^+$ is injective, $\pi_1 = \pi, (R:\beta \rightarrow \gamma) \in \setT$ 
...
Let $\tau = \trans(\sigma,R) $. Suppose $\pi_1 = \pi, (R:\beta \rightarrow \gamma) \in \setT$ , 
