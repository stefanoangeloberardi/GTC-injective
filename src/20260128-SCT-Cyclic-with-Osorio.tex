...
Assume $R=(\Stat,\Prog) \in \ColRel$ is an colored relation.
We say that $R$ is \emph{necessarily progressing} on $x \in \dom(R)$
...
$\tau = \trans(\sigma,R) \in \States(\setR^+)$ such that if $\sigma \state \pi$ then 
$\tau \state \pi_1$. We also check that $(\pi,\sigma) \succ (\pi_1, \tau)$ 
...
Suppose $\sigma = (U_1, d_1,V_1,\beta_1; \ldots; U_h, d_h, V_h,\beta_h) \in \States(\setR^+)$ 
with $\beta_h = \beta$ and $(R:\beta \rightarrow \gamma) \in \setR^+$. We set
$$
U'_i = U_i R \mbox{ for all $i \in [1,h]$ \ \ \ and } U'_{h+1} = R, \beta_{h+1}=\gamma
$$
...
We define a transition function $\trans(\sigma,R:\beta_h \rightarrow \gamma)$. 
...
\trans(\sigma,S) 
= 
(U_1, d_1, V_1,\beta_1; \ \ldots; U_{i-1}, d_{i-1},V_{i-1},\beta_{i-1}; \ U'_i, 0, \ U'_i, \beta_i)
$$
...
$$
\trans(\sigma,S) = 
(U_1, d_1,V_1,\beta_1;  \ \ldots; U_{i-2}, d_{i-1},V_{i-2},\beta_{i-2}; 
 \ U'_{i-1}, 0, U'_{i-1},\beta_{i-1})
$$

\item
\emph{Assume $\size(\dom(U'_{i})) = \size(U'_i)$ and
($d-{i-1}= 0$ or $V_{i-1}$ is stationary from $\dom(V_{i-1})$ to $\dom(U'_{i})$)}.
Then $\trans$ increases the counter $d_{i-1}$ by one:
...
\item
Let $\pi'=\pi'', (R:\alpha \rightarrow \beta)$ and $\tau = \trans(\sigma,\pi'')$.
We set $\trans(\sigma,\pi') = \trans(\tau,R:\alpha \rightarrow \beta)$, the empty state.
...
is a finite path on $\setR^+$ and  $\sigma \state \pi$ and $R \in \setR^+$. 
Let $\tau = \trans(\sigma,R) $. Then $\sigma > \tau$ and $\tau \state \pi_1$.
\end{theorem}

%18:59 14/02/2026

\begin{proof}
Suppose $\setR^+$ is injective, $\pi = \{R_i:\alpha_i \rightarrow \alpha_{i+1}| i \in [1,n]\}$, $\pi \in \Paths$, $\pi \in \Seq(\setR^+)$ with untyped sequence $\vec{R}= \{R_i | i \in [1,n]\}$.  
...
Assume $\sigma = (U_1, d_1,V_1; \ldots; U_h, d_h, V_h)$, $\sigma \state \pi$.
By definition of $\state$ we have $U_i = S_{a_i} \ldots S_{b_i} $ and $V_i = S_{b_i}$. 

Let $\tau = \trans(\sigma,R) $. 
Suppose $\pi_1 = \pi, (R:\beta \rightarrow \gamma) \in \ProofUnfolding$ , 
in order to prove that $\tau \state \pi_1$. We argue by cases on the definition of $\tau$. 
Let
$$
U'_i = U_i R = S_{a_i} \ldots S_m  R \mbox{ for all $i \in [1,h]$ \ \ \ and } U'_{h+1} = R
$$ 
...
either $i \in [1,h]$ and $\dom(U'_i) \subset \dom(U_i)$
\item 
or $i=h+1$ and $\dom(U'_j) = \dom(U_j)$ for all $j \in [1,h]$
\end{enumerate}
...
$j \in [a_i,b_i]$, and by Lemma \ref{} we have 
$$
...
$$ 
From $j$ the first index such that $\dom(S_{j} \ldots S_m  R) \subset \dom(S_{j} \ldots S_m )$, 
we deduce $j = a_i$.
...
\item 
\emph{Assume that $i=1$ or $i>1$ and $\DomCard(U'_i) \not = \DomCard(U'_{i-1})$}. 
We have
$$
\trans(\sigma,R) = (U_1, d_1, V_1; \ \ldots; U_{i-1}, d_{i-1},V_{i-1}; \ U'_i, 0, \ U'_i)
$$

\begin{enumerate}
\item
Suppose $i=1$. Then $\dom(U'_1) \subset \dom(U_1)$, therefore 
$\minStable(\vec{R},R) = (S_{a_1} \ldots S_m  R) = U'_1$, 
a grouping made of a single product, and $\trans(\sigma,R) = (U'_1, 0, U'_0)$. 
$0$ is a degree of $U'_1$, therefore $(U'_1, 0, U'_0)$ 
is a state of $\vec{R},R$. From $\dom(U'_1) = \dom(U_1 R) \subset \dom(U_1)$ 
and Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-02}
we deduce $\size(U'_1) < \size(U_1)$, hence $\sigma > \trans(\sigma,R)$. 

\item
Suppose $i \in [2,h]$. Then $\dom(U'_i) \subset \dom(U_i)$, namely
$
...
Thus $(S_{b_i} \ldots S_m R)$ is the last component of $\vec{R},R$ made of a single product, 
therefore $\tau \state \pi_1$. From $\dom(U'_i) \subset \dom(U_i)$ 
and Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-02}
we deduce $\size(U'_i) < \size(U_i)$, hence $\sigma > \trans(\sigma,R)$.
\item
Suppose $i=h+1 > 1$. Then $\minStable(\vec{R},R) = S_1, \ldots, S_{b_{h}}, R$.
By assumption from $i>1$ we deduce$\DomCard(U'_{h+1}) \not = \DomCard(U'_{h})$. 
We obtain 
$
\DomCard(R) = 
\DomCard(U'_{h+1}) > 
...
$. 
Namely $R = U_{h+1}$ is a one-product 
component of $\vec{R},R$, with a larger domain-cardinality, and $\tau=\trans(\sigma,R)$ 
is the concatenation of $\sigma$ and of $(R,0,R)$. We deduce $\tau \state \pi_1$ and 
$\sigma > \trans(\sigma,R)$. 
\end{enumerate}
...
\item
\emph{Assume either $V'_{i-1}$ has some necessarily progressing
$x \in \dom(V'_{i-1})$ or $d_{i-1}\ge 1$ and $V'_{i-1}$ is progressing from $\dom(V'_{i-1})$ 
to $\dom(U'_{i})$}. We have

$$
\trans(\sigma,S) = 
(U_1, d_1,V_1;  \ \ldots; U_{i-2}, d_{i-1},V_{i-2};  \ U'_{i-1}, 0, U'_{i})
$$
...
Suppose $i \in [2,h]$. By assumption, $\dom(U'_i) \subset \dom(U_i)$
 ...
 By assumption $\DomCard(U'_i) = \DomCard(U'_{i-1})$. We deduce that 
 $U'_{i} = S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$
 is the last component of $\vec{R},R$, and it has last element 
 $U'_{i} = (S_{a_i} \ldots S_m R)$ and element before the last $V_{i-1} = S_{b_{i-1}}$.
 $0$ is a stationary degree of the last component. We deduce $\tau \state \pi_1$.
 
 We assumed that either $V_{i-1}$ has some necessarily progressing
$x \in \dom(V_{i-1})$ or $d_{i-1}\ge 1$ and $V_{i-1}$ is progressing from $\dom(V_{i-1})$ 
to $\dom(U'_{i})$.  
 ...
 $\dom(U'_{i-1}) = \dom(U_i)$, namely $\DomCard(U'_{h+1}) = \DomCard(U'_{h})$. 
 We have $\minStable(\vec{R},R) = S_1, \ldots, S_{b_h}, R$. We deduce that 
 $S_{a_h}, \ldots, S_{b_h}, R$
 is a component of $\minStable(\vec{R},R)$ with last element $R = U'_{h+1}$. 
 $0$ is a degree of its product $U'_{h}$. Therefore $\tau \state \pi_1$.
 
 We assumed that either $V_{h}$ has some necessarily progressing
$x \in \dom(V_{h})$, namely $\size(\dom(V_h)) > \size(V_h)$, or 
 $V_{h}$ is progressing from $\dom(V_{h})$ to $\dom(U'_{h+1})$,
 namely $\size(\dom(V_{h} U'_{h+1})) > \size(\dom(U'_{h+1}))$.
 By Lemma \ref{lemma-length-bound}.\ref{lemma-length-bound-05} we deduce that 
 $\size(U_{h}) =  
 \size(V_{h})  >
 $
...
\item
\emph{Assume $U'_{i}$ has no necessarily progressing $x \in \dom(U'_{i})$ and
(either $d-{i-1}  = 0$ or $V_{i-1}$ is stationary from $\dom(V_{i-1})$ to $\dom(U'_{i})$)}.
We have

$$
\trans(\sigma,S) = 
(U_1, d_1,V_1;  \ \ldots; U_{i-2}, d_{i-1},V_{i-2};  \ U_{i-1}, d_{i-1}+1, U'_{i})
$$

If $i \in [2,h]$ then $S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$
 is the last component of $\vec{R},R$ with last element $U'_i = (S_{a_i} \ldots S_m R)$. 
 By Lemma \ref{lemma-degree-constant}.\ref{lemma-degree-constant-01},
 $d_{i-1}+1$ is a degree of $U'_{i-1}$. We deduce $\tau \state \pi_1$.
...
is a degree of $U'_{i-1}$. We deduce $\tau \state \pi_1$.
Let $c = \DomCard(U_{i-1}) = \DomCard(U'_{i-1}) $. We conclude that 
$$
 \size(\sigma) = 
