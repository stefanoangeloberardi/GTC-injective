...
Set $\sigma = (\sigma_1, \ldots, \sigma_h) = \state(\pi) \in \States$  where 
...
for $i \in [1,h+1]$, with $U'_i$ the product of $\vec{U'_i}$
and $V'_i =  (S_{a_{i+1}} \ldots S_{b_h} R)$ the last element of $\vec{U'_i}$:
as a particular case we assume that $U'_i{h+1} = R = V'_{h+1}$.
For all $i \in [1,h+1]$ we define a possible element of the next state $\tau$:

$$
\tau_i = 
(U'_i, \deg(\vec{U'_i}), V'_i, \gamma) 
$$
...
In the Mina Theorem we will prove that that 
