...
and therefore it is the maximum stable grouping, but it is trivial. There is no minimum
...
We extend the $\grouping$ relation to a total order $\le$,
and will prove that there is a minimum stable grouping  w.r.t. $\le$ for each $\vec{S}$.
Namely, we claim that 
there is some stable $\vec{T} \grouping \vec{S}$ such that for all stable 
$\vec{U} \grouping \vec{S}$ we have $\vec{U} \le \vec{T}$. 

Lexicographic ordering compares two groupings, looking for the grouping, 
whose products closest to the left are shorter.
...
Lexicographic ordering $\le$ is a total order on a finite set, 
...
$T=\{(1,1)\}$, we have $\phi(1)=1$ because $\dom(RS) = \{1\} = \dom(R)$.
...
\item
\label{lemma-equivalent-phi-03}
Assume that $T_i= S_{a_i} \ldots S_{b_i}$ is a product in a stable grouping 
$\vec{T} = T_1, \ldots, T_m$ of $\vec{S}$, in order to prove that $\phi(a) \le b$. 
...
that is, $\dom(S_a \ldots S_b) = \dom(S_a \ldots S_b S_{b+1} \ldots S_n)$.
By point \ref{lemma-equivalent-phi-02} above, this implies $\phi(\vec{S},a) \le b$.
...
Let $a_1=1$ and $a_{i+1} = \phi(a_i)+1$ 
for all $i \in [1,h-1]$ and $\phi(a_{h}) = n$.
...
\begin{proof}
Assume $S_{a}, \ldots, S_{b}$ is the first product of a stable
grouping (hence $a=1$). 
By Lemma \ref{lemma-equivalent-phi}.\ref{lemma-equivalent-phi-03}.
we have $b \ge \phi(a_1)$, hence the shortest first product is 
...
therefore $\DomCard(R_i) = \DomCard(R_i \ldots R_n))$ and
...
$\DomCard(R_i) = \DomCard(R_i \ldots R_n)$ and
$\DomCard(R_j) = \DomCard(R_j \ldots R_n)$.
By the assumption $\DomCard(R_i) = \DomCard(R_j)$
we deduce $ \DomCard(R_i \ldots R_n) = \DomCard(R_j \ldots R_n)$.

By stability and Lemma \ref{lemma-injective-sequences-03}
$i, j \in[1,n]$ and $i < j$ and $ \DomCard(R_i \ldots R_n) = \DomCard(R_j \ldots R_n)$
we have: 
...
or for some $i \in [1,k]$, and for 
$c_i = \DomCard(\vec{C_i}, d_i = \DomCard(\vec{D_i})$ we have
$$
\vec{C_j} = \vec{D_j} \ \ \ \mbox{ for all $j \in [1,i-1]$}
$$
and for $R$ product of $\vec{C_{i}}, \ldots,\vec{C_k}, S_{n+1}$
we have
$$
(h=i) 
\mbox{ and }    (c_{i} > d_{i})   
\mbox{ and }    (\vec{D_{i-1}} = \vec{C_{i-1}})
\mbox{ and }    (\vec{D_{i}} = R)
$$
...
$$
\end{enumerate}
In the first case $S_{n+1}$ is the stable tail of $\minStable(\vec{S},S_{n+1})$
and in the remaining cases $R$ is the stable tail of $\minStable(\vec{S},S_{n+1})$.
\end{lemma}
...
are $\vec{C_1}, \ldots,\vec{C_k}$. Either $\minStable(\vec{S}),S_{n+1}$ is
stable or not. 
...
$\minStable(\vec{S},S_{n+1}) = \minStable(\vec{S}),S_{n+1}$.
By injectivity and stability we have $\DomCard(\vec{C_h}) \le \DomCard(S_{n+1})$.
\begin{enumerate}
\item
\emph{Assume $\DomCard(\vec{C_h}) < \DomCard(S_{n+1})$}. Then the
components of $\minStable(\vec{S},S_{n+1})$.
are $\vec{D_1}=\vec{C_1}$, \ldots, $\vec{D_h} = \vec{C_h}$ and 
$\vec{D_k} = S_{n+1}$. This is case \ref{lemma-cardinality-stability-01}.
\item
\emph{Assume $\DomCard(\vec{C_h}) = \DomCard(S_{n+1})$}. Then the
components of $\minStable(\vec{S},S_{n+1})$.
are $\vec{D_1}=\vec{C_1}$, \ldots, $\vec{D_h} = \vec{C_h}, S_{n+1}$. 
...
$j$ is one of the raising indexes $a_1, \ldots a_h$ of $\vec{T}$: if it were
$\DomCard(T_{j-1}) = \DomCard(T_j)$, then 
...
If $i>1$ by definition of components
we have $\DomCard(\vec{C_i}) \le \DomCard(S_{n+1})$.
...
strictly decreasing w.r.t. $<_1$ and w.r.t. $\DomCard(\cdot)$. 
\end{enumerate}

Then if $R_1, \ldots,R_k$ are the constant domain-cardinality components of 
...
a stationary bijection by Lemma \ref{}).
...
$R_1, \ldots, R_{d}$ such that all $R_i$ are stationary bijections from
$\dom(R_i)$ to $\dom(R_{i+1})$. All traces in the product must move from 
...
$S, R_1, \ldots, R_{d}$ in $\setR^+$ we have:

\begin{enumerate}

\item
$R_i \cap \dom(R_i) \times \dom(R_{i+1})$ is a stationary for all $i \in [1,d-1]$. 

\item
$R_1 \ldots R_{d} = R$ or $S R_1 \ldots R_{d} = R$.

\end{enumerate}
\end{definition}

All $R \in \setR^+$ have at least one stationary degree in $\setR^+$, which is $1$. 
Suppose $R  \in \setR^+$ has stationary degree $d$, namely $R$ decomposes as 
$S R_1 \ldots R_{d}$ with all $R_i \cap \dom(R_i) \times \dom(R_{i+1})$
stationary. 
...
\ldots, $R_j \cap \dom(R_j) \times \dom(R_{j+1})$, and therefore stationary.
...
Assume $R$ has stationary degree $d$ in $\setR^+$, with 
$c=\DomCard(R)$.
$S$.
...
\item
If $S$ has stationary degree $d$,
then either $S$ is a progressing bijection from $\dom(S)$ to $\dom(T)$
or $R T$ has degree $d+1$.
