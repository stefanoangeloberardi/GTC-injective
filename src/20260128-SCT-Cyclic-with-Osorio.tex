...
An example: a possible grouping of $\vec{R} = R_1, R_2, R_3, R_4, R_5, R_6 \subseteq \Rel$ 
...
\item
If $\vec{S}$ is not empty, then we call $\DomCard(\vec{S}) = \DomCard(S_1)$ 
...
then it can be decomposed into a concatenation of constant domain-cardinality sequences with 
increasing domain-cardinality.
...
has stationary degree $d$, then in the worst case \emph{all} infinite path $\pi$
...
Then all infinite $\pi$ could have one more stationary step, because no matter what is the step 
$x \in \dom(R_d)$ of $\pi$, there is some $y$ such that $R_d$ is progressing on $(x,y)$.
...
Our next task is proving that the stationary degree is at most $\newton{\MaxInd}{c}$, therefore
...
Assume $i,j \in [1,d]$, $i < j$, hence $i \in [1,d-1]$.
...
If $\setR^+$ satisfies PDC, then $R_i \ldots R_{j-1}$ is \emph{progressing} from 
...
of cardinality $c$, and $d$ different domains for $R_1, \ldots, R_{d}$,
therefore $d \le \newton{\MaxInd}{c}$. 
\end{enumerate}
\end{proof}
