...
$R$ is progressing from $\dom(X)$ to $\dom(X)$.
...
\item
We say that closure $\setR^+$ of $\setR$
satisfies GTC if and only if all infinite composable sequences
$R_1 : \alpha_1 \rightarrow \alpha_2, 
  R_2 : \alpha_2 \rightarrow \alpha_3, 
  \ldots,
  R_n : \alpha_n \rightarrow \alpha_{n+1} \in \setR^+$ with $R_i = (\Stat_i,\Prog_i)$ 
  there is some infinite sequence $x_1 R_1 x_2 R_2 x_3 \ldots  x_n R_n x_{n+1}$ such that 
  $(x_i, x_{i+1}) \in \Prog_i$ for infinitely many $i \in \N^+$.
\end{enumerate}
\end{definition}
...
{\bf An example.} Let $R = (\{(1,1)\},\{(2,1)\}): a \rightarrow a$ with $R$ stationary on $(1,1)$
and progressing on $(2,1)$. Then $R$ satisfies PDC on the graph $\{(a,a)\}$, and $RR=R$, 
therefore  $\setR^+=\{R\}$ and $\setR^+$ satisfies PDC. However, the only traces any
infinite path are $(1,1,1,\ldots)$ and $(2,1,1,1,\ldots)$. The
...
$[a,b]$ is any $[a',b'] \subseteq [a,b]$, namely with $[a',b']=\emptyset$ or $a \le a', b' \le b$.
...
hence $(\DomCard(R) = \DomCard(RS))$. If $(\DomCard(R) = \DomCard(RS))$
...
$$
or equivalently $\DomCard(S_i) =  \Card(\dom(S_i \ldots S_j))$.
...
$\DomCard(S_i) =  \Card(\dom(S_i \ldots S_j))$, 
...
All subsegments are stable because the finite quantification defining stability for a subsegment 
...
$\minStable(\vec{S})$ by induction on $\vec{S}$, together with an algorithm computing
...
\begin{definition}[The Transition $\sigma \rightarrow_{R} \tau$]
...
We define a transition $(\sigma \rightarrow_{R} \tau) \in \setG$ for some state $\tau$. 
Let $c = \DomCard(R'_i)$.
...
for some $c \in \N$}.
If $R$ is progressing from $\dom(S)$ to $\dom(R)$
then we reset the counter number $(i-1)$ to its maximum value

$$
\tau = (R'_1, d_1, \ldots, R'_{i-1}, \newton{\MaxInd}{c}, R'_i)
$$

otherwise we reduce the counter number $(i-1)$ by one

$$
\tau = (R'_1, d_1, \ldots, R'_{i-1}, d_{i-1} - 1, R'_i)
$$

\item 
\emph{Assume that $i=1$ or $\DomCard(R'_i) \not = \DomCard(R'_{i-1})$}.
Then we (re)set the counter number $i$ to its maximum value

$$
\tau = (R'_1, d_1, \ldots, R'_{i-1}, d_{i-1}, R'_i, \newton{\MaxInd}{c}, R'_i)
$$
