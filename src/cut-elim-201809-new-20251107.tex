...
From now onw we abbreviate \emph{``$x$ such that $\Nat x$} with  just
\emph{``$x \in \Nat$}.
...
and $\Ack(x,y) \reduces_t z$ for some $t \in \Nat$ is said an image of 
the map $\Ack$ in $(x,y)$.
...
$T(x) = \forall y( \Nat y \rightarrow \Tot(x,y))$. 
\item
The totality of $\Ack$ 
...
$\Ack(Sx,y) \reduces_t z$ for some $t \in \Nat t$, $z \in \Nat$. 
...
We replace the standard totality 
statement $\exists t. (\Nat  t) \wedge \Ack(x,y) \reduces_t z$ for the map $\Ack$ 
with an \emph{inductive predicate} $\Ack(x,y,z)$ in $\LKID$. We replace the correctness 
statement $\exists z. \Nat z \wedge \Ack(x,y,z)$ 
...
$\Nat y$, $\Ack(x,y,z)$, by induction on the definition of $\Ack(x,y,z)$: we do it
in the next subsection.
\end{enumerate}

The inductive rules we choose for $\Ack(x,y,z)$ and $\Tot(x,y)$ are:
...
are required to prove that the image of $(x,y)$ such that $\Nat x$,
$\Nat y$ is in $\Nat$ (see later).
$L$-rules are also required to prove that the image of $(x,y)$ such that $\Nat x$,
$\Nat y$ is unique, but we do not include this proof here.
...
Instead, in LKID we only use an instance on $z$ of the same property, namely: 
...
\infer[\Tot L]
 {\Tot(x,1) \vdash \Tot(Sx,0)}
 {\infer[\Tot R]
    {x=x', \Ack(x',1,z) \vdash \Tot(Sx,0)}
    {\infer[\eq]
      {x=x', \Ack(x',1,z) \vdash \Ack(Sx,0,z)}
...
We put a box around cut formulas. 
...
variables. We frame the cut formula in a box.
...
From each open assumption we obtain $3$ new open assumptions $\Nat 1, \Ack(x',1,z') \vdash \Nat z$ and $\Nat y', \Ack(Sx',y',z') \vdash \Nat z'$ and $\Nat z', \Ack(x',z',w') \vdash \Nat w'$, for some $x', y', z', w' \not \in \FV(x,y,z,w,t,u,v)$. We rename and we re-obtain the $3$ open assumptions: $\Nat 1, \Ack(x,1,z) \vdash \Nat z$ and $\Nat y, \Ack(Sx,y,z) \vdash \Nat z$ and $\Nat z, \Ack(x,z,w) \vdash \Nat w$.
Renaming is possible because $x, y, z, w\not \in \FV(t,u,v,x',y',z',w')$.  
We defined a regular tree $\Pi$.
\\

We prove the global trace condition for $\Pi$. In $\Pi$ whenever we prove some
...
starting from the unique atom $\Ack(t,u,v) $ in the root.
$\Box$ \end{proof}
...
$\Ack$ is the predicate of a total map on $\Nat$
$$\Nat t, \Nat u \vdash \Tot(t,u)$$
We use cuts but no logical connective.
In this subsubsection we use no $\Ack L$-rule, but we use as a lemma the fact that $\Ack$ is
a partial map on $\Nat$ from the previous subsubsection, and this proof used $\Ack L$.
...
Let $t,u$ be any terms of the language $0,S$. 
There is a a connective-free cyclic proof with cuts of $\Nat t, \Nat u \vdash \Tot(t,u)$ in the same language, consisting of four finite trees all having $3$ open assumptions: (1), (2), (3).
...
with $x \not \in \FV(t,u)$. 
$\Nat t$ {\color{red}{\bf progresses to}} $\Nat x$ in the second premise.
...
\begin{itemize}
\item
The base case for induction on $\Nat u$ has \emph{open assumption (1)}: 
$\Nat x, \Nat 1 \vdash \Tot(x,1)$.
\item
The inductive case for induction on $\Nat u$ has \emph{open assumptions (2)}:
$\Nat t, \Nat y \vdash \Tot(t,y)$
and \emph{open assumptions (3)}:
...
$\boxed{\Tot(x,1)} \vdash \Tot(Sx,0)$. Cut formulas are framed in a box.
\begin{enumerate}
\item
...
is proved by an $\eq$-rule and permutation of $\Nat y$, $\Nat t$, with premise
$$t=Sx, \Nat x, \Nat t, \Nat y \vdash \Tot(Sx,Sy)$$
  We prove this latter by a cut on $\Tot(Sx,y)$ with premise (i):
$t=Sx, \Nat x, \Nat t, \Nat y \vdash \boxed{\Tot(Sx,y)}$, and with premise (ii):
$\Nat x, \Nat y,  \boxed{\Tot(Sx,y)} \vdash \Tot(Sx,Sy)$.
\begin{itemize}
\item
The premise (i): $t=Sx, \Nat x, \Nat t, \Nat y \vdash \Tot(Sx,y)$ of the cut 
is derived by $\eq\Weak$-rule from 
...
\boxed{
$
\infer{t=Sx,u=Sy, {\redStat{\Nat t}},{\blueProg{\Nat y}}, \redStat{\Nat x} \vdash \Tot(Sx,u)}
        {\infer[\tt cut]
           {t=Sx, {\redStat{\Nat t}}, {\blueStat{\Nat y}}, \Nat Sx, \redStat{\Nat x} \vdash \Tot(Sx,Sy)}
           {\firstPremiseCut
                               &
              \secondPremiseCut
            }
...
In $\Pi_0$ we proved  $\Nat t, \Nat u \vdash \Tot(t,u)$ from $3$ open assumptions: $\Nat x, \Nat 1 \vdash \Tot(x,1)$ and $\Nat t, \Nat y \vdash \Tot(t,y)$ and $\Nat x, \Nat z \vdash \Tot(x,z)$ for any variables $x,y,z \not \in \FV(t,u)$. We may prove these latter from $\Nat x', \Nat 1 \vdash \Tot(x',1)$ and $\Nat t, \Nat y' \vdash \Tot(t,y')$ and $\Nat x', \Nat z' \vdash \Tot(x',z')$ for any variables $x',y',z' \not \in \FV(t,u,x,y,z)$. By renaming we obtain $\Nat x, \Nat 1 \vdash \Tot(x,1)$ and $\Nat t, \Nat y \vdash \Tot(t,y)$ and $\Nat x, \Nat z \vdash \Tot(x,z)$ because we have $y' \not \in \FV(t)$. 
Thus, we may define a regular proof $\Pi$ by unfolding $\Pi_0$.
