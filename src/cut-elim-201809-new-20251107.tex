...
$3$ open assumptions. We color the progressing points of traces from $\Ack(t,u,v)$
...
As we said, $\Pi_0$ has $3$ open assumptions, 
...
Let $t,u$ be any terms of the language $0,S$. 
There is a a connective-free cyclic proof with cuts of $\Nat t, \Nat u \vdash \Tot(t,u)$ in the same language, consisting of four finite trees all having $3$ open assumptions.
...
\infer{{\redStat{\Nat t}}, {\blueStat{\Nat u}} \vdash \Tot(t,u)}
         {\deduce{\vdots}{{\redStat{\Nat x}}, \Nat 1 \vdash \Tot(x,1)}
         &
           \deduce{\vdots}{{\redStat{\Nat t}}, 
                                       {\blueStat{\Nat y}} \vdash \Tot(t,y)}
         &
           \deduce{\vdots}{{\redStat{\Nat x}}, \Nat z \vdash \Tot(x,z)}
         }
$ 
\end{center}
...
with $x \not \in \FV(t,u)$. $\Nat t$ progresses to $\Nat x$ in the second premise.
...
\begin{center}
$
\infer{t=0, \Nat u \vdash \Tot(t,u)}
         {\deduce{\Nat u \vdash \Tot(0,u)}
...
from $\Nat x, \Nat u, \Nat t \vdash \Tot(Sx,u)$.
We prove this latter by induction on $u$ such that $\Nat u$, with first premise
$\Nat x, u=0, \Nat u, \Nat t \vdash \Tot(Sx,u)$ and second premise 
$\Nat x, u=Sy, \Nat y, \Nat t \vdash \Tot(Sx,u)$ with $y \not \in \FV(t,u,x)$.
$\Nat u$ progresses to $\Nat y$ in the second premise.

\begin{itemize}
\item
The first premise of induction has \emph{1 open assumption $\Nat x, \Nat 1 \vdash \Tot(x,1)$}.
\item
The second premise of induction has \emph{2 open assumptions $\Nat t, \Nat y \vdash \Tot(t,y)$
and $\Nat x, \Nat z \vdash \Tot(x,z)$}, for some variable $z \not = x$.
\end{itemize}

\begin{center}
$
\infer{t=Sx, {\redProg{\Nat x}}, {\blueStat{\Nat u}}, 
                                {\redStat{\Nat t}} \vdash \Tot(t,u)}
         {\infer*{{\redStat{\Nat x}}, u=0, \Nat u, \Nat t \vdash \Tot(Sx,u)}
                      {\deduce{{\redStat{\Nat x}}, \Nat 1 \vdash \Tot(x,1)}{(1)}}
         &
          \infer{\Nat x, u=Sy, {\blueProg{\Nat y}}, 
                    {\redStat{\Nat t}} \vdash \Tot(Sx,u)}
                   {\deduce{\vdots}{\deduce{{\redStat{\Nat t}}, 
                                                                 {\blueStat{\Nat y}} \vdash \Tot(t,y)}{(2)}}
                      &
...
The first premise $\Nat x, u=0, \Nat u, \Nat t \vdash \Tot(Sx,u)$ 
is proved by $\eq$-rule + weakening with premise
$\Nat x \vdash \Tot(Sx,0)$. We prove this latter by a cut between 
$\Nat x \vdash \Tot(x,1)$ and 
$\Tot(x,1) \vdash \Tot(Sx,0)$.

The first premise $\Nat x \vdash \Tot(x,1)$ of the cut 
is proved by a cut between $\vdash \Nat 1$ and
$\Nat x, \Nat 1 \vdash \Tot(x,1) $. This latter is open assumption (1).
...
\infer{{\redStat{\Nat x}}, u=0, \Nat u, \Nat t \vdash \Tot(Sx,u)}
         {\infer[\tt cut]{{\redStat{\Nat x}} \vdash \Tot(Sx,0)}
                                {\infer{{\redStat{\Nat x}} \vdash \Tot(x,1)}
                                           {\vdash \Nat 1
                                           &
                                ...
                                &
                                \deduce{\Nat x, \Nat y, \Tot(Sx,y) \vdash \Tot(Sx,Sy)}
                                             {Lemma \ \ref{lemma-ackermann-base}.
                                                               \ref{lemma-ackermann-base-4}
                                              }
                                }
         }
$
...
The second premise $\Nat x, u=Sy, \Nat y, \Nat t \vdash \Tot(Sx,u)$
is proved by an $\eq$-rule+weakening with premise
$t=Sx,\Nat x, \Nat y, \Nat t \vdash \Tot(Sx,Sy)$.  We prove this latter by a cut between 
$t=Sx, \Nat x, \Nat y, \Nat t \vdash \Tot(Sx,y)$, and 
$\Nat x, \Nat y, \Tot(Sx,y) \vdash \Tot(Sx,Sy)$.

The first premise $t=Sx, \Nat x, \Nat t, \Nat y\vdash \Tot(Sx,y)$ of the cut 
is derived by $\eq$-rule+weakening from 
$\Nat t, \Nat y \vdash \Tot(t,y)$, which is open assumption (2).
...
for some $z \not \in \FV(t,u,x,y)$. 
We prove this latter by a cut between $\Nat x, \Nat z \vdash \Tot(x,z)$,
which is open assumption (3), and $\Ack(Sx,y,z), \Tot(x,z) \vdash \Tot(Sx,y)$.
...
\infer{\Nat x, u=Sy, {\blueProg{\Nat y}}, 
                                {\redStat{\Nat t}} \vdash \Tot(Sx,u)}
        {\infer[\tt cut]{t=Sx,\Nat x, {\redStat{\Nat y}}, 
                               {\redStat{\Nat t}} \vdash \Tot(Sx,Sy)}
                               {\infer{t=Sx, \Nat x, {\blueStat{\Nat y}}, 
                                            {\redStat{\Nat t}} \vdash \Tot(Sx,y)}
                                          {\deduce{{\redStat{\Nat t}}, 
                                                            {\blueStat{\Nat y}} \vdash \Tot(t,y)}
                                                         {(2)}
                                          }
                               &
                                \infer[\Ack L]{{\redStat{\Nat x}}, \Nat y, \Tot(Sx,y) 
                                                         \vdash \Tot(Sx,Sy)}
                                                       {\infer[\tt cut]{{\redStat{\Nat x}}, 
                                                                                \Nat y, \Ack(Sx,y,z) \vdash \Tot(Sx,y)}
                                                                     {\deduce{{\redStat{\Nat x}}, 
                                                                                       \Nat z \vdash \Tot(x,z)}
                                                                                    {(3)}
                                                                               &
                                                                        \deduce{\Ack(Sx,y,z), \Tot(x,z) \vdash \Tot(Sx,y)}
                                                                                     {Lemma \ \ref{lemma-ackermann-base}.
                                                                                                     \ref{lemma-ackermann-base-5}
                                                                                       }
                                                                      }
                                                       }
                               } 
