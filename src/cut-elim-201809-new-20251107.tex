...
The totality statement $\Tot(x,y)$ requires that the image $z$ of $(x,y)$ is in $\Nat$.
...
By induction on $x$ such that $\Nat x$ we may prove totality of $\Ack$.
...
we have $\Ack(x,z) \reduces_v w$ for some $v$,$w$ such that $\Nat v$, $\Nat w$. 
...
with an \emph{inductive predicate} $\Ack(x,y,z)$. We replace the correctness 
...
\item
In the definition of $\Tot(x,y)$ as $\Ack(x,y,z)$ for some $z$
...
the totality proof. 
In the informal totality proof we used 
that $$\Tot(Sx,y), \forall z( \Nat z \rightarrow \Tot(x,z) ) \vdash \Tot(Sx,Sy)$$
but in LKID we only use the weaker property that 
...
\infer[\Tot L]
 ...
 {\infer[\Tot R]
    ...
    {\infer[\eq]
      {x=x', z=z', \Ack(Sx,y,z), \Ack(x',z',w) \vdash  \Ack(Sx,Sy,z)}
...
$3$ open assumptions (1), (2), (3), listed below. 
...
$\infer*[\Ack L] %having 3 vertical dots instear of an horizontal line
 ...
 {\deduce{\Nat 1, {\redStat{\Ack(x,1,z)}}   \vdash \Nat z} {(1)} & 
   \deduce{\Nat y, {\redStat{\Ack(Sx,y,z)}} \vdash \Nat z} {(2)} &
   \deduce{\Nat z, {\redStat{\Ack(x,z,w)}}  \vdash \Nat w}{(3)}
 }
$
\end{center}
...
%% Premise 1 of \Ack L %%%%%%%
 {
   \infer[\eq]{\Nat u, t\!=\!0, u\!=\!y, v\!=\!Sy \vdash \Nat v} 
...
%% Premise 2 of \Ack L %%%%%%%
 & 
   \infer{\Nat u, t\!=\!x, u\!=\!1, v\!=\!z, {\redProg{\Ack(x,1,z)}} \vdash \Nat v} 
...
%% Premise 3 of \Ack L %%%%%%%
 &
   ...
   {\infer[\tt cut]
     ...
     {
        \deduce{\Nat y, {\redStat{\Ack(Sx,y,z)}} \vdash \boxed{\Nat z}} {(2)} 
     &
       \deduce{\boxed{\Nat z}, {\redStat{\Ack(x,z,w)}}  \vdash \Nat w}{(3)}
     }
   }
%% End Premise 3 %%%%%%%%%
 }
...
$\Ack$ is the predicate of a total map on $\Nat$, using cuts but no logical connective.
In this subsubsection we sue no $\Ack L$-rule, but we use as a lemma the fact that $\Ack$ is
...
\begin{lemma}[Ackermann is a total binary function on $\Nat$]
...
We color the progressing traces from $\Nat t$
in {\bf \color{red}red} and the stationary traces from $\Nat t$
in {\bf \color{darkred} dark red}. 
We color the progressing traces from $\Nat u$
in {\bf \color{blue}blue} and the stationary traces from $\Nat t$
...
\begin{proof}
We use induction on $t$ such that $\Nat t$. We prove $\Nat t, \Nat u \vdash \Tot(t,u)$
from two premises:
...
We prove $t=0, \Nat u \vdash \Tot(t,u)$ from $\eq+\Weak$-rule
...
\begin{center}
$
\infer[\eq+\Weak]{t=0, \Nat u \vdash \Tot(t,u)}
...
We prove this latter by induction on $u$ such that $\Nat u$, with first premise
$t=Sx, \Nat x, u=0, \Nat t \vdash \Tot(Sx,u)$ and second premise 
$t=Sx, \Nat x, u=Sy, \Nat y, \Nat t \vdash \Tot(Sx,u)$ with $y \not \in \FV(t,u,x)$.
...
\end{itemize}
This is Inductive case for $\Nat t$:

\begin{center}
$
\infer{t=Sx, {\redProg{\Nat x}}, {\blueStat{\Nat u}}, {\redStat{\Nat t}} \vdash \Tot(t,u)}
         {\infer{t=Sx, {\redStat{\Nat x}}, u=0, \Nat t \vdash \Tot(Sx,u)}
                      {\deduce{\mbox{ Base case for } \Nat u}
                                     {\deduce{{\redStat{\Nat x}}, \Nat 1 \vdash \Tot(x,1)}{(1)}}}
         &
          \infer{t=Sx, \Nat x, u=Sy, {\blueProg{\Nat y}}, 
                    {\redStat{\Nat t}} \vdash \Tot(Sx,u)}
                   {\deduce{\vdots }
                                  {\deduce{{\redStat{\Nat t}}, {\blueStat{\Nat y}} \vdash \Tot(t,y)}
                                                {(2)}
                                  }
                      &
                    \deduce{\vdots }{\deduce{{\redStat{\Nat x}}, 
                                                                \Nat z \vdash \Tot(x,z)}{(3)}}
                    }
         }
$
\end{center}
...
$t=Sx,\Nat x, \Nat y, \Nat t \vdash \Tot(Sx,Sy)$.  We prove this latter by a cut between 
$t=Sx, \Nat x, \Nat y, \Nat t \vdash \boxed{\Tot(Sx,y)}$, and 
$\Nat x, \Nat y,  \boxed{\Tot(Sx,y)} \vdash \Tot(Sx,Sy)$.
\begin{itemize}
\item
The first premise $t=Sx, \Nat x, \Nat t, \Nat y\vdash \Tot(Sx,y)$ of the cut 
is derived by $\eq+\Weak$-rule from 
$\Nat t, \Nat y \vdash \Tot(t,y)$, which is \emph{open assumption (2)}.
\item
The second premise $\Nat x, \Nat y, \Tot(Sx,y) \vdash \Tot(Sx,Sy)$ of the cut 
is derived by $\Ack L$ with premise $\Nat x, \Nat y, \Ack(Sx,y,z) \vdash \Tot(Sx,y)$, 
for some $z \not \in \FV(t,u,x,y)$. 
We prove this latter by a cut between $\Nat x, \Nat z \vdash \boxed{\Tot(x,z)}$,
which is \emph{open assumption (3)}, and $\Ack(Sx,y,z), \boxed{\Tot(x,z)} \vdash \Tot(Sx,y)$.
This premise has a  finite \emph{cut-free and connective-free} proof
by Lemma \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-5}.
\end{itemize}
...
\newcommand{\firstPremiseCut}    
{{\infer
           {t=Sx,{\redStat{\Nat t}}, {\blueStat{\Nat y}}, \vdash \boxed{\Tot(Sx,y)}}
           {\deduce
...
\newcommand{\secondPremiseCut}
{\infer[\Ack L]
  {\Nat Sx,{\redStat{\Nat x}}, \Nat y, \boxed{\Tot(Sx,y)} \vdash \Tot(Sx,Sy)}
  {\infer
  {\Nat Sx,{\redStat{\Nat x}}, \Nat y, \Ack(Sx,y,z) \vdash \Tot(Sx,Sy)}
  {\infer[\tt cut]
      { \Nat Sx,{\redStat{\Nat x}}, \Nat y, \Ack(Sx,y,z) \vdash \Tot(x,z)}
      {\deduce{\Nat Sx, \Nat y, \Ack(Sx,y,z) \vdash \boxed{\Nat z}}
                     {Lemma \ \ref{lemma-ackermann-partial}: \ \Ack \mbox{ is a partial map } }
       &
        \deduce{{\redStat{\Nat x}}, \boxed{\Nat z} \vdash \Tot(x,z)}
                    {(3)}
       }
     &
      \deduce{\Ack(Sx,y,z), \Tot(x,z) \vdash \Tot(Sx,Sy)}
