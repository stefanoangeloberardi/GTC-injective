...
In this section we define the GTC, for the cyclic proof systems $\CLKIDomega$ and $\CLJIDomega$. $\CLKIDomega$ and $\CLJIDomega$ are both first order logics, hence we will consider a first-order signature $\Sigma$, which contain denumerable many constant, function and predicate symbols. Terms, formulas and $\Sigma$-structures are defined as usually for first order logic. We consider sequents as originally defined by Gentzen using finite lists, intead of the usual formulations given in \cite{Brotherston2,BrotherstonSimpson} using sets and multisets. In the case of $\CLKIDomega$, sequents have the form $\Gamma \vdash \Delta$, while in $\CLJIDomega$, they are defined as $\Gamma \vdash \phi$, where $\Gamma$ and $\Delta$ are finite lists of formulas, and $\phi$ is list containing the single formula $\phi$. 
...
For each inference rule in $\CLKIDomega$, we use the \emph{sub-occurrence} relation, to map occurrences of inductive predicates on the left side of the conclusion sequent, to those on the left side of the premises’ sequents. To simplify the presentation, we define this relation pictographically (inspired by the work in \cite{SaurinBauer}) using colored arrows over the rules, to illustrate how occurrences are tracked through each inference. Each arrow is defined as follows:
