...
\begin{enumerate}[i)]
    ...
    \item An arrow from a list $\Gamma$ in the conclusion to the same list $\Gamma$ in a premise defines a bijection between occurrences $(F,i)$ in the conclusion and $(F,j)$ in the premise.
    \item An arrow from a list $\Gamma[\theta]$ in the conclusion to $\Gamma$ in a premise defines a bijection between occurrences $(F[\theta],i)$ in the conclusion and $(F,j)$ in the premise.
    \item An arrow from a list $\Gamma[t/x,u/y]$ in the conclusion to $\Gamma[u/x,t/y]$ in a premise defines a bijection between occurrences $(F[t/x,u/y],i)$ in the conclusion and $(F[u/x,t/y],j)$ in the premise.
\end{enumerate}
...
\begin{itemize}
     ...
     contains only the ocurrences of $\Gamma$ (resp. $\Gamma'$), of the form $(P\mathbf{u}, i)$, where $P$ is an inductive predicate. We define the bi-partite relation $(S,P)_{v,v'}$ as follows:
        \begin{enumerate}[i)]
            ...
            \item If $r(v)$ is a case split rule, then for all $(Q\mathbf{t},j)$ in $\Theta$, $(P\mathbf{u},i) P (Q\mathbf{t},j)$ if and only if $(P\mathbf{u},i) \rightarrow_{r(v)} (Q_\mathbf{t},j)$ and $(P\mathbf{u},i)$ is the active occurrence in $\Gamma$. Otherwise, if $(P\mathbf{u},i)$ is not the active occurrence, then $(P\mathbf{u},i) S (Q\mathbf{t},j)$.
