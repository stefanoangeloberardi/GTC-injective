...
\section{Injective Bi-partite Relations}
\label{sec:injective}
...
Indeed, by injectivity if $xSyS'z$ then $y \not \in \cod{P} \cup \dom{P'}$ hence $(x,z) \not \in ( SP' \cup PS' \cup PP')$. We say that the set of bi-partite relations $\biRel$ is {\em injective} if all $(S,P) \in \biRel$ is such. By Lemma \ref{lem:biRelInjectiveComp}, if $\biRel$ is injective then $\biRel^*$ is such. If $\Graph = (V, \biRel, E)$ with injective $\biRel$, by abuse of language we say that $\Graph$ is injective. It is now possible to state the main result of this section:
...
\begin{proof}
    We proceed by case analysis on the construction of the bi-partite relations $(S,P)_{v,v'}$ specified in Definition \ref{def:preProofTrans}.
\end{proof}
