...
Given a $\CLKIDomega$ or $\CLJIDomega$ pre-proof $\Pi$, we show how to construct the induced transition graph with bi-partite relations. Our approach follows Ikebuchi's work \cite{Mirai}, where the equivalence between the GTC and SCT is formally stated by extracting the size-change graphs of a cyclic proof. In our case, we extract a transition graph with bi-partite relations of a cyclic proof, which in principle corresponds to the same process, with some simplification in the case of an injective trace relation.
...
\begin{itemize}
     ...
     \item Let $v,v' \in V'$ be two vertices which share an edge, i.e., there exists a $j$ such that $p'_j(v) = v'$. Moreover, let $s(v) = \Gamma \vdash \Delta$ (resp. $s(v') = \Gamma' \vdash \Delta'$) and let $\Psi \subseteq \Gamma$ (resp. $\Theta \subseteq \Gamma'$)
     be the sub-sequences containing only the occurrences of $\Gamma$ (resp. $\Gamma'$), of the form $(P\mathbf{u}, i)$, where $P$ is an inductive predicate. We define the bi-partite relation $(S,P)_{v,v'}$ as follows:
        \begin{enumerate}[i)]
            ...
            \item If $r(v)$ is a case split rule, then for all $(Q\mathbf{t},j)$ in $\Theta$, $(P\mathbf{u},i) P (Q\mathbf{t},j)$ if and only if $(P\mathbf{u},i) \rightarrow_{r(v)} (Q\mathbf{t},j)$ and $(P\mathbf{u},i)$ is the progressing 
            occurrence in $\Gamma$. Otherwise, if $(P\mathbf{u},i)$ is stationary, then $(P\mathbf{u},i) S (Q\mathbf{t},j)$.
...
This process can also be applied to $\CLJIDomega$ pre-proofs, using sequents of the form $\Gamma \vdash \phi$.
...
\begin{proposition}\label{prop:GTC-SCT}
