...
\[
S = \parbox{10cm}{$\{ (i,j) \ |\ $ \ there is some non-progressing trace from
...
\\
P = \{ (i,j) \ |\ \parbox{10cm}{\ there is non-progressing trace from
the $i$-th inductive atomic formula in the bottom sequent of $\pi$
to the $j$-the inductive atomic formula in the top sequent of $\pi$ \}.}
\]
...
to the top sequent of companion number $c_2$.
...
\[
\forall \Vec x((\forall \Vec yfg(\rho(g,\Vec y) <_\Lex \rho(f,\Vec x) \imp A(\Vec y)))
\imp A(\Vec x))
\imp \forall \Vec xA(\Vec x).
\]
By clamin 1,
it implies the induction principle for the path relation
\[
\forall \Vec x((\forall \Vec yfg((g,\Vec y) <_\Pi (f,\Vec x) \imp A(\Vec y)))
\imp A(\Vec x))
\imp \forall \Vec xA(\Vec x).
\]
$\Box$
