...
\label{subsection-ackermann}
From now onw we abbreviate the English statement \emph{``$x$ such that $\Nat x$} with  just
\emph{``$x \in \Nat$''}.
...
Assume that $t$ and $u$ are terms in the language $0,S,\Ack$:
\begin{enumerate}
\item
$\Ack(0,u) \ \ \ \ \ \ \ \ \reduces_1 \ \ \ \ Su$ 
\item
$\Ack(St,0)  \ \ \ \ \ \  \reduces_1 \ \ \ \ \Ack(t,1)$
\item
$\Ack(St,Su) \ \ \ \ \reduces_1 \ \ \ \ \Ack(t,\Ack(St,u))$
\end{enumerate}
We write $\reduces_t$ for reduction $t$ times. Any $z \in \Nat$
and such that $\Ack(x,y) \reduces_t z$ for some $t \in \Nat$ is said an image $z$ of 
the map $\Ack$ in $(x,y)$.

Ackermann is a total map $\Nat^2 \rightarrow 
\Nat$. The standard way of stating totality of $\Ack$ on a pair $(x, y)$ is 
$\Tot(x,y) \equiv \exists z, t. (\Nat z) \wedge (\Nat t) \wedge (\Ack(x,y) \reduces_t z)$. 
The standard totality statement $\Tot(x,y)$ requires that the image $z$ of $(x,y)$ is in $\Nat$ 
and that it can be obtained in a finite number $t$ of steps.
The Ackermann binary function defines a family $\{\Ack(x,\cdot)| x \in \Nat \}$ 
of unary maps. 
\begin{enumerate}
\item
The totality of the map $\Ack(x,\cdot)$ from $\Nat$ to $\Nat$ 
is formulated as the $\Pi^0_2$-predicate 
$T(x) \equiv \forall y( \Nat y \rightarrow \Tot(x,y))$. 
\item
The totality of $\Ack$ 
...
\subsubsection{Totality of $\Ack:\Nat^2 \rightarrow \Nat$ (standard proof)}
\label{subsubsection-ackermann-standard}
By induction on $x \in \Nat$ we may prove totality of the map $\Ack$.

\begin{lemma}[Ackermann is Total standard proof)]
\label{lemma-ackermann-standard}
$\Ack$ is a total map $:\Nat^2 \rightarrow \Nat$. 
\end{lemma}

\begin{proof}
We prove by induction on $x \in \Nat$ the totality of each map
...
$\Ack(Sx,\cdot)$ is total. We prove by induction on $y \in \Nat$ that: 
$\Ack(Sx,y) \reduces_t z$ for some $t \in \Nat $, $z \in \Nat$. 

\begin{enumerate}
\item
{\bf Base Case of the secondary induction: $y=0$}.
We prove that $\Ack(Sx,0) \reduces_t z$ for some $t \in \Nat$, $z \in \Nat$.
By principal induction hypothesis \emph{instanciated on $y=1$} 
we have $\Ack(x,1) \reduces_u z$ for some $u \in \Nat$, $z \in \Nat$, 
...
for some $t \in \Nat$, $z \in \Nat$. 
By principal induction hypothesis we have $\Ack(Sx,y) \reduces_u z$ for some 
$u \in \Nat$, $z \in \Nat$. 
By secondary induction hypothesis \emph{istanciated on $y=z$} 
we have $\Ack(x,z) \reduces_v w$ for some $v \in \Nat$, $w \in \Nat$. 
...
\subsubsection{Ackermann as an inductive predicate in $\LKID$}
\label{subsubsection-ackermann-inductive}
We provide a cyclic proof of the totality of Ackermann function. To this aim,
we first replace the 
statement \emph{``the image of $(x,y)$ by $\Ack$ is $z$''}
with an \emph{inductive predicate} $\Ack(x,y,z)$ in $\LKID$. We replace the totality
statement $\exists z. \Nat z \wedge \Ack(x,y,z)$ of $\Ack$ on $(x,y)$
with an \emph{inductive predicate} $\Tot(x,y)$. We insert two simplifications:

\begin{enumerate}
\item 
In the definition of $\Ack(x,y,z)$
we skip the index $t$ counting the number of reduction steps.
\item
In the definition of $\Tot(x,y)$ as ``$\Ack(x,y,z)$ for some $z \in \Nat$''
we skip the requirement $\Nat z$. The reason is that $\Nat z$ can be proved from 
$\Nat y$ and $\Ack(x,y,z)$ by induction on the definition of $\Ack(x,y,z)$: we do it
in the next subsection.
\end{enumerate}

The inductive rules we choose for $\Ack(x,y,z)$ are the corresponding of 
the reduction rules.  The only inductive rule we choose for $\Tot(x,y)$ 
expresses that $\Tot(x,y) \equiv \exists z \Ack(x,y,z)$. The inductive rules are:

\begin{enumerate}
\item
$\Ack(0,y,Sy)$
\item
if $\Ack(x,1,z)$ then $\Ack(Sx,0,z)$
\item
if $\Ack(Sx,y,z)$ and $\Ack(x,z,t)$ then $\Ack(Sx,Sy,t)$
\item
if $\Ack(x,y,z)$ then $\Tot(x,y)$.
\end{enumerate}

They generate the following $R$-rules for $\Ack$ and $\Tot$ in $\LKID$.
...
$L$-rules for $\Ack$ and $\Tot$ in $\LKID$
are required to prove that the image of $x \in \Nat$, $y \in \Nat$ by $\Ack$ 
is some $z \in \Nat$ (see later).
$L$-rules are also required to prove that for all $x \in \Nat$, $y \in \Nat$,
the image $z$ of $(x,y)$ is unique, but we do not include this proof here.

The $\Ack L$, $\Tot L$-rules are defined in $\LKID$ as a consequence of 
$\Ack R$, $\Tot R$-rules, using \emph{fresh} variables. 

\begin{definition}[The $L$-rules for $\Ack$ and $\Tot$]
...
has $3$ premises a,b,c, one for each $R$-rule for the same predicate:
\begin{itemize}
\item
Premise (a) is: 
...
\item 
Premise (c) is:  
$t=Sx, u=Sy, v=z, \Ack(Sx,y,w), \Ack(x,z,w),\Gamma \vdash \Delta$
\end{itemize}
Here is the $\Ack L$-rule:
\begin{center}
{\scriptsize          %in order to make the proof-tree smaller
 ...
 $\infer[\Ack L]
  ...
  {   \deduce{t\!=\!0, u\!=\!y, v\!=\!Sy,\Gamma \vdash \Delta}{(a)} & 
    	 \deduce{t\!=\!Sx, u\!=\!0, v\!=\!z, \Ack(x,1,z),\Gamma \vdash \Delta}{(b)} & 
    	 \deduce{t\!=\!Sx, u\!=\!Sy, v\!=\!z, \Ack(Sx,y,w), \Ack(x,z,w),\Gamma \vdash \Delta}{(c)}
  }
 $
 }
}
...
As an immediate consequence we have the following finite proofs, which are lemmas of 
the totality proof. 
In the standard totality proof in \S \ref{subsubsection-ackermann-standard} we used 
that $$\Tot(Sx,y), \forall z( \Nat z \rightarrow \Tot(x,z) ) \vdash \Tot(Sx,Sy)$$
Instead, in LKID we only use an instance $z \in \Nat$ of  
$\forall z( \Nat z \rightarrow \Tot(x,z) )$, namely the fact that: 
$$\Ack(Sx,y,z), \Tot(x,z) \vdash \Tot(Sx,Sy)$$
%19:51 09/11/2025

\begin{lemma}
\label{lemma-ackermann-base}
Assume $x,y,z,w$ are variables.
There are \emph{finite, cut-free and connective-free} proofs of:
\begin{enumerate}
\item
...
$\Ack(Sx,y,z), \Ack(x,z,w) \vdash \Ack(Sx,Sy,w)$
...
\infer[\Ack R]
 {\Ack(x,1,z) \vdash \Ack(Sx,0,z)}
 {\deduce{\Ack(x,1,z) \vdash \Ack(x,1,z)}
               {\id}
 }
$
\end{center}
...
\item 
%\label{lemma-ackermann-base-2}
$\Ack(Sx,y,z), \Ack(x,z,w) \vdash \Ack(Sx,Sy,w)$ is derived by $\Ack R$ with premise
...
\infer[\Ack R]
 {\Ack(Sx,y,z), \Ack(x,z,w) \vdash \Ack(Sx,Sy,w)}
 {
   \deduce{\Ack(Sx,y,z), \Ack(x,z,w) \vdash \Ack(Sx,y,z)}
               {\id}
 &
  \deduce{\Ack(Sx,y,z), \Ack(x,z,w) \vdash  \Ack(x,z,w)}
               {\id}
 }
$
\end{center}
...
\item
%\label{lemma-ackermann-base-3}
$\vdash \Tot(0,y)$ is derived by $\Tot R$-rule with premise $\vdash \Ack(0,y,Sy)$.
...
$x=x', 1=y', \Ack(x',1,z) \vdash \Tot(Sx,0)$ with $x',y',z$ fresh. This latter is derived
by $\Tot R$-rule with premise $x=x', 1=y', \Ack(x',y',z) \vdash \Ack(Sx,0,z)$,
...
\infer[\Tot L]
 {\Tot(x,1) \vdash \Tot(Sx,0)}
 {\infer[\Tot R]
    {x=x',1=y', \Ack(x',y',z) \vdash \Tot(Sx,0)}
    {\infer[\eq]
      {x=x', 1=y',\Ack(x',y',z) \vdash \Ack(Sx,0,z)}
      {\deduce{\Ack(x,1,z) \vdash \Ack(Sx,0,z)}
                     {\mbox{ point \ref{lemma-ackermann-base-1} above }}
      }
    }
 }
$
...
$x=x', z=z', \Ack(Sx,y,z), \Ack(x',z',w) \vdash \Tot(Sx,Sy)$ with $x',z',w$ fresh.
This latter is derived by $\Tot R$-rule with premise $x=x', z=z', \Ack(Sx,y,z), \Ack(x',z',w) \vdash  \Ack(Sx,Sy,z)$,
which is derived by $\eq$-rule with premise $\Ack(Sx,y,z), \Ack(x,z,w) \vdash \Ack(Sx,Sy,w)$.
...
\infer[\Tot L]
 {\Ack(Sx,y,z), \Tot(x,z) \vdash \Tot(Sx,Sy)}
 {\infer[\Tot R]
    {x=x', z=z', \Ack(Sx,y,z), \Ack(x',z',w) \vdash \Tot(Sx,Sy)}
    {\infer[\eq]
      {x=x', z=z', \Ack(Sx,y,z), \Ack(x',z',w) \vdash  \Ack(Sx,Sy,w)}
      {\deduce{\Ack(Sx,y,z), \Ack(x,z,w) \vdash \Ack(Sx,Sy,w)}
                     {\mbox{ point \ref{lemma-ackermann-base-2} above }}
      }
    }
 }
$
...
(hence quantifier-free) proof with cuts of the totality of $\Ack$. 
We prove first that all $\Ack(t,\cdot)$ 
are a \emph{partial} map $:\Nat \rightarrow \Nat$: that is, if $\Nat t, \Nat u, \Ack(t,u,v)$ then
$\Nat v$. In fact we prove a little more, skipping the assumption $\Nat t$: we prove that 
if $\Nat u, \Ack(t,u,v)$ then $\Nat v$. The use of $\Ack L$-rule, namely of induction on
the definition of $\Ack(t,u,v)$ is essential for the proof.
...
{\bf with cuts} of $$\Nat u, \Ack(t,u,v) \vdash \Nat v$$
\end{lemma}
...
\newcommand{\redStat}   [1]  { {\color{darkred}\mathbf {#1}} }
\newcommand{\redProg}  [1]  { {\color{red}\mathbf {#1}} }

\newcommand{\blueStat} [1]  { {\color{darkblue}\mathbf {#1}} }
\newcommand{\blueProg}[1]  { {\color{blue}\mathbf {#1}} }
...
$3$ open assumptions (1), (2), (3), as listed below. 
We color \emph{some} traces from $\Ack(t,u,v)$, with the progressing points of 
in {\bf \color{red}red} and the stationary points in {\bf \color{darkred}dark red}.
We put a box around cut formulas: our only cut formula is $\Nat z$. 

\begin{center}
$\infer*[\Ack L] %having 3 vertical dots instear of an horizontal line
 {\Nat u, {\redStat{\Ack(t,u,v)}} \vdash \Nat v}
 {\deduce{\Nat 1, {\redStat{\Ack(x,1,z)}}   \vdash \Nat z} {(open \ assumption \ 1)} & 
   \deduce{\Nat y, {\redStat{\Ack(Sx,y,z)}} \vdash \Nat z} {(open \ assumption \ 2)} &
   \deduce{\Nat z, {\redStat{\Ack(x,z,w)}}  \vdash \Nat w}{(open \ assumption \ 3)}
 }
$
\end{center}

We define the $3$ premises of the rule $\Ack L$. Let $x,y,z,w$ be \emph{fresh}
variables. %As we said, we frame the cut formula $\Nat z$ in a box.

\newcommand{\firstPremiseAckL}
{{ 
   \infer[\eq]
              {\Nat u, t\!=\!0, u\!=\!y, v\!=\!Sy \vdash \Nat v} 
              {\infer[\Nat R]{\Nat y \vdash \Nat Sy}
                         {\deduce{\Nat y \vdash \Nat y}
                                        {\id}
                         } 
              }
}}

\newcommand{\secondPremiseAckL}
{{
   \infer[\eq]
            {\Nat u, t\!=\!x, u\!=\!1, v\!=\!z, {\redProg{\Ack(x,1,z)}} \vdash \Nat v} 
            {\deduce{\Nat 1, {\redStat{\Ack(x,1,z)}} \vdash \Nat z}
                          {(open \ assumption \ 1)}
             }
}}

\newcommand{\thirdPremiseAckL}
{{
\infer[\eq]
   {\Nat u, t\!=\!Sx, u\!=\!y, v\!=\!w, 
                  {\redProg{\Ack(Sx,y,z)}}, 
                  {\redProg{\Ack(x,z,w)}} \vdash \Nat v}
   {\infer[\tt cut]
     {\Nat y,  {\redStat{\Ack(Sx,y,z)}}, 
                   {\redStat{\Ack(x,z,w)}} \vdash \Nat w}
     {
        \deduce{\Nat y, {\redStat{\Ack(Sx,y,z)}} 
                       \vdash 
                       \boxed{ \color{darkgreen}\mathbf{\Nat z} }} 
                    {(open \ assumption \ 2)} 
     &
       \deduce{\boxed{ \color{darkgreen}\mathbf{\Nat z}}, {\redStat{\Ack(x,z,w)}}  
                     \vdash 
                     \Nat w}
                   {(open \ assumption \ 3)}
     }
...
It is proved by $\eq$ with premise $\Nat y \vdash \Nat Sy$, 
...
It is proved by $\eq$ with premise $\Nat 1, \Ack(x,1,z) \vdash \Nat z$. This is the 
\emph{open assumption $1$}.

\item
\emph{Premise $3$} is 
$\mathbf{\Nat u, t=Sx, u=y, v=w, \Ack(Sx,y,z), \Ack(x,z,w) \vdash \Nat v}$. 
It is proved by $\eq$ with premise $\Nat y,  \Ack(Sx,y,z), \Ack(x,z,w) \vdash \Nat w$. 
This latter is proved by a cut with two premises $\Nat y,  \Ack(Sx,y,z) \vdash \boxed{\Nat z}$ 
and $\boxed{\Nat z}, \Ack(x,z,w) \vdash \Nat w$. These  two premises are 
\emph{open assumption $2$} and \emph{open assumption $3$}
\end{enumerate}

We define a regular infinitary proof tree $\Pi$ \emph{with cuts} out of $\Pi_0$. 

We can repeat the same operations we did on $\Nat u, \Ack(t,u,v) \vdash \Nat v$ 
on each of the $3$ open assumption, which are:
...
From each open assumption we obtain $3$ new open assumptions $\Nat 1, \Ack(x',1,z') \vdash 
\Nat z$ and $\Nat y', \Ack(Sx',y',z') \vdash \Nat z'$ and $\Nat z', \Ack(x',z',w') \vdash \Nat w'$, 
for some $x', y', z', w' \not \in \FV(x,y,z,w,t,u,v)$. We rename and we re-obtain the same 
$3$ open assumptions: $\Nat 1, \Ack(x,1,z) \vdash \Nat z$ and $\Nat y, \Ack(Sx,y,z) \vdash \Nat 
z$ and $\Nat z, \Ack(x,z,w) \vdash \Nat w$.
Renaming is possible because $x, y, z, w\not \in \FV(t,u,v,x',y',z',w')$.  
We defined 3 finite proofs $\Pi_1, \Pi_2, \Pi_3$
with conclusion and assumptions the 3 open assumptions.
With $\Pi_0$ and $\Pi_1, \Pi_2, \Pi_3$ we defines 
a regular proof tree $\Pi$ for $\Nat u, \Ack(t,u,v) \vdash \Nat v$.
\\

In order to have a cyclic proof we have to prove the global trace condition for $\Pi$. 
In $\Pi$ whenever we prove some
sequent of the form $\Nat u', \Ack(t',u',v') \vdash \Nat v'$ we derive it from $3$ sequents of the 
form $\Nat u'', \Ack(t'',u'',v'') \vdash \Nat v'$, and in each case there is a \emph{progressing} 
trace from $\Ack(t',u',v') $ in the bottom sequent to $\Ack(t'',u'',v'') $ in the top sequent. 
Every infinite path of $\Pi$ has to pass infinitely many times from  
$\Nat u', \Ack(t',u',v') \vdash \Nat v'$ to some sequent 
$\Nat u'', \Ack(t'',u'',v'') \vdash \Nat v'$, therefore it includes an infinitely progressing trace 
starting from the unique atom $\Ack(t,u,v) $ in the root and passing through
$\Ack(t',u',v') , \Ack(t'',u'',v''), \ldots$
$\Box$ \end{proof}


\subsubsection{Ackermann is a total binary function on $\Nat$}
From the fact that $\Ack$ is the predicate representing a partial map on $\Nat$ we derive that 
$\Ack$ is the predicate of a total map on $\Nat$, namely the sequent:
$$\Nat t, \Nat u \vdash \Tot(t,u)$$
We use cuts but no logical connective.
In this subsubsection we use no $\Ack L$-rule, 
but we use Lemma \ref{lemma-ackermann-partial}, the fact that $\Ack$ is
a partial map on $\Nat$, and the proof of this Lemma used $\Ack L$.
We use the following notations:

\begin{enumerate}
\item
We write $\eq$ for the equality rule and $\Weak$ for the weakening rule.
\item
We color some progressing traces from $\Nat t$
in {\bf \color{red}red} and some stationary traces from $\Nat t$
in {\bf \color{darkred} dark red}. 
\item
We color some progressing traces from $\Nat u$
in {\bf \color{blue}blue} and some stationary traces from $\Nat t$
in {\bf \color{darkblue} dark blue}. 
\item
We put a box around cut formulas and we draw each pair of cut formulas in the same color.
\end{enumerate}

We first define a finite proof $\Pi_0$ with open assumptions. Then we will
explain how to remove the open assumptions and to obtain a cyclic proof.

\begin{lemma}[A totality proof for Ackermann with open assumption]
\label{lemma-ackermann-total}
Let $t,u$ be any terms of the language $0,S$. 
There is a a connective-free cyclic proof $\Pi_0$ with cuts of $\Nat t, \Nat u \vdash \Tot(t,u)$ in 
the same language, consisting of four finite trees all having $3$ open assumptions: (1), (2), (3).


\begin{center}
\boxed{
$
\deduce{{\redStat{\Nat t}}, {\blueStat{\Nat u}} \vdash \Tot(t,u)}
         {
           \deduce{\vdots}
                       {\deduce{{\redStat{\Nat x}}, \Nat 1 \vdash \Tot(x,1)}
                                     {(open \ assumption \ 1)}
                       }
         &
           \deduce{\vdots}
                       {\deduce{{\redStat{\Nat t}}, {\blueStat{\Nat y}} \vdash \Tot(t,y)}
                                     {(open \ assumption \ 2)}
                        }
         &
           \deduce{\vdots}
                       {\deduce{{\redStat{\Nat x}}, \Nat z \vdash \Tot(x,z)}
                                      {(open \ assumption \ 3)}
                        }
         }
$ 
...
\begin{proof}
We use induction on $t \in \Nat$. We prove $\Nat t, \Nat u \vdash \Tot(t,u)$
from two premises:
$t=0, \Nat u \vdash \Tot(t,u)$ and $t=Sx, \Nat x, \Nat u, \Nat t \vdash \Tot(t,u)$
with $x \not \in \FV(t,u)$. 
$\Nat t$ {\color{red}{\bf progresses to}} $\Nat x$ in the second premise, while there is no
progress from $\Nat u$ to $\Nat u$.

\begin{center}
$
 \infer{{\redStat{\Nat t}}, {\blueStat{\Nat u}} \vdash \Tot(t,u)}
          {t=0, \Nat u \vdash \Tot(t,u)
          &
           t=Sx, {\redProg{\Nat x}}, {\blueStat{\Nat u}}, 
           {\redStat{\Nat t}} \vdash \Tot(t,u)
           }
$
\end{center}

Remark that in the second premise the trace of $\Nat t$
in the conclusion forks into $\Nat x$ (progressing) and $\Nat t$ (stationary). 
This can be done while using injective traces only.


\begin{enumerate}
\item
{\bf Base case for $\Nat t$: $t=0$}. 
We prove $t=0, \Nat u \vdash \Tot(t,u)$ from $\eq$-rule
with premise $\Nat u \vdash \Tot(0,u)$. This premise 
has a finite \emph{cut-free and connective-free} proof
by Lemma \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-3}.
This is Base case for $\Nat t$:

\begin{center}
\boxed{
$
\infer[\eq]{t=0, \Nat u \vdash \Tot(t,u)}
         {\deduce{\vdash \Tot(0,u)}
                       {Lemma \ \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-3}}
         }
$
}
\\
...
\item
{\bf Inductive case for $\Nat t$: $t=Sx$}.
We prove $t=Sx, \Nat x, \Nat u, \Nat t \vdash \Tot(t,u)$ by $\eq$-rule
from $$t=Sx, \Nat x, \Nat u, \Nat t \vdash \Tot(Sx,u)$$
We prove this latter by induction on $u \in \Nat$, with:
\begin{itemize}
\item
first premise
$$t=Sx, \Nat x, u=0, \Nat t \vdash \Tot(Sx,u)$$ 
\item
second premise with some $y \not \in \FV(t,u,x)$
$$t=Sx, \Nat x, u=Sy, \Nat y, \Nat t \vdash \Tot(Sx,u)$$
\end{itemize}
$\Nat u$ {\bf \color{blue}{progresses to}} $\Nat y$ in the second premise.

\begin{itemize}
\item
We prove the base case for induction on $\Nat u$ with \emph{open assumption (1)}: 
$\Nat x, \Nat 1 \vdash \Tot(x,1)$.
\item
We prove the inductive case for induction on $\Nat u$ with \emph{open assumptions (2)}:
$\Nat t, \Nat y \vdash \Tot(t,y)$
and with \emph{open assumptions (3)}:
$\Nat x, \Nat z \vdash \Tot(x,z)$, for some variable $z \not = x$.
\end{itemize}
Therefore this is the Inductive case for $\Nat t$:

\begin{center}
\boxed{
$
\infer{t=Sx, {\redProg{\Nat x}}, {\blueStat{\Nat u}}, {\redStat{\Nat t}} \vdash \Tot(t,u)}
         {\deduce{t=Sx, {\redStat{\Nat x}}, u=0, \Nat t \vdash \Tot(Sx,u)}
                      {\deduce{\mbox{ Base case for } \Nat u}
                                     {\deduce{{\redStat{\Nat x}}, \Nat 1 \vdash \Tot(x,1)}
                                                   {(open \ assumption \ 1)}}
                                     }
         &
          \deduce{t=Sx, \Nat x, u=Sy, {\blueProg{\Nat y}}, {\redStat{\Nat t}} \vdash \Tot(Sx,u)}
                   {\deduce
                     { \mbox{ Inductive case for $\Nat u$} }
                     {\deduce{\vdots }
                                  {\deduce{{\redStat{\Nat t}}, {\blueStat{\Nat y}} \vdash \Tot(t,y)}
                                                {(open \ assumption \ 2)}
                                  }
...
\begin{enumerate}
\item
{\bf Base case for $\Nat u$: $u=0$}. 
The first premise $t=Sx, \Nat x, u=0, \Nat t \vdash \Tot(Sx,u)$ 
is proved by $\eq\Weak$-rule with premise
$\Nat x \vdash \Tot(Sx,0)$. We prove this latter by a cut with premise (i): 
$\Nat x \vdash \boxed{\Tot(x,1)}$, and with premise (ii):  
$\boxed{\Tot(x,1)} \vdash \Tot(Sx,0)$. We recall that formulas are framed in a box.
\begin{enumerate}
\item
The premise (i):  $\Nat x \vdash \Tot(x,1)$ of the cut 
is proved by a cut between $\vdash \boxed{\Nat 1}$ and
$\Nat x, \boxed{\Nat 1} \vdash \Tot(x,1) $. This latter is \emph{open assumption (1)}.
\item
The premise (ii):  $\Tot(x,1) \vdash \Tot(Sx,0)$ of the cut
has a finite \emph{cut-free and connective-free} proof
by Lemma \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-4}.
\end{enumerate}
Summing up, this is  Base case for $\Nat u$.

\newcommand{\firstPremiseCut}
{{\infer[\tt cut]
           {{\redStat{\Nat x}} 
               \vdash 
              \boxed{\color{darkgreen}\mathbf{\Tot(x,1)}}}
           {\infer[\Nat R]
                     {\vdash \boxed{\color{darkcyan}\mathbf{\Nat 1}}}
                     {\infer[\Nat R]{\vdash \Nat 0}{}
                     }
              &
             \deduce{{\redStat{\Nat x}}, \boxed{\color{darkcyan}\mathbf{\Nat 1}} 
                            \vdash 
                            \Tot(x,1)
                          }
                         {(1)}
...
\newcommand{\secondPremiseCut}
{{\deduce{\boxed{\color{darkgreen}\mathbf{\Tot(x,1)}} 
                  \vdash 
                  \Tot(Sx,0)}
                {Lemma \ \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-4}
                }
}}
...
$
\infer[\tt\eq\Weak]
         {t=Sx, {\redStat{\Nat x}}, u=0, \Nat t \vdash \Tot(Sx,u)}
         {\infer[\tt cut]{{\redStat{\Nat x}} \vdash \Tot(Sx,0)}
                                {\firstPremiseCut
                                &
                                 \secondPremiseCut
                                }
...
\item
{\bf Inductive case for $\Nat u$: from $y$ to $Sy$}.
The second premise $t=Sx, \Nat x, u=Sy, \Nat y, \Nat t \vdash \Tot(Sx,u)$
of the induction on $\Nat u$
is proved by an $\eq$-rule with premise
$$t=Sx, \Nat x, \Nat y, \Nat t \vdash \Tot(Sx,Sy)$$
  We prove this latter by a cut on $\Tot(Sx,y)$ with premise (i):
$t=Sx, \Nat x, \Nat y, \Nat t \vdash \boxed{\Tot(Sx,y)}$, and with premise (ii):
$\Nat x, \Nat y,  \boxed{\Tot(Sx,y)} \vdash \Tot(Sx,Sy)$.
\begin{itemize}
\item
The premise (i): $t=Sx, \Nat x, \Nat y, \Nat t \vdash \Tot(Sx,y)$ of the cut 
is derived by $\eq$-rule and permutation for $\Nat t$, $\Nat y$ from 
$\Nat t, \Nat y \vdash \Tot(t,y)$, which is \emph{open assumption (2)}.
\item
The premise (ii): $\Nat x, \Nat y, \Tot(Sx,y) \vdash \Tot(Sx,Sy)$ of the cut 
is derived by $\Ack L+\eq$ with premise for some $z \not \in \FV(t,u,x,y)$:
$$ \Nat x, \Nat y, \Ack(Sx,y,z) \vdash \Tot(Sx,Sy)$$ 
This latter is derived by a cut on $\Tot(x,z)$ with 
\begin{itemize}
\item
premise (ii.a):
$ \Nat x, \Nat y, \Ack(Sx,y,z) \vdash \boxed{\Tot(x,z)}$
\item
premise (ii.b): $\Ack(Sx,y,z), \boxed{\Tot(x,z)} \vdash \Tot(Sx,y)$, which is 
Lemma \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-5}.
\end{itemize}
We derive the premise (ii.a) above
by a cut  on $\Nat z$ between $ \Nat y, \Ack(Sx,y,z) \vdash \boxed{\Nat z}$,
which is Lemma \ref{lemma-ackermann-partial},
and $\Nat x, \boxed{\Nat z} \vdash \Tot(x,z)$, which is 
\emph{open assumption (3)}.
\end{itemize}
...
\newcommand{\firstPremiseCut}    
{{\infer[\eq\Weak]
           {t=Sx, {\blueStat{\Nat y}},{\redStat{\Nat t}}
              \vdash 
              \boxed{\color{darkgreen}\mathbf{\Tot(Sx,y)}}
            }
           {\deduce
                 {{\redStat{\Nat t}}, {\blueStat{\Nat y}} \vdash \Tot(t,y)}
                 {(open \ assumption \ 2)}
            }
}}
...
\newcommand{\secondPremiseCut}
{\infer[\Ack L+\eq]
  {{\redStat{\Nat x}}, \Nat y, \boxed{\color{darkgreen}\mathbf{\Tot(Sx,y)}} 
     \vdash \Tot(Sx,Sy)
  }
  {\infer
  {{\redStat{\Nat x}}, \Nat y, \Ack(Sx,y,z) \vdash \Tot(Sx,Sy)}
  {\infer[\tt cut]
      { {\redStat{\Nat x}}, \Nat y, \Ack(Sx,y,z) 
         \vdash 
         \boxed{\color{darkyellow}\mathbf{\Tot(x,z)}}
      }
      {\deduce{ \Nat y, \Ack(Sx,y,z) 
                        \vdash 
                        \boxed{\color{darkcyan}\mathbf{\Nat z}}
                     }
                     {Lemma \ \ref{lemma-ackermann-partial}: \ \Ack \mbox{ partial map } }
       &
        \deduce{{\redStat{\Nat x}}, \boxed{\color{darkcyan}\mathbf{\Nat z}} 
                       \vdash 
                       \Tot(x,z)
                     }
                    {(open \ assumption \ 3)}
     ...
     &
      \deduce{\Ack(Sx,y,z), \boxed{\color{darkyellow}\mathbf{\Tot(x,z)}} 
                    \vdash 
                    \Tot(Sx,Sy)
                    }
                  {Lemma \ \ref{lemma-ackermann-base}.\ref{lemma-ackermann-base-5}
...
\boxed{
$
\infer{t=Sx, \redStat{\Nat x}, u=Sy,{\blueProg{\Nat y}}, {\redStat{\Nat t}} \vdash \Tot(Sx,u)}
        {\infer[\tt cut]
           {t=Sx, \redStat{\Nat x}, {\blueStat{\Nat y}}, {\redStat{\Nat t}} \vdash \Tot(Sx,Sy)}
           {\firstPremiseCut
                               &
              \secondPremiseCut
            }
...
%%%%%%%%%%%%%%%%%%%%%%
%% THIS WAS THE ORIGINAL 2018 PROOF
%%%%%%%%%%%%%%%%%%%%%%
%\begin{proof}
%
...
%\begin{enumerate}
%\item
%We prove $\Nat t, u=0 \vdash \Tot(t,u)$ by $\eq$ with premise $\Nat t \vdash \Tot(t,0)$, then by $\Nat L$-rule with assumptions: $t=0 \vdash \Tot(t,0)$ and $t=Sx, \Nat x \vdash \Tot(t,0)$. 
%
%\begin{enumerate}
%\item
%Proof of $t=0 \vdash \Tot(t,0)$. We use $\eq$ with premise $\vdash \Tot(0,0)$, 
%then $\Tot R$-rule and $\Ack(0,0,1)$, which is an $\Ack R$-axiom. 
%\item
%We prove $t=Sx, \Nat x \vdash \Tot(t,0)$ by $\eq$-rule with premise 
...
%$\Tot(x,1) \vdash \Tot(Sx,0)$ is proved by $\Tot L$-rule from $x' = x, y'=1, \Ack(x',y',z) \vdash \Tot(Sx,0)$, then by $\eq$ with assmption $\Ack(x,1,z) \vdash \Tot(Sx,0)$, by $\Tot R$-rule from $\Ack(x,1,z) \vdash \Ack(Sx,0,z)$, by $\Ack R$-rule from $\Ack(x,1,z) \vdash \Ack(x,1,z)$, eventually by $\id$.
%\end{enumerate}
%
%\item
%Proof of $\Nat t, u=Sy, \Nat y \vdash \Tot(t,u)$. We use $\eq$ with premise $\Nat t, \Nat y \vdash \Tot(t,Sy)$. Then we use a cut with $\Nat t, \Nat y \vdash \Tot(t,y)$ and $\Nat t, \Nat y,  \Tot(t,y) \vdash \Tot(t,Sy)$.  $\Nat t, \Nat y \vdash \Tot(t,y)$ is \emph{(open assumption 2)}. 
...
%\begin{enumerate}
%\item
%We prove the first premise $t=0, \Nat y,  \Tot(t,y) \vdash \Tot(t,Sy)$ by $\eq$ and $W L$ from $\vdash \Tot(0,Sy)$.  By $\Tot R$-rule it is enough to prove  $\vdash \Ack(0,Sy,SSy)$, which is some $\Ack R$-axiom.
%\item
%We prove the second premise $t=Sx, \Nat x, \Nat y,  \Tot(t,y) \vdash \Tot(t,Sy)$, by $\eq$ and $W L$ from $\Nat x, \Tot(Sx,y) \vdash \Tot(Sx,Sy)$. By $\Tot L$-rule we have to prove $\Nat x, Sx=x', y=y', \Ack(x',y',z) \vdash \Tot(Sx,Sy)$, by $\eq$ we have to prove $\Nat x, \Ack(Sx,y,z) \vdash \Tot(Sx,Sy)$. We prove $\Nat x, \Ack(Sx,y,z) \vdash \Tot(Sx,Sy)$
...
From Lemma \ref{lemma-ackermann-total} we define a cyclic proof of the totality of the 
inductive predicate $\Ack(t,u,v)$ for the Ackermann function.

\begin{theorem}
\label{theorem-ackermann-total}
There is a cyclic proof of the totality for Ackermann:
$$\Nat t, \Nat u \vdash \Tot(t,u)$$
\end{theorem}

\begin{proof}
We define a regular infinite proof $\Pi$ from $\Pi_0$ in Lemma \ref{lemma-ackermann-total}. 
In $\Pi_0$ we proved  $\Nat t, \Nat u \vdash \Tot(t,u)$ from $3$ open assumptions: $\Nat x, 
\Nat 1 \vdash \Tot(x,1)$ and $\Nat t, \Nat y \vdash \Tot(t,y)$ and $\Nat x, \Nat z \vdash \Tot(x,z)
$ for any variables $x,y,z \not \in \FV(t,u)$. Using Lemma \ref{lemma-ackermann-total} again 
may prove these 3 open assumptions from $\Nat x', \Nat 1 \vdash \Tot(x',1)$ and $\Nat t, \Nat y' 
\vdash \Tot(t,y')$ and $\Nat x', \Nat z' \vdash \Tot(x',z')$ for any variables $x',y',z' \not \in 
\FV(t,u,x,y,z)$. By renaming we obtain the original 3 open assumptions: 
$\Nat x, \Nat 1 \vdash \Tot(x,1)$ and $\Nat t, \Nat y 
\vdash \Tot(t,y)$ and $\Nat x, \Nat z \vdash \Tot(x,z)$ because we have $y' \not \in \FV(t,y)$. 
We obtained 3 regular proof $\Pi_1, \Pi_2, \Pi_3$, with conclusion and assumptions the
3 open assumption. Thus, we define a regular proof $\Pi$ of $\Nat t, \Nat u \vdash \Tot(t,u)$.

We prove the global trace condition for $\Pi$. Each $\Nat t, \Nat u \vdash \Tot(t,u)$ is proved by: $(1) \Nat x, \Nat 1 \vdash \Tot(x,1)$ or $(2) \Nat t, \Nat y \vdash \Tot(t,y)$ or $(3)\Nat x, \Nat z \vdash \Tot(x,z)$. 

In cases $(1)$ and $(3)$ there is a progressing trace from $\Nat t$ to $\Nat x$. 

In case $(2)$ there is a stationary trace from $\Nat t$ to $\Nat t$ and a progressing trace from $\Nat u$ to $\Nat y$. 

Take any infinite path $\pi$ of the proof $\Pi$, in order to prove that there is some infinitely progressing trace. $\pi$ passes infinitely many times through $(1)$ or $(2)$ or $(3)$. 

Assume $\pi$ passes infinitely many times through $(1)$ or $(3)$ and any number of times through $(2)$. Then there is an infinite trace $\tau$ from $\Nat t$ in the root, $\tau$ is infinitely progressing, whenever $\tau$ passes through $(1)$ or $(3)$, and we are done. 

Assume $\pi$ passes finitely many times through $(1)$ or $(3)$. Then from some node $\xi$ on, $\pi$ passes cyclically through $(2)$, progressing each time from some $\Nat u$ to $\Nat y$. From $\xi$ on, the trace $\upsilon$ from the occurrence of $\Nat u$ in $\xi$ progresses in \emph{every} step. Also in this case there is some infinitely progressing trace in $\pi$.
$\Box$ 
