...
\newcommand{\cod}                          {{\tt cod}}
\newcommand{\Rel}                          {{\tt Rel}}
\newcommand{\ColRel}                    {{\tt ColRel}}
\newcommand{\InjRel}                      {{\tt InjRel}}
\newcommand{\InjColRel}                {{\tt InjColRel}}
\newcommand{\TypedRel}                {{\tt TypedRel}}
\newcommand{\TypedInjRel}            {{\tt TypedInjRel}}
\newcommand{\Nodes}                     {{\tt Nodes}}
%\newcommand{\Paths}                    {{\tt Paths}}
\newcommand{\PowerSet}                {{\mathcal P}}
\newcommand{\ProofUnfolding}      {{\tt Proof}}
...
\address{Universit\`{a} di Torino, Torino, Italy}
\email{stefano.berardi@unito.it}

\author[U. deLiguoro]{Ugo de' Liguoro}
\address{Universit\`{a} di Torino, Torino, Italy}
\email{ugo.deliguoro@unito.it}

\author[D. Osorio]{Daniel Osorio}
\address{Universit\`{a} di Torino, Torino, Italy}
\email{dfosoriovalencia@gmail.com}
...
\begin{definition}[Cyclic Proof]
\label{definition-cyclic-proof}
A cyclic proof consists of:
\begin{enumerate}
\item 
a finite proof tree $\Pi$ in sequent calculus for Martin Lof inductive definitions, with 
root $\Root$ and free assumptions called \emph{buds}. 
\item 
a map $f$ from buds to the nodes of $\Graph$, such that the sequent labeling $f(\beta)$ is a renaming of the sequent labeling $\beta$. When $\alpha = f(\beta)$ we say that
$\alpha$ is the \emph{companion} of the bud $\beta$. 
\item 
A proof-graph $\Graph$, whose nodes are those of $\Pi$, and whose edges are those 
of $\Pi$ plus the bud/companion relationship. We write $g = \Card(\Graph)$ for the 
number of nodes of $\Graph$.
\item
a labeling of each edge of the proof-graph with some bicolored relation 
$R \subseteq [1,\MaxInd] \times [1,\MaxInd]$, where  $\MaxInd$ is any upper bound to
the number of inductive formulas $N(t)$ in the antecedent of a sequent of $\Graph$. 
\end{enumerate}
We will include a figure with a description of $R$ for each rule of the system and for the 
bud/companion relationship.
\end{definition}

The translation starts from a cyclic proof $\Graph$ and returns a proof in Martin-Lof
inductive system.

Each (possibly colored) relation $R$ is associated to an arrow $a \rightarrow b$ 
between nodes of $\Graph$, and any two relations $R:a \rightarrow b$, $S:b \rightarrow c$
are said to be composable, with composition $RS: a \rightarrow c$. 

A finite path in $\Graph$ is any finite composable sequence 
$$
\pi  \ \ \
=  \ \ \
R_1:\alpha_1 \rightarrow \alpha_2, \ \ \ 
R_2:\alpha_2 \rightarrow \alpha_3, \ \ \ 
\ldots, \ \ \ 
R_n:\alpha_n \rightarrow \alpha_{n+1}
$$
We write $R_1 \ldots R_n : \pi$ to express the fact that $R_1 \ldots R_n$ is the product
of the labels associated to the path $\pi$, and $R_1 \ldots R_n : \alpha_1 \rightarrow \alpha_{n+1}$ to express the fact that $R_1 \ldots R_n$ is the product
of the labels of some path $\pi$ from $\alpha_1$ to $\alpha_{n+1}$.

We write $\Paths$ for the set of all finite path in $\Graph$. We write $\ProofUnfolding$ for the 
set of paths in $\Graph$ from the root $\Root$ of $\Graph$. The set $\ProofUnfolding$ is a 
possibly infinite proof-tree with root $\Root$: 
we call $\ProofUnfolding$ \emph{the unfolding of $\Graph$}.


\section{A Quick Overview of the GTC, Injectivity and PDC Conditions}

\subsection{Binary Relations on a Finite Set}
...
$[a,b]$ is any $[a',b'] \subseteq [a,b]$, namely with $[a',b']=\emptyset$ or with
$a \le a' \le b' \le b$.

We consider binary relations on a finite set: for sake of simplicity we only 
consider the finite sets $[1,\MaxInd]$. The integer $\MaxInd \in \N$ is some
fixed value representing an upper bound to the number of inductive formulas in the
antecedent of the sequents of $\Graph$. 
...
$R(x)$ for the set $\{y | x R y\}$ of $R$-images of $x \in \dom(R)$. 
We call $\DomCard(R) = \Card(\dom(R))$ 
the domain-cardinality of $R$. 

%We extend $\dom$ and $\DomCard$ to relation sequences by 
%taking the product of the sequence: $\dom(R_1, \ldots, R_n) =  \dom(R_1 \ldots R_n)$ and 
%$\DomCard(R_1, \ldots, R_n) = \DomCard(R_1 \ldots R_n)$.

We denote with $\Rel = \PowerSet([1,\MaxInd] \times [1,\MaxInd])$ 
the set of all binary relations on the finite set $[1,\MaxInd]$.


\subsection{Bicolored Typed Binary relations on a Finite Set}
A bicolored relation, a colored relation for short, 
is any tuple $(\Stat,\Prog) \in \Rel^2$ of binary relations on
$[1,\MaxInd]$ such that $\Stat \cap \Prog = \emptyset$. We say that any pair $(x,y) \in \Stat$
has the \emph{stationary color}, and any pair $(x,y) \in \Prog$ has the 
\emph{progressing color}. We call $\ColRel \subseteq \Rel^2$ the set of colored relations. 

If $R= (\Stat,\Prog) \in \ColRel$, we define $\universe{R} = \Stat \cup \Prog \in \Rel$ 
the associated (uncolored) relation and we say that $R$ is injective if $\universe{R}$ is. 
$\InjColRel \subseteq \ColRel$ is the subset of \emph{injective colored relations}.
For colored relation we define $\dom(R) = \dom(\universe{R})$ and 
$\DomCard(R) = \DomCard(\universe{R})$. 

We suppose be fixed some finite proof-graph 
$\Graph = \{\alpha, \beta, \ldots\}$ whose elements are called nodes. 

\begin{definition}
\label{definition-typed-injective-colored-rel}
\begin{enumerate}
\item
We call $\TypedRel = \ColRel \times \Graph \times \Graph$ the set of typed colored relation.
\item
We write any triple $(R,\alpha,\beta)$ as $R: \alpha \rightarrow \beta$. 
\item
We say that $(R,\alpha,\beta)$ has domain $\alpha$ and codomain $\beta$.
\item
We say that $(R,\alpha,\beta)$ is injective if $\universe{R}$ is.
\item
$\TypedInjRel$ is the subset of
typed injective colored relations
\end{enumerate}
\end{definition}

We fix a set $\setR \subseteq \TypedInjRel$ of typed injective colored relations in the paper. 
We define a composion of colored relation. The color of an edge $(x,z) \in RS$ is progressing 
if and only if there are two edge $(x,y) \in \universe{R}$ and $(y,z) \in \universe{S}$ 
and at least one of them is progressing. 
The color of an edge $(x,z) \in RS$ is stationary otherwise.

 
\begin{definition}[Colored Composition] 
\label{definition-colored-composition}
For any $R = (\Stat,\Prog), S = (\Stat',\Prog')\in\ColRel$ we set:
$$
RS = (\Stat'', \Prog'') 
\ \ \  \mbox{with} \ \ \ 
(\Prog'' = \Stat \Prog' \cup \Prog \Stat' \cup \Prog \Prog')
\ \ \  \mbox{with} \ \ \ 
(\Stat'' = \Stat \Stat' \setminus \Prog'')
$$ 
For any $R:\alpha \rightarrow \beta, S:\beta \rightarrow \gamma \in\TypedRel$ we define
$(R:\alpha \rightarrow \beta)(S:\beta \rightarrow \gamma) = (RS:\alpha \rightarrow \gamma)$.

$\setR^+ \subseteq \ColRel$ is the closure of a family $\setR \subseteq \ColRel$ 
under colored composition.
\end{definition}
...
(\Stat \cup \Prog) \cup (\Stat' \cup \Prog')
$$
We just proved that $\universe{RS} = \universe{R}\universe{S}$ for all $R, S \in \ColRel$, 
namely, that uncoloring and colored composition commute.
\\

In the case of $R = (\Stat,\Prog), S = (\Stat',\Prog')\in\InjColRel$ we have 
$\Stat \Stat' \cap \Prog'' = \emptyset$, because there is at most one $z$ such that
$(x,y) \in \universe{R}$ and $(y,z) \in \universe{S}$.
For injective colored relations, 
the definition of composition can be simplified to $\Stat'' = \Stat \Stat'$ and to 
$ \Prog'' = \Stat \Prog' \cup \Prog \Stat' \cup \Prog \Prog'$. 


\subsection{GTC and PDC Conditions For  Typed Colored Relations}

We introduce the the Progressing Domain Condition PDC. We will prove that when all colored 
relations in a cyclic proof $\Graph$ are injective, then PDC is a sufficient condition for 
correctness of $\Graph$. 

\begin{definition}[The Progressing Domain Condition PDC]
\label{definition-PDC}
Assume that $\setR \subseteq \ColRel$, that $R=(\Stat,\Prog) \in \ColRel$, and that 
$X,Y \subseteq [1,\MaxInd]$ are sets of positive integers.

\begin{enumerate}
\item
\label{definition-PDC-01}
$R$ is \emph{progressing from $X$ to $Y$} if and only if 
$\Prog \cap (X \times Y) \not = \emptyset$.

\item
\label{definition-PDC-02}
The \emph{Progressing Domain Condition (PDC)} for 
$(R : \alpha \rightarrow \alpha)  \in \ColRel$ says: 
$R$ is progressing from $\dom(R)$ to $\dom(R)$.

\item
\label{definition-PDC-03}
\emph{$\setR^+$ satisfies PDC} if and only if: $(R : \alpha \rightarrow \alpha)$ satisfies PDC
for all $(R : \alpha \rightarrow \alpha) \in \setR$.
\end{enumerate}
\end{definition}

When not all colored relations in a cyclic proof $\Graph$ are injective, then we need a stronger
condition for correctness of $\Graph$: the Global Trace Condition GTC.

\begin{definition}[The Global Trace Condition GTC]
\label{definition-GTC}
Assume that $\setR \subseteq \TypedRel$.
...
\item
\label{definition-GTC-03}
$\sigma$ is infinitely progressing in $\pi$ if and only if $\sigma(i) \Prog_i \sigma(i+1)$ 
for infinitely many $i \ge i_0$.

\item
\label{definition-GTC-04}
$\setR^+ $ satisfies GTC if and only if for all infinite paths
$\pi$ there is some infinitely progressing trace $\sigma$ in $\pi$.

\end{enumerate}
\end{definition}

Remark that the Global Trace Condition (GTC) is a property of infinite composable
chains of $\setR^+$, while PDC is a property of single relations
$R:\alpha \rightarrow \alpha \in \setR^+$, which is extended to a property of  $\setR^+$. 

Through the paper assume that:

\begin{enumerate}
\item  
$\setR^+ \subseteq \TypedRel$ is a set of typed \emph{injective} colored relations.
\item
$\setR^+$ satisfies PDC
 \emph{(i.e., all relation of the form $R:\alpha \rightarrow \alpha$ in $\setR^+$ satisfy PDC)}
\end{enumerate}
...
Let $R = (\{(1,1)\},\{(2,1)\}) \in \ColRel$ with $R$ stationary on $(1,1)$
and progressing on $(2,1)$ and  $\universe{R} = \{(1,1),(2,1)\}$. 
Let $\setR=\{R: \alpha \rightarrow \alpha\} \subseteq \TypedRel$.
Then $\dom(R) = \{1,2\}$ and $R$ is progressing on $(2,1)$, therefore 
$R: \alpha \rightarrow \alpha$ satisfies PDC. From $RR=R$ we deduce that  
$\setR^+=\{R: \alpha \rightarrow \alpha\}$, therefore $\setR^+$ satisfies PDC. 

However, there is a unique infinite path 
$
\pi = 
R: \alpha \rightarrow \alpha, R: \alpha \rightarrow \alpha, R: \alpha \rightarrow \alpha, \ldots$.
The only infinite traces in $\pi$ are $(1,1,1,\ldots)$ and $(2,1,1,1,\ldots)$. The first trace never progresses, the second trace progresses \emph{only once}, on the first arrow $(2,1)$. 

So $\setR^+$ does \emph{not} satisfy GTC. The fact that GTC is false for $\setR^+$ 
is not in contradiction with what we said. Indeed, $R: \alpha \rightarrow \alpha \in \setR^+$ 
is  \emph{not} injective, given that $1$ has two $\universe{R}$-counter-images, 
namely $1$ and $2$.
\end{example}
...
We assume having some tree $\ProofUnfolding \subseteq \Paths$ of paths. We assume having
some map $\size$, assigning an expression denoting a positive integer 
$\size(\pi,i)$ to each $\pi \in \ProofUnfolding$ and each $i \in [1,\MaxInd]$.

We assume that if $x$ $R$-progresses to $y$ when moving from $\pi:\alpha \rightarrow \beta$ 
to $\pi_1 = \pi, (R:\beta \rightarrow \gamma)$, 
then we can prove $\size(\pi, x) > \size(\pi_1, y)$, while if $(x,y)$ is $R$-stationary then 
we can prove $\size(\pi, x) = \size(\pi_1, y)$. 
...
Furthermore, in the case of \emph{injective} relations, the images of distinct $x, x', \ldots
\in \dom(S_i)$ are distinct in $ \dom(S_{i+1})$, therefore
$\Card(\dom(S_i)) \le \Card(\dom(S_{i+1})$. Let $c_i = \Card(\dom(S_i))$ be the cardinality
of the $i$-th domain of the stable path: then $c_1 \le c_2 \le \ldots \le c_m \le \ldots$,
the cardinality sequence is weakly increasing. 
...
c_{a_h} = \ldots = \ldots 
$$
We decompose the stable path $\pi' = S_1, S_2, \ldots$ in the same way:
$$
S_{a_1}, \ldots, S_{b_1}, \ \ \ 
\ldots, \ \ \ 
...
S_{a_{h}}, \ldots, \ldots
$$ 
We call these subsequences the \emph{components} of $\pi$. The behaviour of each
$S_i$ in each component is particulary simple: a bijection.
Indeed, by injectivity we can prove that $S_{a_1}$ is a bijection from 
...
All $S_j \ldots S_{k-1} : \alpha \rightarrow \beta$ with $j, k \in [a_1, b_1]$, $j>k$
are biejctions.
The same is true for all sub-sequences, including the infinite subsequence $S_{a_{h}}, \ldots$.

This means that eventually the traces all follow the bijections 
$S_j \ldots S_{k-1} : \alpha \rightarrow \beta$ in $S_{a_{h}}, \ldots$,
and from the index $a_h$ on there are exactly $c$ traces, with $c = c_{a_h}$
the common cardinality of all $\dom(S_j)$ for $j \ge a_{h}$. Assume $g = \Card(\Graph)$.
There are at most $\newton{\MaxInd}{c}$ different sets of
cardinality $c$, and $g$ nodes, therefore for some node $\alpha$ and some 
$j > k \ge a_h$ we have  $S_j \ldots S_{k-1} : \alpha \rightarrow \alpha$,
with same domain and codomain, and 
$\dom(S_j) = \dom(S_k)$. $S_j \ldots S_{k-1}$ is a bijection from 
$\dom(S_j)$ to $\dom(S_k) = \dom(S_j)$.
We will prove that $\dom(S_j) = \dom(S_j \ldots S_{k-1}) = \dom(S_k)$.
PDC implies that $S_j \ldots S_{k-1}$ is progressing from some 
$x \in \dom(S_j)$ to some $y \in \dom(S_k)$. This progression happens at least once each 
$g \newton{\MaxInd}{c}$ steps.

This suggests that we first take a stable sequence $S_1, \ldots, S_m$, then we decompose
it into components $S_{a_i}, \ldots, S_{b_i}$ for $i \in [1,h]$;
and eventually we prove correctness of the cyclic proof using induction with one parameter 
$$
\size(\dom(S_{a_h} \ldots S_{b_h})) 
=
\Sigma_{x \in \dom(S_{a_h} \ldots S_{b_h})} \size(\pi,x)
$$ 
Whenever some $x \in \dom(S_{a_h} \ldots S_{b_h})$ progresses then this sum decreases. 
\\

However, this idea is incomplete in several ways, as soon as we check it in detail.

\begin{enumerate}

\item
First, given a finite prefix $R_1, \ldots, R_n$ of $\pi$, we ignore whether the last component 
$S_{a_{h}}, \ldots, S_{b_h}$ of the stable version $S_1, \ldots, S_m$ of $R_1, \ldots, R_n$ 
is the last component of the stable version for infinite sequence $\pi$. It could not be.
The grouping required to produce a stable relation sequence may erase one or more components 
of $R_1, \ldots, R_n$, starting from the rightmost one. 

We can fix this objection by taking \emph{nested induction} on one index for each component:
$$
\size(\dom(S_{a_1} \ldots S_{b_1})) 
=
\Sigma_{x \in \dom(S_{a_1}\ldots S_{b_1})} \size(\pi,x), \ \ \ 
\ldots, \ \ \ 
\size(\dom(S_{a_h} \ldots S_{b_h})) 
=
\Sigma_{x \in \dom(S_{a_h}\ldots S_{b_h})} \size(\pi,x)
...
$\Sigma_{x \in \dom(S_{a_i}\ldots S_{b_i})} \size(\pi,x)$.

\item
Second, if $x \in \dom(S_{a_h}\ldots S_{b_h})$ has a set 
$S_{a_h}\ldots S_{b_h}(x)$ of one or more $S_{a_h}\ldots S_{b_h}$-images, 
...
some $y \in S_{a_h}\ldots S_{b_h}(x)$ such that $\size(\beta_{b_h},y)$ is maximum.
In this case, the trace through $y$ requires more steps to decrease to zero. 
This suggests switching to the indexes
$$
\size(S_{a_i} \ldots S_{b_i}) 
=
\Sigma_{x \in \dom(S_{a_i} \ldots S_{b_i})} 
\max(\{\size(\pi,y) | y \in S_{a_i} \ldots S_{b_i}(x)\}
$$
expressing such worst-case possibility for traces.

\item
Third, we can have up to $g \newton{\MaxInd}{c_{a_i}}$ steps until the index number $i$
decreases (even if such a bound looks like a vast exaggeration). We need $h$ auxiliary indexes
$t_i \in [0, g \newton{\MaxInd}{c_{a_i}}]$ which, whenever the index number $i$ 
is the last index, counts how many steps we can still have until the index number $i$ decreases. 
Such an index decreases by $1$ at each step in which $\size(S_{a_i} \ldots S_{b_i}) $,
the index number $i$, is the last index and it does not decrease.

\end{enumerate}

This is no formal translation, rather this a plan which we will realize in the paper. 

We first to derive some elementar properties 
of the sequences of uncolored injective relations on $[1,\MaxInd]$ which are required to
...
\section{Domain-Stable Sequences and Grouping for Binary Relations}
We define the grouping relation between sequences of (uncolored) relations. We define
a total ordering $\le$ on grouping and we prove that there exists a minimum stable
grouping w.r.t. $\le$.

We introduce the notion of grouping through an example.
A possible grouping of $\vec{R} = R_1, R_2, R_3, R_4, R_5, R_6 \subseteq \Rel$ 
...
Assume $\vec{T} = T_1, \ldots, T_m  \subseteq \Rel$ and 
$\vec{R} = R_1, \ldots, R_n  \subseteq \Rel$. 
We define the grouping relation $\vec{T} \grouping \vec{R}$ by induction on $n$ as follows.
\begin{enumerate}
...
Assume $R, S, T \in \Rel$ are any relations and $R_1, \ldots, R_n \subseteq \Rel$
is any sequence of relations on $[1,\MaxInd]$.
\begin{enumerate}
...
Let $R = \{(1,1)\}$, $S = \{(2,2)\}$. Then $RS=\emptyset$ and 
$\dom(RS) = \emptyset \subset \{1\} = \dom(R) $. 
\end{example}
...
Instead we have $R=T=RS=ST=RST=\{(1,1)\}$, therefore all other groupings  of
$R,S,T$, namely:
\begin{center}
...
from $\dom(R)$ when we compute $\dom(RS)$.
...
and the fact for all subsets of the finite set $[1,\MaxInd]$, any
strict subset has a smaller cardinality.
...
are two minimal stable groupings and they are not comparable. The unique grouping smaller
than $R,ST$ and $RS,T$ is $R,S,T$ itself, which is no stable grouping of $R,S,T$.

However, we can extend the $\grouping$ relation to a total order $\le$:
by totality of $\le$ and finiteness of the number of groupings of $\vec{S}$, 
there is a minimum stable grouping  w.r.t. $\le$ for each $\vec{S}$.
To say otherwise: there is some stable $\vec{T} \grouping \vec{S}$ such that for all stable 
$\vec{U} \grouping \vec{S}$ we have $\vec{U} \ge \vec{T}$. 
...
of the lengths of its products, namely 
$$
(b_1, b_2 - b_1, \ldots, b_{h} - b_{h-1})
$$
...
We conclude that there is a minimum stable grouping w.r.t. $\le$.
...
\item
We call the last product $(S_{a_h} \ldots S_{b_h})$ of $\minStable(\vec{S}) $: 
 the \emph{stable tail} of $\vec{S}$.
\end{enumerate}
\end{definition}
...
Assume $R, S \in \InjRel$ and $\vec{R}=R_1, \ldots, R_n \subseteq \InjRel$.
...
Assume $\vec{R}=R_1, \ldots, R_n \subseteq \InjRel$ is a \emph{domain-stable}
...
If  $i, j \in[1,n]$ and $i < j$ and $\DomCard(R_i) = \DomCard(R_j)$ then $R_i \ldots R_{j-1}$ 
has domain $\dom(R_i)$ and when restricted to $\dom(R_i) \times \dom(R_j)$ is a bijection.
\end{enumerate}
\end{lemma}
...
Assume $\vec{S}=S_1, \ldots S_n \in \Seq(\Rel)$ is any sequence of relations.
\begin{enumerate}

\item
$\vec{S}$ is a constant domain-cardinality sequence if and only if $\vec{S}$ is is stable
and for all $i,j \in [1,n]$ we have:
$$
\DomCard(S_i) = \DomCard(S_j)
$$

\item
If $\vec{S} \not = \nil$, then we call $\DomCard(\vec{S}) = \DomCard(S_1 \ldots S_n)$ 
...
Besides, for all $k \in [i,n]$, $h \in [j,n]$ 
by stability and Lemma \ref{lemma-domain-stability-02}.\ref{lemma-domain-stability-02} 
...
$\dom(S_{i+1})$.  We can picture the maximal traces of $\vec{S}$ from $\dom(S_1)$ as a 
composition of bijections from $\dom(S_i)$ to $\dom(S_{i+1})$, followed by a single 
application of $S_n$, plus some traces passing through $\dom(S_i)$, missing $\dom(S_{i+1})$ 
and dying out.
...
\end{tikzpicture}
\\
{\tiny The traces of $\vec{S}$ from $\dom(S_1)$ are the compositions \\ 
of the bijections from $\dom(S_i)$ to $\dom(S_{i+1})$, followed by $S_n$}
\end{center}

Each sequence can be transformed into a stable sequence by a minimum of grouping,
then it can be decomposed into a concatenation of its components (its maximal
constant domain-cardinality sequences). The domain-cardinality of the sequences
is increasing.
...
Every stable sequence $\vec{S} \in \Seq(\InjRel)$ of injective relations is equal to the 
...
%We define the set of \emph{raising suffixes} the products
%$$
...
%\begin{enumerate}
%\item
%a set $\setS$ of sequences of injective relations and a set $\ProofUnfolding$ of  
%raising suffixes of sequences of $\setS$
%\item 
%a measure $\mu:\ProofUnfolding \rightarrow \N$, which is 
...
%We have a notion of composition between colored relations satisfying $\universe{R S} =
%\universe{R} \cup \universe{S}$.

We say that  a bicolored sequence $\vec{R}$ satisfies domain-stable
condition, the constant domain condition if and only if, respectively, $\universe{\vec{R}}$
satisfies domain-stable
condition, the constant domain condition. Namely, we move the properties of relations to 
colored relation by forgetting colors.
\\

We say that $R$ is necessarily progressing if and only if it is not possible that all infinite
traces passing through $R$ have a stationary step in $\dom(R)$. We say that $R$ is possibly 
stationary if and only if it is possible that all infinite traces passing through $R$ have a 
stationary step in $\dom(R)$.

The formal definition runs as follows.

\begin{definition}[Necessarily Progressing and Possibly Stationary]
\label{definition-possibly-stationary}
Assume $R=(\Stat,\Prog) \in \ColRel$ is an colored relation and 
$X,Y \subseteq [1,\MaxInd]$.

\begin{enumerate}
\item
We say that $R$ is \emph{necessarily progressing} if and only if:
$$
\exists x \in \dom(\universe{R}) \forall y \in \universe{R}(x)((x,y) \in \Prog)
$$

\item
We say that $R$ is \emph{possibly stationary} otherwise:
$$
\forall x \in \dom(\universe{R}) \exists y \in \universe{R}(x)((x,y) \in \Stat)
$$

\item
$R$ is possibly stationary (necessarily progressing) from $X$ to $Y$ if and only if 
$R \cap (X \times Y)$ is possibly stationary (necessarily progressing)

\item
A sequence $R_1, \ldots, R_n$ is possibly stationary (necessarily progressing) if and only if 
$R_1 \ldots R_n$ is possibly stationary (necessarily progressing).

\end{enumerate}
\end{definition}

The empty sequence $\nil$ is possibly stationary.
Indeed, when $n=0$ then $R_1 \ldots R_n$ is the identity on $[1,\MaxInd]$ with only
the stationary color, therefore $R_1 \ldots R_n$ is possibly stationary. 
...
\begin{lemma}[Possibly Stationary Relations]
\label{lemma-possibly-stationary}
Assume $R, S \in \ColRel$.

\begin{enumerate}
\item
\label{lemma-possibly-stationary-01}
If $R$ is necessarily progressing, then $R$ is necessarily 
progressing from $\dom(R)$ to $\dom(S)$.

Conversely, if $R$ is possibly stationaary from $\dom(R)$ to $\dom(S)$, then $R$ is possibly 
stationary.

\item
\label{lemma-possibly-stationary-02}
If $R$ is possibly stationary from $\dom(R)$ to $\dom(S)$ and $S$ is possibly stationary
then $RS$ is possibly stationary.

\item
\label{lemma-possibly-stationary-03}
Assume $R, S$ is stable. If $RS$ is possibly stationary, then $R$ is possibly stationary
from $\dom(R)$ to $\dom(S)$.

\item
\label{lemma-possibly-stationary-04}
Assume that $R, S$ is stable and that $R$ is a bijection from $\dom(R)$ to $\dom(S)$.
If $RS$ is possibly stationary, then $S$ is possibly stationary.

\item
\label{lemma-possibly-stationary-05}
Assume that $R, S$ is stable and that $R$ is a bijection from $\dom(R)$ to $\dom(S)$.
Then $RS$ is possibly stationary if and only if $R$ is possibly stationary from $\dom(R)$ to 
$\dom(S)$ and $S$ is possibly stationary.

\item
\label{lemma-possibly-stationary-06}
Assume that $n \ge 1$, $R_1, \ldots, R_n$ is stable and that $R_i$ is a bijection from $\dom(R_i)$ 
to $\dom(R_{i+1})$ for all $i \in [1,n-1]$. 
Then $R_1 \ldots R_n$ is possibly stationary if and only if $R_i$ is possibly stationary
from $\dom(R_i)$ to $\dom(R_{i+1})$ for all $i \in [1,n-1]$ and $R_n$ is possibly stationary.
\end{enumerate}
\end{lemma}
...
\item
%\label{lemma-possibly-stationary-01}
Assume $R$ is necessarily progressing. By definition, 
there is some $x \in \dom(R)$ such that $R$ is progressing on all $(x,y) \in \universe{R}$. 
In particular, $R$ is progressing on all  $(x,y) \in \universe{R}$ such that $y \in \dom(S)$. 
We deduce that $R$ is necessarily progressing from $\dom(R)$ to $\dom(S)$. 

\item
%\label{lemma-possibly-stationary-02}
Assume $R$ is possibly stationary from $\dom(R)$ to $\dom(S)$ and $S$ is possibly stationary,
in order to prove that $RS$ is possibly stationary. For, we assume that $x \in \dom(RS)$
in order to prove that there is some $z \in \universe{RS}$ such that $RS$ is stationary on
$(x,z)$. From $R$ possibly stationary from $\dom(R)$ to $\dom(S)$ we deduce that
there is some $y \in \dom(S)$ such that $R$ is  is stationary on $(x,y)$. From $y \in \dom(S)$
and $S$ possibly stationary we deduce that there is some $z$ such that $S$ is stationary on
$(y,z)$. We conclude that $RS$ is stationary on $(x,z)$, as wished.

\item
%\label{lemma-possibly-stationary-03}
Assume that $RS$ is possibly stationary and $R,S$ is stable, in order to prove that 
$R$ is possibly stationary from $\dom(R)$ to $\dom(S)$. For, we assume that $x \in \dom(R)$ 
in order to prove that for some $y \in \dom(S)$ we have $R$ stationary on $(x,y)$. From 
$RS$ stationary we deduce that for some $z$ we have $RS$ stationary on $(x,z)$.
From definition of product color we deduce that for some $y \in \dom(S)$ we have 
$R$ stationary on $(x,y)$ and $S$ stationary on $(y,z)$. This includes our thesis. 

\item
%\label{lemma-possibly-stationary-04}
Assume $RS$ is possibly stationary and $R,S$ stable and
$R$ is a bijection from $\dom(R)$ to $\dom(S)$,
in order to prove that $S$ is possibly stationary. For, we assume that $y \in \dom(S)$
in order to prove that for some $z$ we have $S$ stationary on $(y,z)$. From $R$ bijection from 
$\dom(R)$ to $\dom(S)$ and $y \in \dom(S)$ we deduce that there is some $x \in \dom(R)$
such that $(x,y) \in R$. From $R,S$ stable we deduce that $x \in \dom(RS)$. From $RS$
possibly stationary there is some $z$ such that $RS$ is stationary on $(x,z)$. 
From definition of product color we deduce that for some $y' \in \dom(S)$ we have 
$R$ stationary on $(x,y')$ and $S$ stationary on $(y',z)$. In order to prove that $R$  is stationary 
on $(x,y)$, there is still to prove that $y=y'$. 
We obtain that $y=y'$ $R$ bijection from $\dom(R)$ to $\dom(S)$ and $(x,y) \in R$,
$y \in \dom(S)$ . This concluded our proof. 

\item
%\label{lemma-possibly-stationary-05}
Assume that $R,S$ is stable and $R$ is a bijection from $\dom(R)$ to $\dom(S)$.
By points  \ref{lemma-possibly-stationary-02}, \ref{lemma-possibly-stationary-03} 
and \ref{lemma-possibly-stationary-04} above, we deduce that
$RS$ is possibly stationary if and only if $R$ is possibly stationary from $\dom(R)$ to 
$\dom(S)$ and $S$ is possibly stationary.

\item
%\label{lemma-possibly-stationary-06}
By point \ref{lemma-possibly-stationary-05} above and the fact that a product of
bijections is possibly stationary if and only if all bijections in the product are possibly stationary.
\end{enumerate}
\end{proof}

We measure with a degree for how long all traces in the proper suffix  of a path 
could all be stationary.

\begin{definition}[Stationary degree in $\setR^+$]
Assume $\vec{S} \in \Seq(\TypedRel)$ is some constant domain-cardinality sequence
of length $n \ge 1$. The stationary degree $\deg(\vec{S})$ of $\vec{S}$ is the maximum 
$d \in [0, n-1]$ such that $\vec{S}$ has some length $d$ possibly stationary suffix.
\end{definition}

All sequences have the possibly stationary suffix $\nil$ of length $0$.
Therefore $\deg(\vec{S})$ is a maximum on a subset of $[0,n-1]$ including $0$
and it exists. 

When $n=1$ then $\deg(\vec{S}) \in [0,0]$, and we trivially have $\deg(\vec{S})=0$.

Assume $\vec{S}$ is some constant domain-cardinality sequence of length $n \ge 1$. 
Let $c = \DomCard(S_1)$ be the common domain-cardinality of the sequence.
The next Lemma implies that there is a bound for the memory required to compute 
$\deg(\vec{S})$.

\begin{lemma}[Characterization of Stationarity Degree]
\label{lemma-degree}
Assume $\vec{S},S$ is some constant domain-cardinality sequence
and $\vec{S} = S_1, \ldots, S_n$ for some $n \ge 1$. 
Let $d = \deg(\vec{S})$ and $d' = \deg(\vec{S},S)$.

\begin{enumerate}

\item
\label{lemma-degree-01}
Assume $S$ is necessarily progressing.
Then $d'=0$.

\item
\label{lemma-degree-02}
Assume $S$ is possibly stationary and $d=0$.
Then $d'=1$.

\item
\label{lemma-degree-03}
Assume $S$ is possibly stationary and $d \ge 1$ and $S_{n}$ is necessarily progressing
from $\dom(S_{n})$ to  $\dom(S)$. Then $d'=1$.

\item
\label{lemma-degree-04}
Assume $S_n$ is possibly stationary and $d \ge 1$ and $S_{n}$ is possibly stationary
from $\dom(S_{n})$ to  $\dom(S)$. Then $d' = d+1$.
...
%\label{lemma-degree-01}
Assume $S$ is necessarily progressing.
Then $d'=0$ by definition of degree.

\item
%\label{lemma-degree-02}
Assume $S$ is possibly stationary and $d=0$. Then $S_n$ is necessarily progressing,
and by Lemma \ref{lemma-possibly-stationary}.\ref{lemma-possibly-stationary-01}, 
$S_n$ is necessarily progressing from $\dom(S_n)$ to $\dom(S)$. 
By Lemma \ref{lemma-possibly-stationary}.\ref{lemma-possibly-stationary-06} 
we conclude that $d'=1$.

\item
Assume $S$ is possibly stationary and $d \ge 1$ and $S_{n}$ is necessarily progressing
from $\dom(S_{n})$ to  $\dom(S)$. 
By Lemma \ref{lemma-possibly-stationary}.\ref{lemma-possibly-stationary-06} 
we conclude that $d'=1$.

\item
Assume $S_n$ is possibly stationary and $d \ge 1$ and $S_n$ is possibly stationary
from $\dom(S_{n})$ to  $\dom(S)$. 
By Lemma \ref{lemma-possibly-stationary}.\ref{lemma-possibly-stationary-06}
and definition of degree, either $d=n-1$ or $S_{n-d}$ is necessarily progressing
from $\dom(S_{n-d})$ to $\dom(S_{n-d+1})$, while $S_{i}$ is possibly stationary
from $\dom(S_{i})$ to $\dom(S_{i+1})$ for all $i \in [n-d+1,n-1]$. Then $d'=d+1$ by
definition of degree.

\end{enumerate}
\end{proof}

Lemma \ref{lemma-degree} above says that in order to compute $\deg(\vec{S},S)$ for 
$\vec{S},S$ of length $n+1 \ge 2$ we only have to know 
$\deg(\vec{S}) \in [0,\newton{\MaxInd}{c}] \subseteq [0,2^\MaxInd]$, the relation $S$ 
and the last element $S_n$ of $\vec{S}$. 

This is an information requiring a polinomial space in $\MaxInd$, to be accurate a number of
bits bounded by  $(\MaxInd + 2\MaxInd^2)$.
What is relevant is that \emph{do not} have to know the entire sequence $\vec{S},S$, 
whose number of elements is not bounded by an expression in $\MaxInd$. 

%%%%%%%%%%%%%%%%%%%%%%%%%
%% PREVIOUS DEFINITION STATIONARY DEGREE 11:12 21/02/2026
%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{definition}[Stationary degree in $\setR^+$]
%Assume $\vec{C}= S_1, \ldots, S_e, R_1, \ldots, R_d \in \Seq(\TypedRel)$ is some 
%\emph{non-empty} constant domain-cardinality sequence.
%Let $d \in \N$. We say that $\vec{C}$ has stationary degree $d$
%if and only if  and:
%
%\begin{enumerate}
%\item
...
%\end{enumerate}
%
%%We say that $R \in \TypedRel$ has stationary degree $d$
%%if and only if there is some costant domain-degree sequence 
%%$\vec{C}= S_1, \ldots, S_e, R_1, \ldots, R_d \in \Seq(\TypedRel)$ with stationary degree $d$
%%and such that $R =  (S_1 \ldots S_e R_1 \ldots R_d)$, 
%%namely, $R$ is the product of $\vec{C}$.
%
%\end{definition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%

%
%All $R \in \TypedRel$ have at least stationary degree $0$ in $\setR^+$: 
%it is enough to take $\vec{C}=S_1$. In some case we can have a non-trivial degree, too.

We explain the role of stationary degree for constant domain-cardinality sequences. 
If $\vec{C}= S_1, \ldots, S_e, R_1, \ldots, R_{d-1}, R_d$ 
has stationary degree $d$, this means that in the worst case \emph{all} infinite trace $\sigma$
passing through $\vec{C}$ could have $d$ consecutive stationary steps.
Indeed, all infinite $\sigma$ must move from $\dom(R_i)$ to $\dom(R_{i+1})$ through $R_i$, 
therefore must have $d-1$ consecutive stationary steps. 
Then all infinite $\sigma$ could have one more stationary step, because no matter what is 
the element $x \in \dom(R_d)$ of $\sigma$, there is some $y$ such that $R_d$ is stationary 
on $(x,y)$.

Let $c = \DomCard(S_{a_h})$ be the domain-cardinality of the last constant domain-cardinality
sequence in an infinite path $\pi$. From $S_{a_h}$ on there are $c$ infinite traces in $\pi$.
Let $g = \Card(\Graph)$. Our next task is proving that the stationary degree is at most 
$g\newton{\MaxInd}{c}$, therefore every $\newton{\MaxInd}{c}$ steps some trace 
progresses. After $n g \newton{\MaxInd}{c}$steps there are at least $n$ progressions, and the 
maximum number of progressions in the $c$ traces is at least $n/c$.
Therefore after infinitely many steps the maximum is infinite and there is some infinitely 
progressing trace, provided we are able to prove that the stationary degree is at most 
$g \newton{\MaxInd}{c}$.


%Suppose $R \in \TypedRel$ has stationary degree $d$, namely $R$ decomposes as 
%a constant domain-cardinality $R =  S_1 \ldots S_e R_1 \ldots R_d$ 
...
Assume $\vec{C}= S_1, \ldots, S_e, R_1, \ldots, R_{d}$ has stationary degree $d$.
Assume $\vec{C}, T$ is a constant domain-cardinality sequence. Let $g = \Card(\Graph)$.

\begin{enumerate}
\item
\label{lemma-degree-constant-01}
Assume that $T$ is possibly stationary
%has no necessarily progressing $x \in \dom(T)$,
and either $d = 0$ or $R_{d}$ is a stationary bijection from $\dom(R_{d})$ to $\dom(T)$.

Then $\vec{C}, T$ has stationary degree $d+1$.


\item
\label{lemma-degree-constant-02}
Suppose $\vec{C} \in \Seq(\setR^+)$ and $\setR^+$ satisfies PDC.
Then $d \le g \newton{\MaxInd}{c}$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
Assume $\vec{C}= S_1, \ldots, S_e, R_1, \ldots, R_{d}$ has stationary degree $d$,
in particular $R_i$ is a stationary bijection from $\dom(R_i)$ to $\dom(R_{i+1})$,
for all $i \in [1,d-1]$. 
Assume $\vec{C}, T$ is a constant domain-cardinality sequence.
...
Indeed, assume that $T$ has no necessarily progressing $x \in \dom(T)$,
and if $d \ge 1$ then $R_{d}$ is a stationary bijection from $\dom(R_{d})$ to $\dom(T)$.

We have to prove that $\vec{C}, T$ has stationary degree $d+1$, namely that
$R_i$ is a stationary bijection from $\dom(R_i)$ to $\dom(R_{i+1})$,
 for all $i \in [1,d]$, and that $T$ has no necessarily progressing $x \in \dom(T)$.
 
 The entire thesis is part of the assumptions.

\item
Suppose $\vec{C} \in \Seq(\setR^+)$ and $\setR^+$ satisfies PDC, in order to prove
that $d \le \newton{\MaxInd}{c}$. If $d=0$ then $0 \le \newton{\MaxInd}{c}$, 
so let $d \ge 1$.

Assume $i,j \in [1,d]$, $i < j$, hence $i \in [1,d-1]$. Suppose $R_i$ has domain $\alpha_i$ 
and $R_j$ has domain $\alpha_j$. By assumption, $R_i$ is a stationary bijection from 
$\dom(R_i)$ to $\dom(R_{i+1})$.
$\vec{C} = S_1, \ldots, S_e, R_1, \ldots, R_{d+1}$ is constant domain-cardinality sequence, 
therefore it is stable and $\dom(R_i \ldots R_{j-1}) = \dom(R_i)$. 

If $\setR^+$ satisfies PDC, then either $\alpha_i \not = \alpha_j$, or
if  $\alpha_i = \alpha_j$, then $R_i \ldots R_{j-1}$ is \emph{progressing} from 
$\dom(R_i \ldots R_{j-1})$ to $\dom(R_i \ldots R_{j-1})$, namely from  
$\dom(R_i)$ to $\dom(R_i)$. By assumption, $R_i \ldots R_{j-1}$ is \emph{stationary} 
from  $\dom(R_i)$ to $\dom(R_{j})$. This forces $\alpha_i \not = \alpha_j$ or 
$\dom(R_i) \not = \dom(R_{j})$, for all $i,j \in [1,d]$, $i < j$.

Let $c = \DomCard(R_{1})$ the common domain-cardinality of all $R_1, \ldots, R_{d}$. 
There are $d$ different sets $\dom(R_i)$, and for each of them there is
a different pair $(\alpha_i,\dom(R_i)$. These pairs are $g \newton{\MaxInd}{c}$. Indeed,
there are $g$ different domains $\alpha_i$ for $R_i$, and there are $\newton{\MaxInd}{c}$ 
subsets of $[1,\MaxInd]$ of cardinality $c$. We conclude that
$$
d \le g \newton{\MaxInd}{c}
$$  
\end{enumerate}
...
Lemma \ref{lemma-degree-constant} says that if $d$ is any of the degrees of
$\vec{C}$, then $(g\newton{\MaxInd}{c}-d) \in [0,\newton{\MaxInd}{c}]$.
Suppose we extend $\vec{C}$  to a longer constant domain-cardinality sequence with possibly stationary bijections and with a last possibly stationary relation. Then we can extend for at most
$\newton{\MaxInd}{c}-d$ times. When $g\newton{\MaxInd}{c}-d=0$, in 
all constant domain-cardinality extensions $R_1, \ldots, R_{d},T$, either
the relation $R_{d}$ is necessarily progressing between $\dom(R_{d})$ and $\dom(T)$,
or $T$ itself is necessarily progressing.
...
%
%We assume having a size map $\size : \Graph, [1,\MaxInd] \rightarrow \N^+$, 
%returning an expression $\sigma(\alpha,i)$ denoting a 
%\emph{positive} integer. 
%
%Assume $p(\vec{t})$ is the inductive formula number $i$ of node $\alpha$ and
...
%inductive rules in the proof of a closure of $q(\vec{t})$. We only consider examples
%in which the proof of $q(\vec{t})$ is unique.
%
%\begin{enumerate}
%\item 
...
For sake of simplicity we considered the case in which the unique inductive predicate is $N$. 
We define a size map $\size$, taking any node $\alpha \in \Graph$, any $i \in [1,\MaxInd]$, 
and returning $t+1$ if the $i$-th inductive formula in the antecedent of $\alpha$ is $N(t)$,
returning $0$ if there is no $i$-th inductive formula.  We extend size to any path 
$\pi \in \ProofUnfolding$ with last node $\alpha$ by $\size(\pi,i) = \size(\alpha,i)$. 

%%%%% IRRELEVANT ? %%%%%%%%%
% The set of expressions returned by $\size(\alpha,i)$ has at most $g n$ expressions. Paths can % be infinitely many, but the set of expressions returned by $\size(\pi,i)$ is finite.
%%%%%%%%%%%%%%%%%%%%%%%

We extend the size map to sets of integers $X \subseteq [1,\MaxInd]$,
and to typed colored relations $R:\alpha \rightarrow \beta \in \TypedRel$.
 
 %16:19 06/02/2026

%For each colored relation $R \in \ColRel$ we recall that 
%$R(x) = \{y \in [1,\MaxInd] | x \universe{R} y \}$. 


\begin{definition}[Size of Sets and of Relations]
Assume $X \subseteq [1,\MaxInd]$ and $R:\alpha \rightarrow \beta$  is a colored
relation and $\pi \in \ProofUnfolding$ a finite composable path from $\Root$.
\begin{enumerate} 
\item
$\size(\pi, X) = \Sigma_{i \in X} \size(\pi,i)$.

\item
$\size(\pi, R) = \Sigma_{x \in \dom(R)} \max( \{ \size(\pi,y) | y \universe{R} x \} )$.
\end{enumerate}
\end{definition}

\begin{lemma}[Decreasing Size]
\label{lemma-decreasing-size}
Assume $R:\alpha \rightarrow \beta, S:\beta \rightarrow \gamma \in \TypedRel$
and $\pi_1 = \pi, (R:\alpha \rightarrow \beta)$, $\pi_2 = \pi_1, (S:\beta \rightarrow \gamma)$
with $\pi_2 \in \ProofUnfolding$.
\begin{enumerate}
\item
\label{lemma-decreasing-size-01}
$\size(\pi_2, RS) \le \size(\pi_1, R)$.
\item
\label{lemma-decreasing-size-02}
If $\dom(RS) \subset \dom(R)$ then
$\size(\pi_2, RS) < \size(\pi_1, R)$.
\end{enumerate}
\end{lemma}
...
\item
%\label{lemma-decreasing-size-01}
We have $\size(\pi_2, RS) =
\Sigma_{x \in \dom(RS)} \max(\size(\pi_2, RS(x))$. For all $x \in \dom(RS)$ we have
$x \in \dom(R)$ and if $z \in RS(x)$ then for some $y \in R(x)$ we have $y S z$,
therefore $\size(\pi_1,y) \ge \size(\pi_2,z)$. We deduce 
$\max( \{ \size(\pi_2,z) | z \in RS(x) \}\le \max( \{ \size(\pi_1,y) | y \in R(x) \}$, 
hence 
$
\size(\pi_2, RS)                                                                                   =
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_2,z) | z \in RS(x) \})     \le
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_1,y) | y \in R(x) \}             \le
$
(by $\dom(RS) \subseteq \dom(R)$)
$
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi_1,y) | y \in R(x) \}             =
\size(\pi_1, R)
$.
...
Then 
$
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_1,y) | y \in RS(x) \}           <
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi_1,y) | y \in R(x) \}
$, 
because the second sum has more addends and all addends are positive
by definition of $\size(\pi,i)$. We conclude that 
$
\size(\pi_2, RS)                                                                                    =
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_2,z) | z \in RS(x) \})     \le
\Sigma_{x \in \dom(RS)} \max( \{ \size(\pi_1,y) | y \in R(x) \}              <
\Sigma_{x \in \dom(R)}   \max( \{ \size(\pi_1,y) | y \in R(x) \}              =
\size(\pi_1, R)
$, 
as wished.
...
%
%If $\vec{S} = S_1, \ldots, S_n \in \setR^+$ is injective stable then we will define a notion of
%size for $\vec{S}$. For a generic injective sequence $\vec{R}$, instead, we take the stable 
%version $\vec{S}$ of $\vec{R}$ first.
%
%
...
%Suppose the product $R_i = (S_{a_i}\ldots S_{b_i})$ of $\vec{C_i}$
...
\begin{lemma}[Decreasing Size for Constant Domain-Cardinality Sequences]
\label{lemma-decreasing-size-constant}
Assume $\pi_1 = \pi, (R:\alpha \rightarrow \beta)$ and 
$\pi_2 = \pi_1, (S:\beta \rightarrow \gamma) $ and $\pi_2 \in \ProofUnfolding$.
\begin{enumerate}
\item
\label{lemma-decreasing-size-constant-01}
For all constant-cardinality domain colored sequence 
$\vec{C} = R_1, \ldots, R_n$ we have $\size(\pi,R_1 \ldots R_n) = \size(\pi,R_{n})$.

\item
\label{lemma-decreasing-size-constant-02}
For all $R:\alpha \rightarrow \beta \in \TypedRel$ we have 
$\size(\pi,\dom(R)) \ge \size(\pi_1,R)$.
 
\item
\label{lemma-decreasing-size-constant-03}
Assume $R \cap (\dom(R) \times \dom(S))$ is a bijection.
Then $\size(\pi,\dom(R)) \ge \size(\pi_1, R) \ge \size(\pi_1,\dom(S)) \ge \size(\pi_2,S)$.

\label{lemma-decreasing-size-constant-03bis}
Assume $R \cap (\dom(R) \times \dom(S))$ is a bijection.
If $\size(\pi_1,R) = \size(\pi_2,RS)$ then $R$ is possibly stationary.

\item
\label{lemma-decreasing-size-constant-04}
Assume $R,S \in \InjColRel$ is a constant domain-cardinality  sequence.
If $\size(\pi,\dom(R)) = \size(\pi_1,R)$ and $\size(\pi,\dom(R)) = \size(\pi_1,\dom(S))$
and $\size(\pi_1,\dom(S)) = \size(\pi_2,S)$ then $\size(\pi_1,R) = \size(\pi_2,S)$.

\item
\label{lemma-decreasing-size-constant-05}
Assume $R,S \in \InjColRel$ is a constant domain-cardinality sequence.
If $\size(\pi,\dom(R)) = \size(\pi_1,R)$ ($R$ is possibly stationary) 
and $\size(\pi,\dom(R)) >\size(\pi_1,\dom(S))$
or $\size(\pi_1,\dom(S)) > \size(\pi_2,S)$ then $\size(\pi_1,R) > \size(\pi_2,S)$.
...
\item
%\label{lemma-decreasing-size-constant-01}
We assume that $R_1, \ldots, R_n \in \Seq(\ColRel)$ is some constant domain-cardinality  
sequence  with $R = R_1 \ldots R_n$ in order to prove that
$\size\pi,R) = \size\pi,R_{n})$.
By Lemma \ref{lemma-domain-stability}.\ref{lemma-domain-stability-02}, 
$\dom(R) = \dom(R_1)$.
By Lemma \ref{lemma-injective-sequences}.\ref{lemma-injective-sequences-01},
$R_1 \ldots R_{n-1}$ restricted to $\dom(R)$ and  $\dom(R_{n})$ 
is the graph of some bijection $\psi$. Therefore for
all $x \in \dom(R)$ we have $R(x) = R_1 \ldots R_n(x) = R_{n}(\psi(x))$.

If $\psi:X \rightarrow Y$ is a bijection, for any sum $\Sigma_{x \in X} a_{\psi(x)}$, 
we have $\Sigma_{x \in X} a_{\psi(x)} = \Sigma_{y \in Y} a_y$. 

Thus, from $\dom(R) = \dom(R_1)$ and $x R z \equiv \psi(x) R_{n} z$ and $\psi$ bijection 
we deduce that 
$$
\size\pi,R) =
\Sigma_{x \in \dom(R)}             \max(\{\size\pi,z) | z \in R(x) \})                      = 
\Sigma_{x \in \dom(R)}             \max(\{\size\pi,z) | z \in R_{n}(\psi(x))  \})     =
\Sigma_{y \in \dom(R_{n})}     \max(\{\size\pi,z) | z \in R_{n}(y)           \})     =
\size\pi,R_{n})
$$
...
about coloring, we deduce that $\size(\pi,x) \ge size(\pi_1,y)$. 
Thus, $\size(\pi,x) \ge \max(\{\size(\pi_1,z) | z \in R(x) \}$.
We conclude that $size(\pi,\dom(R)) = \Sigma_{x \in \dom(R)} \size(\pi,x) \ge 
\Sigma_{x \in \dom(R)} \max(\{\size(\pi_1,z) | z \in R(x) \}) = \size(\pi_1,R)$.

\item
%\label{lemma-decreasing-size-constant-03}
Assume that $R \cap (\dom(R) \times \dom(S))$ is a bijection $\psi$.
Then for all $x \in \dom(R)$ there is exactly  one $y \in \dom(S)$ with $y = \psi(x)$.
Thus, 
$ 
\size(\pi,R) = 
\Sigma_{x \in \dom(R)} \max(\{\size(\pi,y) | y \in R(x) \}) \ge 
\Sigma_{x \in \dom(R)} \size(\pi,\psi(x)) =  
\Sigma_{y \in \dom(S)} \size(\pi,y) = 
\size(\pi,\dom(S))
$.

We have $\size(\pi,\dom(S)) \ge \size(\pi_1,S)$ by point \ref{lemma-decreasing-size-constant-02} above.

\item
%\label{lemma-decreasing-size-constant-03bis}
Assume $R \cap (\dom(R) \times \dom(S))$ is a bijection and 
$\size(\pi_1,R) = \size(\pi_2,RS)$.
By point\ref{lemma-decreasing-size-constant-01} above we have $\size(\pi_1,R) = \size(\pi_2,S)$.
By point \ref{lemma-decreasing-size-constant-03} above we have
$\size(\pi_1, R) \ge \size(\pi_1,\dom(S)) \ge \size(\pi_2,S)$.
We conclude that
$\size(\pi_1, R) = \size(\pi_1,\dom(S)) = \size(\pi_2,S)$, in particulary that $S$ is possibly
stationary.
...
\item
%\label{lemma-decreasing-size-constant-04}
By assumption we have the equality chain 
$\size(\pi_1,R)  = \size(\pi,\dom(R)) = \size(\pi_1,\dom(S)) = \size(\pi_2,S)$.
%By $R, S$ constant domain-cardinality sequence and $R, S$ injective and
%Lemma \ref{lemma-injective-sequences}.\ref{lemma-injective-sequences-01} 
%we deduce that $R \cap (\dom(R) \times \dom(S))$ is a bijection.
%By point \ref{lemma-decreasing-size-constant-03} above we deduce that 
%$\size(\pi,R) \ge \size(\pi,\dom(S)) \ge \size(\pi_1,S)$, and from $\size(\pi,R) = \size(\pi_1,S)$ 
%we conclude that $\size(\pi_1,R) = \size(\dom(\pi_1,S)) = \size(\pi_2,S)$. 

\item
%\label{lemma-decreasing-size-constant-05}
Assume $\size(\pi,\dom(R)) = \size(\pi_1,R)$. Then
$\size(\pi,\dom(R)) = \size(\pi_1,R) \ge \size(\dom(\pi_1,S)) \ge \size(\pi_2,S)$
by point {lemma-decreasing-size-constant-03} above, therefore $\size(\pi_1,R) > \size(\pi_2,S)$
if and only if $\size(\pi,\dom(R)) > \size(\pi_1,\dom(S))$ or 
$\size(\pi,\dom(R)) >\size(\pi_1,\dom(S))$.
...
%\begin{enumerate}
%\item
%$\setR^+$ satisfies PDC and $setR^+ \subseteq \InjColRel$. 
%\item 
%$R_1, \ldots, R_n, R_{n+1}$ is a constant domain-cardinality sequence of 
%relations in $\setR^+$. 
%\item
%Suppose $\pi_1 = \pi, (R_{n}:\alpha \rightarrow \beta)$ and
%$\pi_2 = \pi, (R_{n+1}:\beta \rightarrow \gamma)$ and 
%$\size(\pi_1, R_1 \ldots R_n) = \size(\pi_2, R_1 \ldots R_n R_{n+1})$.
%\end{enumerate}
%
%Then for all $i, j \in [1,n]$, $i \le j$:
%If $\size(\pi_1,R_1 \ldots R_n) = \size(\pi_2,R_1 \ldots R_n R_{n+1})$,
%then $\size(\pi_1,R_n) = \size(\pi_1,\dom(R_{n+1})) = \size(\pi_2,R_{n+1})$,
...
%
%\begin{proof}
%By Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-01}
%from $\size(\pi_1,R_1 \ldots R_n) = \size(\pi_2,R_1 \ldots R_n R_{n+1})$
%we deduce that $\size(R_n) = \size(\pi_2,R_{n+1})$.
%
%By Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-03}
%we conclude that $\size(\pi_1,R_n) = \size(\pi_1,\dom(R_{n+1})) = \size(\pi_2,R_{n+1})$.
%
%Assume $R_1 \ldots R_n$ has stationary degree $d$.
%If $d=0$, then $R_1, \ldots, R_{n+1} $ has stationary degree $1$ because
%$\size(\pi_1,\dom(R_{n+1})) = \size(\pi_2,R_{n+1})$. Now assume $d \ge 1$. 
%$R_n \cap \dom(R_n) \times \dom(R_{n+1})$ is a bijection. By definition of degree
%$d \ge 1$ we have $\size(\pi,\dom(R_{n})) = \size(\pi_1,R_n)$, therefore 
%$\size(\pi,\dom(R_{n})) = \size(\pi_1,\dom(R_{n+1})) $. Thus,
%$R_n \cap \dom(R_n) \times \dom(R_{n+1})$ is a stationary bijection.
%From $\size(\dom(R_{n+1})) = \size(R_{n+1})$ we conclude that 
%$R_1 \ldots R_{n+1} $ has stationary degree $d+1$.
%\end{proof}


\section{A Decreasing Index for Constant Domain-cardinality Sequences}
Our goal is to provide a decreasing index for sequences. We first consider a particular case.
We claim that the index:
$$
(\size(\vec{S},S), \newton{\MaxInd}{c} - \deg(\size(\vec{S},S))
$$
is decreasing in the case of \emph{constant domain-cardinality sequences}.

Remark that we can decide whether $\size(\dom(R)) = \size(R)$, by checking whether
no $x \in \dom(R)$ is necessarily progressing, namely checking whether
for all $x \in \dom(R)$ there is some $y \in R(x)$ such that $R$ is stationary on $(x,y)$.


\begin{lemma}[A Decreasing Index for Constant Domain-Cardinality Sequences] 
\label{lemma-stationarity-degree}
Assume $\vec{S},S$ is a constant domain-degree sequence with domain-cardinality
$c = \DomCard(S)$. Then

\begin{enumerate}
\item
\label{lemma-stationarity-degree-01}
$\size(\vec{S}) \ge \size(\vec{S},S)$, and either $\size(\vec{S}) > \size(\vec{S},S)$
or $\deg(\vec{S}) < \deg(\vec{S},S)$.

\item
\label{lemma-stationarity-degree-02}
$
(\size(\vec{S}), \newton{\MaxInd}{c} - \deg(\size(\vec{S}))
>
(\size(\vec{S},S), \newton{\MaxInd}{c} - \deg(\size(\vec{S},S))
$
...
\item
%\label{lemma-stationarity-degree-01}
By Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-01}
we have $\size(S_1 \ldots S_n) \ge \size(S_1 \ldots S_n S)$ 
If $\size(\vec{S}) > \size(\vec{S},S)$ we are done, assume that 
$\size(\vec{S}) = \size(\vec{S},S)$ in order to prove that $\deg(\vec{S}) < \deg(\vec{S},S)$.
From $\size(\vec{S}) = \size(\vec{S},S)$ and 
Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-03bis} 
we deduce that $S$ is possibly stationary. 

If $S_n$ is necessarily progressing from $\dom(S_n)$ to $\dom(S)$ then 
by Lemma \ref{lemma-possibly-stationary}.\ref{lemma-possibly-stationary-06}
we conclude that $\deg(\vec{S}) = 0 < 1 = \deg(\vec{S},S)$, as wished.

If $S_n$ is possibly stationary from $\dom(S_n)$ to $\dom(S)$ 
by Lemma \ref{lemma-possibly-stationary}.\ref{lemma-possibly-stationary-06}
and definition of degree we conclude that $\deg(\vec{S}) + 1 =  \deg(\vec{S},S)$, as wished.

\item
%\label{lemma-stationarity-degree-02}
By point \ref{lemma-stationarity-degree-01} above, $\size(\vec{S}) \ge \size(\vec{S},S)$.
In the case $\size(\vec{S}) > \size(\vec{S},S)$ we deduce 
$
(\size(\vec{S}), \newton{\MaxInd}{c} - \deg(\size(\vec{S}))
>
(\size(\vec{S},S), \newton{\MaxInd}{c} - \deg(\size(\vec{S},S))
$.
Assume $\size(\vec{S}) = \size(\vec{S},S)$. Then by the second part of point 
\ref{lemma-stationarity-degree-01} above we obtain $\deg(\vec{S}) < \deg(\vec{S},S)$,
then $\newton{\MaxInd}{c} - \deg(\size(\vec{S}) > 
\newton{\MaxInd}{c} - \deg(\size(\vec{S},S))$. 
...
associate a state to each sequence of injective relations. A state for a sequence is the information 
require to compute the next degree for a constant domain-cardinality component of the 
sequence and it requires.
...
Assume  $\pi = \{(S_i:\alpha_i \rightarrow \alpha_{i+1}) | i \in [1,m]\}$
and $\vec(S) = S_1, \ldots, S_m$ is stable.

Suppose $\vec{S}$ has components $\vec{C_i} = S_{a_i},\ldots, S_{b_i}$ for $i \in [1,h]$.
%and $\pi_i = \{(S_j:\alpha_j \rightarrow \alpha_{j+1}) | j \in [a_i,b_i]\}$ 
%is the typed version of $\vec{C_i}$, with codomain $\beta_i = \alpha_{b_i + 1}$.

Suppose $C_i = S_{a_i}\ldots S_{b_i}$ is the product of $\vec{C_i}$.

Suppose $d_1, \ldots, d_h$ are stationarity degrees of  $\vec{C_1}$, \ldots,  $\vec{C_h}$.

\begin{enumerate}

\item
We say that a sequence $\sigma \in \Seq(\ColRel \times \N \times \ColRel \times \Graph)$ 
is a state of $\vec{S} = S_1, \ldots, S_n$ and we write $\sigma \state \vec{S}$
or $\sigma = \state(\vec{S})$ if and only if 
for some degrees  $d_1, \ldots, d_h$ and some codomains $\beta_1, \ldots, \beta_h$ we have:
$$
\sigma \ \ \ 
= \ \ \ 
(
C_1, d_1, S_{b_1},\alpha_{b_1 + 1};
\ldots;
C_h, d_h, S_{b_h},\alpha_{b_h + 1}
)
$$
%%%%%%%%%%%%%%%%%%%%%%
% THE NEXT CLAUSE IS BUT A CONSEQUENCE OF THE DEFINITION OF DEGREE
% and for all $i \in [1,h]$, either $\size(\dom(R_{b_i})) = \size(R_{b_i})$ or $d_i=0$.
%%%%%%%%%%%%%%%%%%%%%%
We define the size of $\sigma$ in $\pi$ as:
$$
\size(\pi,\sigma)
=
(
\size(\alpha_{b_1 + 1}, C_1), \newton{\MaxInd}{c_1}-d_1; 
\ldots;
\size(\alpha_{b_h + 1}, C_h), \newton{\MaxInd}{c_h}-d_h
)
$$

\item
Assume $\pi = \{(R_i:\alpha_i \rightarrow \alpha_{i+1}) | i \in [1,n]\}$ is any finite path
and $\vec{R} = R_1, \ldots, R_n$ and  $\minStable(\vec{R}) = \vec{S}$
Then $\sigma \state \pi$ if and only if $\sigma \state  \vec{S}$. 

\item 
We call 
$$
\States =  \{\state(\pi) | \pi \in \ProofUnfolding\} 
\subseteq 
\Seq(\ColRel^2 \times [0,2^\MaxInd] \times \Graph)
$$
the set of states. 

\item
Assume $\pi_1:\alpha \rightarrow \beta, \pi_2:\alpha \rightarrow \gamma \in \ProofUnfolding$ 
and $\sigma_1 = \state(\pi_1)$ and 
$\sigma_2 = \state(\pi_2)$. We write $\pi_1 \prec \pi_2$ for: 
$\size(\beta,\sigma_1) < \size(\gamma,\sigma_2)$ in the lexicographic order.
\end{enumerate}
\end{definition}

In the definition above, due to the fact that the domain cardinality is increasing,
we have $h \le \MaxInd+1$.

%We can decide $\size(\beta,\sigma_1) < \size(\gamma,\sigma_2)$ in polynomial time in $\MaxInd$.


%
%\begin{lemma}[State of a Sequence]
%Assume  $\setR^+$ is any set of injective relations and
%$\sigma \state \vec(R)$ in $\setR^+$. Then $\sigma \in \States$.
%\end{lemma}
%
%\begin{proof}
%Assume  $\setR^+$ is any set of injective relations and
...
that $\setR^+ \subseteq \TypedRel$ is a set of injective relations which satisfies the Progressing 
Domain Condition PDC. 
...
Suppose $\pi_1 = \pi, (R : \beta \rightarrow \gamma) \in \ProofUnfolding$ is a typed path in 
$\setR^+$. We define map $\trans:  \States, \TypedRel \rightarrow  \States$ computing the state 
of $\pi_1$ from the state of $\pi$, without accessing to $\pi$. Namely we claim we have

$$
\trans(\state(\pi), R : \beta \rightarrow \gamma) 
= 
\state(\pi_1)
$$

We also check that $\pi \succ \pi_1$ in the lexicographic ordering.
...
The next definition uses the fact that we can compute the degree of $\vec{U},U$ from
the degree of $\vec{U}$, the last element of $\vec{U}$ and $U$, without accessing $\vec{U}$
(Lemma \ref{lemma-degree}). 


\begin{definition}
[The Transition Function $\trans(\sigma,R : \beta \rightarrow \gamma) \in \States$]
\label{definition-transition-state}

Suppose $\pi$ is a path with relations $vec{R}=R_1, \ldots, R_n$ and
$\minStable(\vec{R}) = \vec{S}$. Let 
$$\vec{U_i} = S_{a_i}, \ldots, S_{b_i}:\beta_{i-1} \rightarrow \beta_i$$
be the constant domain-cardinality components of $\vec{S}$.

Let $\sigma = (\sigma_1, \ldots, \sigma_h) = \state(\pi) \in \States$  where 
$\sigma_i = (U_i, \deg(\vec{U_i}),V_i,\beta_i)$ for $i \in [1,h]$ for some
 $U_i = $ the product of $\vec{U_i}$ and some $V_i = S_{b_i} = $
 the last element of $\vec{U_i}$.
 
We define a map $\tau = \trans(\sigma,R:\beta_h \rightarrow \gamma)$
computing the next state.
 
Suppose $(R:\beta_h \rightarrow \gamma) \in \setR^+$
and $\vec{U'_i} = S_{a_i}, \ldots, S_{b_i}, (S_{a_{i+1}} \ldots S_{b_h} R)$, 
with $U'_i$ the product of $\vec{U'_i}$.
As a particular case we assume that $\vec{U'_{h+1}} = R = U'_{h+1}$,
with $R$ the last and unique element of $\vec{U'_{h+1}}$, while for $i \in [1,h]$, we have 
$U_{i+1} = (S_{a_{i+1}} \ldots S_{b_h} R) = $ the last element of $\vec{U'_i}$.
We define two alternative possibilities for the last state of $\tau$:
...
\mbox{ \ \ \ and \ \ \ }
\tau_{i-1} = 
(U'_{i-1}, \deg(\vec{U'_{i-1}}), U'_{i}, \gamma) 
$$

Suppose that $i$ is the unique element of $i \in [1,h+1]$ such that
$\dom(S_{a_j} \ldots S_{b_h} R) = \dom(U_j)$ for all $j \in [1,i-1]$ and:
...
%The index number $i$ restarts from $U'_i$ and 
%$\trans$ sets the stationarity degree number $i$ to its minimum value $0$
...
\tau 
= 
(\sigma_1, \ldots, \sigma_{i-2}, \tau_{i-1})
$$

%$\tau = (\sigma_1, \ldots, \sigma_{i-2}, \tau_{i-1})$ 
%and we define $\tau_{i-1}$ by cases.
%
%\begin{enumerate}
%\item
%\emph{Assume 
%($\size(\dom(U'_{i})) \not = \size(U'_i)$) or 
%($d_{i-1} \ge 1$ and $V_{i-1}$ is progressing from $\dom(V_{i-1})$ to $\dom(U'_{i})$)
%}. 
%Then
%%$U'_i$ is concatenated to the index number $i-1$ and 
%%$\trans$ sets the stationarity degree $d_{i-1}$ to its minimum value $0$
%
%$$
%\tau_{i-1}
%\ \ \ = \ \ \ 
%(U'_{i-1}, 0, U'_{i}, \gamma)
%$$
%
%\item
%Otherwise:
%%Then  $U'_i$ is concatenated to the index number $i-1$ and 
%%$\trans$ increases the stationarity degree $d_{i-1}$ by one:
%
%$$
%\tau_{i-1}
%\ \ \ = \ \ \ 
%(U'_{i-1}, d_{i-1}+1, U'_{i}, \gamma)
%$$
% 
%\end{enumerate}
\end{enumerate}
...
In the definition above, for all $j \in [1,h]$, by definition we have 
$\dom(U'_j) = \dom(U_j \ldots U_h R)$. 
By Lemma \ref{lemma-domain-inclusion-01}.\ref{lemma-domain-inclusion-01}
applied to $U_j$ and $U_{j+1} \ldots U_h R$ we have
$\dom(U_j \ldots U_h R)  \subseteq \dom(U_j)$. 
This is why $i$ in the definition above exists and it is unique.

In the Main Theorem we will prove that that 

$$
\trans(\state(\pi),R:\beta \rightarrow \gamma) 
\ \ \ = \ \ \ 
\state(\pi,R:\beta \rightarrow \gamma)
$$



Recall that we can decide whether $\size(\pi,\dom(R)) = \size(\pi_1,R)$ without computing the
actual size of $\dom(R)$ and $R$, by checking whether
no $x \in \dom(R)$ is necessarily progressing, namely checking whether
for all $x \in \dom(R)$ there is some $y \in R(x)$ such that $R$ is stationary on $(x,y)$.
By Lemma \ref{lemma-domain-stability}.\ref{lemma-domain-stability-01}, 
we have $\dom(U'_i) = \dom(U_i R) \subseteq \dom(U_i)$ for all $i \in [1,n]$. 
In the definition above, we have $i=n+1$ if and only if $\dom(U'_i) = \dom(U_i)$
for all $i \in [1,n]$. 

We extend the transition map $\trans$ to any path in the proof-graph moving from the 
state $\sigma$ of the position
$\pi$ in the proof-graph by a path $\pi'$. 
We repeat the one-step transition map for each edge of the path.

%19:16 19/02/2026

\begin{definition}[Extending the Transition map to Paths]
\label{definition-extending-transition}
Assume $\pi\pi' \in \Paths$ and $\sigma \state \pi$.
We define $\trans(\sigma, \pi')$ by induction on $\pi'$. 
\begin{enumerate}
...
\item
Let $\pi'=\pi'', (R:\alpha \rightarrow \beta)$. Then
$$
\trans(\sigma,\pi') = \trans(\trans(\sigma,\pi''),R:\alpha \rightarrow \beta)
$$
...
\begin{theorem}[Transition Theorem]
\label{theorem-transition}
Suppose $\setR^+$ is injective, $\pi_1 = \pi, (R:\beta \rightarrow \gamma) \in \ProofUnfolding$ 
is a finite path on $\setR^+$ and  $\sigma \state \pi$. 
Let $\tau = \trans(\sigma,R:\beta \rightarrow \gamma) $. Then 
\begin{center}
$(\tau \state \pi_1)$
and $\pi \succ \pi_1$ 
\end{center}
...
\begin{proof}
Suppose $\setR^+$ a set of injective colored relations, 
$\pi = \{R_i:\alpha_i \rightarrow \alpha_{i+1}| i \in [1,n]\}$ and 
$\pi_1 = \pi, (R:\alpha_{b_h+1} \rightarrow \alpha_{b_h+2}) \in \ProofUnfolding$.
Let $\vec{R}= \{R_i | i \in [1,n]\}$.  
Assume $\minStable(\vec{R}) = \vec{S} = S_1, \ldots, S_m$ is the minimum stable grouping
of $\vec{R}$. Let $S_{a_i}, \ldots, S_{b_i}$ for $i \in [1,h]$ be the components of $\vec{S}$,
with $a_1=1$ and $b_h=m$. Let $d_i = \deg(S_{a_i}, \ldots, S_{b_i})$.
Assume $\sigma \state \pi$ with $\sigma = (\sigma_1, \ldots, \sigma_h)$ and for all $i \in [1,h]$:
$$
\sigma_i
\ \ \ = \ \ \ 
(S_{a_i} \ldots S_{b_i}, \ d_i, \ S_{b_i}, \ \alpha_{b_i+1})
$$ 

Let $\tau = \trans(\sigma,R:\alpha_{b_h+1} \rightarrow \alpha_{b_h+2}) $.
We have to prove that $\tau = \state(\pi_1)$. We argue by cases on the definition of $\tau$. 
For all $i \in [1,h]$ we have 
$\dom(S_{a_i} \ldots S_m  R) \subseteq \dom(S_{a_i} \ldots S_m )$ 
by Lemma \ref{lemma-domain-inclusion-01}.\ref{lemma-domain-inclusion-01}.
Suppose that $i$ is the first index in $[1,h]$ such that 
$\dom(S_{a_i} \ldots S_m  R) \subset \dom(S_{a_i} \ldots S_m )$, if any, otherwise $i=h+1$.

We first remark that if $j \in [1,m]$ is the first index such that
$\dom(S_{j} \ldots S_m  R) \subset \dom(S_{j} \ldots S_m )$, then $j = a_i$ for some 
$i \in [1,h]$. Indeed, for some $i \in [1,h]$ we have
$j \in [a_i,b_i]$.
By Lemma \ref{lemma-injective-sequences}.\ref{lemma-injective-sequences-02},
by $\dom(S_{j} \ldots S_m  R) \subset \dom(S_{j} \ldots S_m )$
and by definition of constant domain-cardinality we have 
$$
\DomCard(S_{a_i} \ldots S_m  R) \le 
\DomCard(S_{j} \ldots S_m  R) < 
\DomCard(S_{j} \ldots S_m ) =
\DomCard(S_{a_i} \ldots S_m)
$$ 
From $j \ge a_i$ and $j$ the first index such that 
$\dom(S_{j} \ldots S_m  R) \subset \dom(S_{j} \ldots S_m )$ we deduce $j = a_i$.

Thus, by Lemma \ref{lemma-minstable-inductive}, in the case $i \in [1,h]$ we have 
$$
\minStable(\vec{R},R) = 
S_{a_1}, \ldots, S_{b_1}, \ \ldots, \ S_{a_{i-1}}\ldots, S_{b_{i-1}}, \ (S_{a_i} \ldots S_m  R)
$$
with the last component having number $i$ and consisting of a single relation, 
$(S_{a_i} \ldots S_m  R)$.
...
$$
with the last component having number $h+1$ and consisting of a single relation, $R$.

% 11:01 12/02/2026


Now we prove $\tau = \state(\pi,R:\beta \rightarrow \gamma) $ by cases on the definition of $\tau$.
...
\item 
\emph{Assume that $i=1$ or 
$\DomCard(S_{a_{i-1}} \ldots S_{m} R) \not = \DomCard(S_{a_i} \ldots S_{m}R)$}. 
We have
$$
\tau
\ \ \ = \ \ \ 
(S_{a_1} \ldots S_{b_1}, d_1, S_{b_1},\alpha_{b_1+1}; 
\ \ldots; 
\ S_{a_{i-1}} \ldots S_{b_{i-1}}, d_{i-1}, S_{b_{i-1}},\alpha_{b_{i-1}+1}; 
\ (S_{a_i} \ldots S_{m}R), 0, \ (S_{a_1} \ldots S_{m}R),\gamma)
$$

\begin{enumerate}
\item
Suppose $i=1$. Then $\dom(S_{a_1} \ldots S_{m} R) \subset \dom(S_{a_1} \ldots S_{m})$, 
therefore $\minStable(\vec{R},R) = (S_{a_1} \ldots S_m  R)$, a grouping made of a single 
product, and $\tau = ((S_{a_1} \ldots S_m  R), 0, (S_{a_1} \ldots S_m  R), \gamma)$. 
$0$ is a degree of $U'_1$, therefore $\tau$ 
is a state of $\vec{R},R$. 
From $\dom(S_{a_1} \ldots S_m  R) \subset \dom(S_{a_1} \ldots S_m )$ 
and Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-02}
we deduce $\size(S_{a_1} \ldots S_m  R) < \size(S_{a_1} \ldots S_m)$, 
hence $\pi \succ \pi_1$. 

\item
Suppose $i \in [2,h]$. 
Then $\dom(S_{a_i} \ldots S_m  R) \subset \dom(S_{a_i} \ldots S_m )$, 
%namely
%$
%\dom(S_{a_i} \ldots S_m R) \not = 
%\dom(S_{a_{i-1}} \ldots S_m  R) 
%$ 
%%%%INUTILE
%therefore 
%$$
%\minStable(\vec{R},R) = S_1, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)
%$$
%and $\DomCard(U'_i) \not = \DomCard(U'_{i-1})$, namely
%$
%\DomCard(S_{a_i} \ldots S_m R) \not = 
%\DomCard(S_{a_{i-1}} \ldots S_m  R) 
%$ 
%By injectivity and 
%Lemma \ref{lemma-injective-sequences}.\ref{lemma-injective-sequences-02} 
%we have 
...
%\DomCard(S_{a_{i-1}} \ldots S_m  R) = 
%$ 
%(by stability and Lemma \ref{lemma-domain-stability}.\ref{lemma-domain-stability-02}) 
%$\DomCard(S_{a_{i-1}} \ldots S_{b_{i-1}})$.
Thus $(S_{a_i} \ldots S_m R)$ is the last component of $\vec{R},R$ and it is 
made of a single product, therefore $\tau \state \pi_1$. 
From $\dom(S_{a_i} \ldots S_m  R) \subset \dom(S_{a_i} \ldots S_m )$, 
and Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-02}
we deduce $\size(S_{a_i} \ldots S_m  R) < \size(S_{a_i} \ldots S_m)$, 
hence $\sigma >\tau$.
\item
Suppose $i=h+1 > 1$. Then $\minStable(\vec{R},R) = S_1, \ldots, S_{b_{h}}, R$.
By assumption from $i=h+1>1$ we deduce$\DomCard(R) \not = 
\DomCard(S_{a_h} \ldots S_m  R)$. 
We obtain 
$
\DomCard(R) 
> 
...
Lemma \ref{lemma-domain-stability}.\ref{lemma-domain-stability-02})  
$ 
\DomCard(S_{a_{h}} \ldots S_{b_{h}})
$. 
Namely $R$ is a one-product 
component of $\vec{R},R$, with a larger domain-cardinality, and $\tau$ 
is the concatenation of $\sigma$ and of $(R,0,R,\alpha_{n+2})$. 
We deduce $\tau \state \pi_1$ and 
$\pi \succ \pi_1$. 
\end{enumerate}
...
\item
\emph{Assume that $i>1$ and 
$\DomCard(S_{a_{i-1}} \ldots S_{m} R) = \DomCard(S_{a_i} \ldots S_{m}R)$}. 

\begin{enumerate}

\item
\emph{Assume either $S_{b_{i-1}}$ has some necessarily progressing
$x \in \dom(S_{b_{i-1}})$ or $S_{b_{i-1}}$ is progressing from $\dom(S_{b_{i-1}})$ 
to $\dom(S_{a_i} \ldots S_{m}R)$}. We have

$$
\tau_{i-1} = 
(S_{a_{i-1}} \ldots S_{m} R, \ 0, \ S_{a_{i}} \ldots S_{m} R, \ \gamma)
$$

\begin{enumerate}
\item
Suppose $i \in [2,h]$. 
By assumption, $\dom(S_{a_{i}} \ldots S_{m} R) \subset \dom(S_{a_{i}} \ldots S_{m})$
 therefore $\minStable(\vec{R},R) = S_1, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$.
 By assumption 
 $\DomCard(S_{a_{i-1}} \ldots S_{m} R) = \DomCard(S_{a_i} \ldots S_{m}R)$. 
 We deduce that $S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$
 is the last component of $\vec{R},R$, and it has last element 
 $(S_{a_i} \ldots S_m R)$ and element before the last $S_{b_{i-1}}$.
 $0$ is a stationary degree of the last component. We deduce $\tau \state \pi_1$.
 
 We assumed that either $S_{b_{i-1}}$ has some necessarily progressing
$x \in \dom(S_{b_{i-1}})$ or $S_{b_{i-1}}$ is progressing from $\dom(S_{b_{i-1}})$ 
to $\dom(S_{a_i} \ldots S_m R)$.  
 By Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-05} we deduce that 
 $
 \size(S_{a_{i-1}} \ldots S_{b_{i-1}}) = 
 \size(S_{b_{i-1}}) > 
 $ 
 (By Lemma \ref{lemma-decreasing-size}.\ref{lemma-decreasing-size-01}) 
 $
 \size(S_{a_i} \ldots S_m R) = 
 $
 (by Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-01}
 $ 
 \size(S_{a_{i-1}} \ldots S_m R)$. 
 Thus, $(\pi,\sigma) > (\pi_1,\tau)$.
 
 %13:30 20/02/2026
 \item
 Suppose $i=h+1$. We assumed that $i>1$, therefore $i-1 = h \in [1,h]$ and 
 $\DomCard(S_{a_{h}} \ldots S_{m} R) = \DomCard(R)$. 
 
 We have $\minStable(\vec{R},R) = S_1, \ldots, S_{b_h}, R$. We deduce that 
 $S_{a_h}, \ldots, S_{b_h}, R$
 is a component of $\minStable(\vec{R},R)$ with last element $R$. 
 $0$ is a degree of $S_{a_h}, \ldots, S_{b_h}, R$. Therefore $\tau \state \pi_1$.
 
 We assumed that either $S_{b_h}$ has some necessarily progressing
$x \in \dom(S_{b_h})$, namely $\size(\dom(S_{b_h})) > \size(S_{b_h})$, or 
 $S_{b_h}$ is progressing from $\dom(S_{b_h})$ to $\dom(R)$,
 namely $\size(\dom(S_{b_h} R)) > \size(\dom(R))$.
 ...
 By Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-05} we deduce that 
 $
 \size(S_{a_h} \ldots S_{b_h}) =  
 \size(S_{b_h})  >
 \size(S_{b_h} R) = 
 $
 (By Lemma \ref{lemma-decreasing-size-constant}.\ref{lemma-decreasing-size-constant-01})
 $\size(S_{a_h} \ldots S_{b_h} R)$.  
 Thus, $(\pi,\sigma > (\pi_1, \tau)$.
 \end{enumerate}
 
\item
\emph{Assume $S_{b_{i-1}}$ has no necessarily progressing
$x \in \dom(S_{b_{i-1}})$ and $S_{b_{i-1}}$ is progressing from $\dom(S_{b_{i-1}})$ 
to $\dom(S_{a_i} \ldots S_{m}R)$}.
We have

$$
\tau_{i-1}
= 
(S_{a_{i-1}} \ldots S_m R, d_{i-1}+1, S_{a_{i}} \ldots S_m R)
$$

If $i \in [2,h]$ then $S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$
 is the last component of $\vec{R},R$ with last element $(S_{a_i} \ldots S_m R)$. 
 By Lemma \ref{lemma-degree-constant}.\ref{lemma-degree-constant-01},
 $d_{i-1}+1$ is a degree of $S_{a_{i-1}}, \ldots, S_{b_{i-1}}, (S_{a_i} \ldots S_m R)$. 
 We deduce $\tau \state \pi_1$.
 Let 
 $
 c = \DomCard(S_{a_{i-1}}\ldots S_{b_{m}}) = \DomCard(S_{a_{i-1}} \ldots S_{b_{m}} R) 
 $. 
 We conclude that 
 $$
 \size(\pi,\sigma) 
 = 
 (\size(\pi,S_{a_{1}}\ldots S_{b_{1}}),  \newton{\MaxInd}{c}-d_1, 
 \ldots,  
 \size(\pi,S_{a_{i-2}}\ldots S_{b_{i-2}}),  \newton{\MaxInd}{c}-d_{i-2}, 
 \size(\pi,S_{a_{i-1}}\ldots S_{b_{i-1}}),  \newton{\MaxInd}{c}-d_{i-1})
  \succ
  $$
 ...
 \ldots,  
 \size(\pi,S_{a_{i-2}}\ldots S_{b_{i-2}}),  \newton{\MaxInd}{c}-d_{i-2}, 
 \size(\pi,S_{a_{i-1}}\ldots S_{b_{i-1}}R),  \newton{\MaxInd}{c}-d_{i-1}-1)
 = 
 \size(\tau)
 $$
 
If $i = h+1$ then $S_{a_{h}}, \ldots, S_{b_{h}}, R$ is the last component of $\vec{R},R$ 
with last element $R$. 
By Lemma \ref{lemma-degree-constant}, $d_{h}+1$ 
is a degree of $S_{a_{h}},\ldots S_{b_{h}},R$. We deduce $\tau \state \pi_1$.
Let 
$c = \DomCard(S_{a_{h}},\ldots, S_{b_{h}}) = \DomCard(S_{a_{h}},\ldots, S_{b_{h}},R) $. We conclude that  
 $$
 \size(\pi,\sigma) 
 =
 (\size(\pi,S_{a_{1}}\ldots S_{b_{1}}),  \newton{\MaxInd}{c}-d_1, 
 \ldots,  
 \size(\pi,S_{a_{h-1}}\ldots S_{b_{h-1}}),  \newton{\MaxInd}{c}-d_{h-1}, 
 \size(\pi,S_{a_{h}}\ldots S_{b_{h}}),  \newton{\MaxInd}{c}-d_{h})
  \succ
$$
...
$$
 ...
 \ldots,  
 \size(\pi,S_{a_{h-1}}\ldots S_{b_{h-1}}),  \newton{\MaxInd}{c}-d_{h-1}, 
 \size(\pi,S_{a_{h}}\ldots S_{b_{h}}R),  \newton{\MaxInd}{c}-d_{h}-1)
 = 
 \size(\tau)
 $$
\end{enumerate}
...
\section{The Transition Graph $\setG$}
...
\section{An Exponential Time Translation From Cyclic To Inductive}
Suppose we have a proof in canonical form: each bud $\beta$ is above its companion $\alpha$.
Suppose 
$\pi = \{R_j:\alpha_j \rightarrow \alpha_{j+1} | j \in [1,n]\}$ 
is the path from the companion $\alpha =\alpha_1$ to 
any of its buds $\beta = \alpha_{n+1}$. 
Suppose $\sigma \state \pi$ is the state generated by the transition
function applied to the typed colored relations in the path $\pi$ in the order they are in $\pi$
and starting from the empty state$()$. Suppose 

$$
\sigma = 
(
S_{a_1}\ldots S_{b_1}, d_1, S_{b_1};
\ldots
S_{a_h}\ldots S_{b_h}, d_h, S_{b_h};
)
$$

The proof has bound variables $z_1, \ldots, z_k$, with $k$ the maximum number
of nesting for binding rules in the proof. In each path, the first binding rule introduces $z_1$,
the second binding rule introduces $z_2$ and so forth.
We use fresh variables $z'_1, \ldots, z'_k$ in order to represent $z_1, \ldots, z_k$
in the companion $\alpha$.

Let $\pi_0=$ the empty path. For any $i \in [1,h]$, let 
$\pi_i = \{R_j:\alpha_j \rightarrow \alpha_{j+1} | j \in [1,b_i]\}$ be a path from
the companion $\alpha$, with $\alpha = \alpha_1$.

We define $\Eq_1(\nil,\alpha) = \emptyset$.

Assume $\pi \not = \nil$ be any path from $\alpha$.
We define $\Eq_1(\pi, \alpha)$ as the set of equations 
$\size(\pi_{i-1},a) > \size(\pi_i,b)$ for $(a,b)$ progressing
in $S_{a_i}\ldots S_{b_i}$ and 
$\size(\pi_{i-1},a) = \size(\pi_i,b)$ for $(a,b)$ stationary
in $S_{a_i}\ldots S_{b_i}$. 

Let us fix any bud $\beta$,  any $i \in [0,h]$, $a \in [1,MaxInd]$.
Then $\size(\pi_i,a)$ is a term with at most one free variable, which could be some 
$x_i$ free in the companion $\alpha$, or some bound variable $z_1, \ldots, z_k$
used by a logical rule in the path $\pi$, or their renaming $z'_1, \ldots, z'_k$.
We define a set of equations $\Eq_2(\pi,\alpha)$, with a renaming turning 
$\Eq_2(\pi,\alpha)$ into $\Eq_1(\pi,\alpha)$. We first apply the renaming of the cyclic 
proof from the bud $\beta$ to the companion $\alpha$, then we rename each $z_i$
which is left into $z'_i$. Therefore we can add 
$$
\Eq_1(\alpha)
 =
\bigvee 
\{ 
(\bigwedge \Eq_1(\pi,\alpha)) \wedge  (\size(\sigma) = y_1, \ldots, y_n) | 
\pi \mbox{ path from $\alpha$ to a bud and $n = \length(\size(\sigma))$}
\}
$$

From 
$(\bigwedge \Eq_1(\pi',\sigma)) \wedge (\size(\sigma)=\vec{y})$ 
we deduce 
$\bigwedge \Eq_1(\pi' \conc \pi)) \wedge  (\size(\pi,\sigma) < \vec{y})$. This makes the
proof by lexicographic induction of the modified companion $\alpha'$ correct.

Now we derive the original companion $\alpha$ from t$\alpha'$.

From the modified companion $(\bigwedge \Eq_1(\pi,\sigma)) \wedge (\size(\sigma)=\vec{y})$
with instance $\pi := \nil$ and $\vec{y} :=\size(\sigma)$ we obtain 
some derivable  $(\bigwedge \Eq_1(\pi,\sigma)) \wedge (\size(\sigma)=\vec{y})$.
So we derive $\Eq_1(\alpha)$ and the original companion.
